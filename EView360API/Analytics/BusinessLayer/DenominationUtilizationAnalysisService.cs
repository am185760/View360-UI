...
namespace Analytics.BusinessLayer
{
    public class DenominationUtilizationAnalysisService
    {
        ...
        public DenominationUtilizationAnalysisService(Executor executor)
        {
            _executor = executor;
        }
        ...
        public BaseModel GetDenominationUtilizationAnalysis(DenominationUtilizationAnalysisRequestModel filter)
        {
            ...
            if (filter.fromDate.HasValue)
                queryFilter += "trxn_datetime >= convert(datetime, '" + filter.fromDate.Value.ToString("dd/MM/yyyy HH:mm") + "',103) and ";

            if (filter.toDate.HasValue)
                queryFilter += "trxn_datetime <= convert(datetime, '" + filter.toDate.Value.ToString("dd/MM/yyyy HH:mm") + "',103) ";
            ...
            DataTableResult result = _executor.ExecuteDSRequest<DataTableResult>("GetDenominationUtilization", paramArray, filter.SelectedAtmIds);
