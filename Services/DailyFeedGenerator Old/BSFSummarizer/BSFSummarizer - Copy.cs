...
public class CMS
{
    ...
    private Avanza.CCMS.DAL.Replenishment ProcessReplenishment(AtmAlert atmAlert, Atm atm, UserTask userTask, int counterType1, int counterType2, int counterType3, int counterType4, string alertDatetime, SqlTransaction trxn)
    {
    ...
    public bool BuildSummary(LogableTask task, List<int> reqATMs, bool? executingForOneATM, bool deleteOldData, bool enableDFFGeneration)
    {
    ...
    private void ExtractDayWiseReplenishment()
    {
        ...
        Avanza.CCMS.DAL.Replenishment.ReplenishmentReader reader = Avanza.CCMS.DAL.Replenishment.ExecuteReader("atm_id =" + atm_id + " and rep_datetime>=convert(datetime,'" + Day.ToString("dd/MM/yyyy") + " 00:00:00',103) and rep_datetime<=convert(datetime,'" + Day.ToString("dd/MM/yyyy") + " 23:59:59',103) order by rep_datetime");
        try
        {
            while (reader.Read())
            {
            ...
            foreach (string part in parts)
            {
                ...
                try
                {
                    ...
                    if (atm.IsRecycler.GetValueOrDefault())
                    {
                        ...
                        try
                        {
                            result = ConnectionFactory.ExecuteScalar("SELECT convert(varchar, MAX(counts_cleared_at),103) from vBnaCountsCleared where atm_id = " + atm.ATMId + " and counts_cleared_at < convert(datetime,'" + Day.AddDays(1).ToString("dd'/'MM'/'yyyy") + "',103)");
                            ...
                            object LastSwapRep = ConnectionFactory.ExecuteScalar("SELECT convert(varchar, MAX(rep_datetime),103) from Replenishment where atm_id = " + atm.ATMId + "and is_swap = 1 and rep_datetime < convert(datetime,'" + Day.AddDays(1).ToString("dd'/'MM'/'yyyy") + "',103)");
                    ...
                    for (int i = 0; i < listCurrencies.Count; i++)
                    {
                        ...
                        if (isBNA)
                        {
                            if (atm.IsRecycler.GetValueOrDefault() && listCurrencies[i].Value)
                            {
                                ...
                                balEsc = depositAmount - (recycledWithdrawalSummary[i] * denominationValue);
