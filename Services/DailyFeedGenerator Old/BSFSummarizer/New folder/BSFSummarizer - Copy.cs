...
isnull(cash_remaining3,0),isnull(cash_remaining4,0),isnull(cash_remaining5,0),isnull(cash_remaining6,0),isnull(cash_remaining7,0)
                ...
                //return GetValue(cmd.ExecuteScalar()) - totalWithdrawals;
                {
                    ...
                    return (int.Parse(dt1.Rows[0][0].ToString()) - int.Parse(parts[0].ToString())) + "|" +
                        (int.Parse(dt1.Rows[0][1].ToString()) - int.Parse(parts[1].ToString())) + "|" +
                        (int.Parse(dt1.Rows[0][2].ToString()) - int.Parse(parts[2].ToString())) + "|" +
                        (int.Parse(dt1.Rows[0][3].ToString()) - int.Parse(parts[3].ToString())) + "|" +

                        (int.Parse(dt1.Rows[0][4].ToString()) - int.Parse(parts[4].ToString())) + "|" +
                        (int.Parse(dt1.Rows[0][5].ToString()) - int.Parse(parts[5].ToString())) + "|" +
                        (int.Parse(dt1.Rows[0][6].ToString()) - int.Parse(parts[6].ToString()));
...
isnull(sum(cash_dispensed3),0),isnull(sum(cash_dispensed4),0),isnull(sum(cash_dispensed5),0),isnull(sum(cash_dispensed6),0),isnull(sum(cash_dispensed7),0)
                ...
                where atm_id ={0} and trxn_datetime >= convert(datetime,'{1}',103) 
                and trxn_datetime <=convert(datetime,'{2}',103)",
                atm_id, DateTime.Parse(lastSummaryTrxnDate.ToString()).AddDays(1).ToString("dd/MM/yyyy"), Day.AddDays(-1).ToString("dd/MM/yyyy") + " 23:59:59");
...
and rep_datetime <=convert(datetime,'{1} 23:59:59',103) ",
    ...
    private decimal GetClosingBalance(DateTime dt)
    {
        ...
        if (GetReplenishmentAmount(dt) > 0)
        {
            ...
            //return GetReplenishmentAmount(dt) - (totalWithdrawals - totalPreWithdrawals); right
