...
{
[Serializable()]
public class SummaryCorrectionDataHistory
{
...
private SummaryCorrectionDataHistory( int summary_correction_data_history_id,int atm_id,int notes_remaining1,int notes_remaining2,int notes_remaining3,int notes_remaining4,int notes_dispensed1,int notes_dispensed2,int notes_dispensed3,int notes_dispensed4,int notes_rejected1,int notes_rejected2,int notes_rejected3,int notes_rejected4,int modified_by,DateTime modified_datetime,DateTime rep_datetime,int atm_settlement_id )
{
this.summary_correction_data_history_id = summary_correction_data_history_id;
this.summary_correction_data_history_idChanged = true;
this.atm_id = atm_id;
this.atm_idChanged = true;
this.notes_remaining1 = notes_remaining1;
this.notes_remaining1Changed = true;
...
#region SummaryCorrectionDataHistoryId
private bool summary_correction_data_history_idChanged = false;
private int summary_correction_data_history_id;
public int SummaryCorrectionDataHistoryId
{
get { return summary_correction_data_history_id; }
set { 
summary_correction_data_history_id = value;
summary_correction_data_history_idChanged = true;
}
}
private string summary_correction_data_history_idDbString
{
get
{
return summary_correction_data_history_id.ToString();
}
}
#endregion
#region AtmId
...
#region SummaryCorrectionDataHistoryReader
public class SummaryCorrectionDataHistoryReader:IEntityReader, IEnumerator, IEnumerable 
{
IDataReader reader;
IDbConnection conn;
SummaryCorrectionDataHistory currentSummaryCorrectionDataHistory;
Columns columns;
bool partialRead = false;
private SummaryCorrectionDataHistoryReader() { }
/// 
///
///
...
public SummaryCorrectionDataHistoryReader(IDataReader reader,IDbConnection conn)
{
this.reader = reader;
this.conn = conn;
}
public SummaryCorrectionDataHistoryReader(IDataReader reader, IDbConnection conn, Columns columns)
{
this.reader = reader;
this.conn = conn;
...
{
get { return currentSummaryCorrectionDataHistory; }
...
if (reader.Read())
{
currentSummaryCorrectionDataHistory = new SummaryCorrectionDataHistory();
if (partialRead)
{ if ((columns & Columns.summary_correction_data_history_id) == Columns.summary_correction_data_history_id && reader["summary_correction_data_history_id"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.summary_correction_data_history_id =(int) reader["summary_correction_data_history_id"]; 
if ((columns & Columns.atm_id) == Columns.atm_id && reader["atm_id"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.atm_id =(int) reader["atm_id"]; 
if ((columns & Columns.notes_remaining1) == Columns.notes_remaining1 && reader["notes_remaining1"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.notes_remaining1 =(int) reader["notes_remaining1"]; 
if ((columns & Columns.notes_remaining2) == Columns.notes_remaining2 && reader["notes_remaining2"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.notes_remaining2 =(int) reader["notes_remaining2"]; 
if ((columns & Columns.notes_remaining3) == Columns.notes_remaining3 && reader["notes_remaining3"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.notes_remaining3 =(int) reader["notes_remaining3"]; 
if ((columns & Columns.notes_remaining4) == Columns.notes_remaining4 && reader["notes_remaining4"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.notes_remaining4 =(int) reader["notes_remaining4"]; 
if ((columns & Columns.notes_dispensed1) == Columns.notes_dispensed1 && reader["notes_dispensed1"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.notes_dispensed1 =(int) reader["notes_dispensed1"]; 
if ((columns & Columns.notes_dispensed2) == Columns.notes_dispensed2 && reader["notes_dispensed2"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.notes_dispensed2 =(int) reader["notes_dispensed2"]; 
if ((columns & Columns.notes_dispensed3) == Columns.notes_dispensed3 && reader["notes_dispensed3"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.notes_dispensed3 =(int) reader["notes_dispensed3"]; 
if ((columns & Columns.notes_dispensed4) == Columns.notes_dispensed4 && reader["notes_dispensed4"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.notes_dispensed4 =(int) reader["notes_dispensed4"]; 
if ((columns & Columns.notes_rejected1) == Columns.notes_rejected1 && reader["notes_rejected1"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.notes_rejected1 =(int) reader["notes_rejected1"]; 
if ((columns & Columns.notes_rejected2) == Columns.notes_rejected2 && reader["notes_rejected2"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.notes_rejected2 =(int) reader["notes_rejected2"]; 
if ((columns & Columns.notes_rejected3) == Columns.notes_rejected3 && reader["notes_rejected3"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.notes_rejected3 =(int) reader["notes_rejected3"]; 
if ((columns & Columns.notes_rejected4) == Columns.notes_rejected4 && reader["notes_rejected4"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.notes_rejected4 =(int) reader["notes_rejected4"]; 
if ((columns & Columns.modified_by) == Columns.modified_by && reader["modified_by"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.modified_by =(int) reader["modified_by"]; 
if ((columns & Columns.modified_datetime) == Columns.modified_datetime && reader["modified_datetime"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.modified_datetime =(DateTime) reader["modified_datetime"]; 
if ((columns & Columns.rep_datetime) == Columns.rep_datetime && reader["rep_datetime"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.rep_datetime =(DateTime) reader["rep_datetime"]; 
if ((columns & Columns.atm_settlement_id) == Columns.atm_settlement_id && reader["atm_settlement_id"]!=DBNull.Value)
currentSummaryCorrectionDataHistory.atm_settlement_id =(int) reader["atm_settlement_id"]; 

} else
{
if (reader["summary_correction_data_history_id"] != DBNull.Value)
currentSummaryCorrectionDataHistory.summary_correction_data_history_id = (int) reader["summary_correction_data_history_id"]; 
...
currentSummaryCorrectionDataHistory.isNewEntity = false;
return true;
}
else
return false;
...
public SummaryCorrectionDataHistory CurrentSummaryCorrectionDataHistory
{
get{ return currentSummaryCorrectionDataHistory; }
}
...
#region SummaryCorrectionDataHistory functions

public static SummaryCorrectionDataHistoryReader ExecuteReader(string where, IDbConnection conn, Columns columns)
{
StringBuilder qry = new StringBuilder(200);
qry.Append("select ");
if (Columns.summary_correction_data_history_id == (Columns.summary_correction_data_history_id & columns))
qry.Append("summary_correction_data_history_id,");
...
qry.Append("from Summary_correction_data_history ");
...
return new SummaryCorrectionDataHistoryReader(cmd.ExecuteReader(), conn, columns);
}

static public SummaryCorrectionDataHistoryReader ExecuteReader(string where,Columns columns)
{
...
/// 
/// 
public static SummaryCorrectionDataHistoryReader ExecuteReader(string where,IDbConnection conn)
{
if (conn.State != ConnectionState.Open)
conn.Open();
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
cmd.ExecuteNonQuery();
cmd.CommandText = "Select summary_correction_data_history_id,atm_id,notes_remaining1,notes_remaining2,notes_remaining3,notes_remaining4,notes_dispensed1,notes_dispensed2,notes_dispensed3,notes_dispensed4,notes_rejected1,notes_rejected2,notes_rejected3,notes_rejected4,modified_by,modified_datetime,rep_datetime,atm_settlement_id from Summary_correction_data_history ";
if (where != null && where.Trim().Length > 0)
cmd.CommandText = string.Format("{0} where {1}", cmd.CommandText, where);

return new SummaryCorrectionDataHistoryReader(cmd.ExecuteReader(), conn);
}

static public SummaryCorrectionDataHistoryReader ExecuteReader(string where)
{
return ExecuteReader(where, ConnectionFactory.GetNewConnection());
}

public static SummaryCorrectionDataHistory LoadSummaryCorrectionDataHistory(string where)
{
SummaryCorrectionDataHistoryReader reader = SummaryCorrectionDataHistory.ExecuteReader(where);
SummaryCorrectionDataHistory _summarycorrectiondatahistory = null;
if (reader.Read())
_summarycorrectiondatahistory = reader.CurrentSummaryCorrectionDataHistory;
reader.Close();
return _summarycorrectiondatahistory;
}

public static SummaryCorrectionDataHistory LoadSummaryCorrectionDataHistory(string where, IDbConnection conn)
{
SummaryCorrectionDataHistoryReader reader = SummaryCorrectionDataHistory.ExecuteReader(where, conn);
SummaryCorrectionDataHistory _summarycorrectiondatahistory = null;
if (reader.Read())
_summarycorrectiondatahistory = reader.CurrentSummaryCorrectionDataHistory;
reader.Close(false);
...
public static SummaryCorrectionDataHistory LoadSummaryCorrectionDataHistoryByPk( int summary_correction_data_history_id )
{
return LoadSummaryCorrectionDataHistory( " summary_correction_data_history_id="+summary_correction_data_history_id );
}

public static SummaryCorrectionDataHistory LoadSummaryCorrectionDataHistoryByPk( int summary_correction_data_history_id , IDbConnection conn)
{
return LoadSummaryCorrectionDataHistory(" summary_correction_data_history_id="+summary_correction_data_history_id , conn);
}

public void Save()
{
if (summary_correction_data_history_idChanged || atm_idChanged || notes_remaining1Changed || notes_remaining2Changed || notes_remaining3Changed || notes_remaining4Changed || notes_dispensed1Changed || notes_dispensed2Changed || notes_dispensed3Changed || notes_dispensed4Changed || notes_rejected1Changed || notes_rejected2Changed || notes_rejected3Changed || notes_rejected4Changed || modified_byChanged || modified_datetimeChanged || rep_datetimeChanged || atm_settlement_idChanged )
...
if (this.isNewEntity)
{
qry.Append(@"insert into Summary_correction_data_history( summary_correction_data_history_id,atm_id,notes_remaining1,notes_remaining2,notes_remaining3,notes_remaining4,notes_dispensed1,notes_dispensed2,notes_dispensed3,notes_dispensed4,notes_rejected1,notes_rejected2,notes_rejected3,notes_rejected4,modified_by,modified_datetime,rep_datetime,atm_settlement_id ) values(");
lock (ConnectionFactory.connectionString) { this.summary_correction_data_history_id = ConnectionFactory.GetNextId();
qry.Append(this.summary_correction_data_history_id);
} qry.Append(",");
...
if (!(summary_correction_data_history_idChanged || atm_idChanged || notes_remaining1Changed || notes_remaining2Changed || notes_remaining3Changed || notes_remaining4Changed || notes_dispensed1Changed || notes_dispensed2Changed || notes_dispensed3Changed || notes_dispensed4Changed || notes_rejected1Changed || notes_rejected2Changed || notes_rejected3Changed || notes_rejected4Changed || modified_byChanged || modified_datetimeChanged || rep_datetimeChanged || atm_settlement_idChanged ))
return;
qry.Append("UPDATE Summary_correction_data_history set "); if ( atm_idChanged )
{
...
qry.Replace(',', ' ', qry.Length - 1,1);
qry.Append(" where ");
qry.Append("summary_correction_data_history_id = "+summary_correction_data_history_idDbString);
}
...
{
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "DELETE Summary_correction_data_history where summary_correction_data_history_id = "+ summary_correction_data_history_id;
...
public static void DeleteSummaryCorrectionDataHistorys(string where)
{
ConnectionFactory.ExecuteQuery("delete Summary_correction_data_history where " + where);
}
...
{
summary_correction_data_history_id= 1,
atm_id= 2,
notes_remaining1= 4,
notes_remaining2= 8,
notes_remaining3= 16,
...
#endregion
public void BulkSave(List<SummaryCorrectionDataHistory> dataArray,SqlTransaction dbTrx)
{
DataTable dt = new DataTable();
CreateDataTable(dt);
AddToDataTable(dataArray, ref dt);
SqlBulkCopy bulk = new SqlBulkCopy(dbTrx.Connection, SqlBulkCopyOptions.Default, dbTrx);
bulk.DestinationTableName = "Summary_correction_data_history";
bulk.WriteToServer(dt);
}
public void CreateDataTable(DataTable dt)
{
string[] colNames = Enum.GetNames(typeof(SummaryCorrectionDataHistory.Columns));
for (int i = 0; i < colNames.Length; i++)
{
dt.Columns.Add(colNames[i]);
}
}
public void AddToDataTable(List <SummaryCorrectionDataHistory> transList,ref DataTable dt)
{
foreach (SummaryCorrectionDataHistory tran in transList)
{
DataRow Row;
Row = dt.NewRow();
Row["summary_correction_data_history_id"] =ConnectionFactory.GetNextId();
