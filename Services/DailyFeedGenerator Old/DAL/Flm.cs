...
private Flm( int flm_id,DateTime? activity_datetime,string log,string call_reported_by,string atm_id_no,string atm_location,string type_of_problem,string code,string info_passed_to,DateTime? time_of_info,DateTime? time_of_reaching_site,DateTime? time_of_solving,string attended_by,int? reject_receipt_counter_type1,int? reject_receipt_counter_type2,int? reject_receipt_counter_type3,int? reject_receipt_counter_type4,int? physical_reject_counter_type1,int? physical_reject_counter_type2,int? physical_reject_counter_type3,int? physical_reject_counter_type4,string actual_problem_found,string action_taken,string bna_jammed_counter,string te_bag_no_and_count,DateTime? time_of_call_to_bank,int? sno,int? uploaded_by,DateTime? upload_datetime,string type_of_call )
{
this.flm_id = flm_id;
this.flm_idChanged = true;
this.activity_datetime = activity_datetime;
this.activity_datetimeChanged = true;
this.log = log;
this.logChanged = true;
...
#region FlmId
private bool flm_idChanged = false;
private int flm_id;
public int FlmId
{
get { return flm_id; }
...
private string flm_idDbString
{
get
{
return flm_id.ToString();
}
}
#endregion
...
#region FlmReader
public class FlmReader:IEntityReader, IEnumerator, IEnumerable 
{
IDataReader reader;
IDbConnection conn;
Flm currentFlm;
...
public FlmReader(IDataReader reader,IDbConnection conn)
{
this.reader = reader;
this.conn = conn;
}
public FlmReader(IDataReader reader, IDbConnection conn, Columns columns)
{
this.reader = reader;
this.conn = conn;
...
currentFlm.reject_receipt_counter_type1 =(int?) reader["reject_receipt_counter_type1"]; 
...
currentFlm.reject_receipt_counter_type2 =(int?) reader["reject_receipt_counter_type2"]; 
...
currentFlm.reject_receipt_counter_type3 =(int?) reader["reject_receipt_counter_type3"]; 
...
currentFlm.reject_receipt_counter_type4 =(int?) reader["reject_receipt_counter_type4"]; 
...
currentFlm.physical_reject_counter_type1 =(int?) reader["physical_reject_counter_type1"]; 
...
currentFlm.physical_reject_counter_type2 =(int?) reader["physical_reject_counter_type2"]; 
...
currentFlm.physical_reject_counter_type3 =(int?) reader["physical_reject_counter_type3"]; 
...
#region Flm functions

public static FlmReader ExecuteReader(string where, IDbConnection conn, Columns columns)
{
StringBuilder qry = new StringBuilder(200);
qry.Append("select ");
if (Columns.flm_id == (Columns.flm_id & columns))
qry.Append("flm_id,");
...
return new FlmReader(cmd.ExecuteReader(), conn, columns);
}

static public FlmReader ExecuteReader(string where,Columns columns)
{
...
/// 
/// 
public static FlmReader ExecuteReader(string where,IDbConnection conn)
{
if (conn.State != ConnectionState.Open)
conn.Open();
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
cmd.ExecuteNonQuery();
cmd.CommandText = "Select flm_id,activity_datetime,log,call_reported_by,atm_id_no,atm_location,type_of_problem,code,info_passed_to,time_of_info,time_of_reaching_site,time_of_solving,attended_by,reject_receipt_counter_type1,reject_receipt_counter_type2,reject_receipt_counter_type3,reject_receipt_counter_type4,physical_reject_counter_type1,physical_reject_counter_type2,physical_reject_counter_type3,physical_reject_counter_type4,actual_problem_found,action_taken,bna_jammed_counter,te_bag_no_and_count,time_of_call_to_bank,sno,uploaded_by,upload_datetime,type_of_call from Flm ";
if (where != null && where.Trim().Length > 0)
cmd.CommandText = string.Format("{0} where {1}", cmd.CommandText, where);

return new FlmReader(cmd.ExecuteReader(), conn);
}

static public FlmReader ExecuteReader(string where)
{
return ExecuteReader(where, ConnectionFactory.GetNewConnection());
}

public static Flm LoadFlm(string where)
{
FlmReader reader = Flm.ExecuteReader(where);
Flm _flm = null;
if (reader.Read())
_flm = reader.CurrentFlm;
reader.Close();
return _flm;
}

public static Flm LoadFlm(string where, IDbConnection conn)
{
FlmReader reader = Flm.ExecuteReader(where, conn);
Flm _flm = null;
if (reader.Read())
_flm = reader.CurrentFlm;
reader.Close(false);
...
if (this.isNewEntity)
{
qry.Append(@"insert into Flm( flm_id,activity_datetime,log,call_reported_by,atm_id_no,atm_location,type_of_problem,code,info_passed_to,time_of_info,time_of_reaching_site,time_of_solving,attended_by,reject_receipt_counter_type1,reject_receipt_counter_type2,reject_receipt_counter_type3,reject_receipt_counter_type4,physical_reject_counter_type1,physical_reject_counter_type2,physical_reject_counter_type3,physical_reject_counter_type4,actual_problem_found,action_taken,bna_jammed_counter,te_bag_no_and_count,time_of_call_to_bank,sno,uploaded_by,upload_datetime,type_of_call ) values(");
lock (ConnectionFactory.connectionString) { this.flm_id = ConnectionFactory.GetNextId();
qry.Append(this.flm_id);
} qry.Append(",");
...
if (!(flm_idChanged || activity_datetimeChanged || logChanged || call_reported_byChanged || atm_id_noChanged || atm_locationChanged || type_of_problemChanged || codeChanged || info_passed_toChanged || time_of_infoChanged || time_of_reaching_siteChanged || time_of_solvingChanged || attended_byChanged || reject_receipt_counter_type1Changed || reject_receipt_counter_type2Changed || reject_receipt_counter_type3Changed || reject_receipt_counter_type4Changed || physical_reject_counter_type1Changed || physical_reject_counter_type2Changed || physical_reject_counter_type3Changed || physical_reject_counter_type4Changed || actual_problem_foundChanged || action_takenChanged || bna_jammed_counterChanged || te_bag_no_and_countChanged || time_of_call_to_bankChanged || snoChanged || uploaded_byChanged || upload_datetimeChanged || type_of_callChanged ))
return;
qry.Append("UPDATE Flm set "); if ( activity_datetimeChanged )
{
...
qry.Replace(',', ' ', qry.Length - 1,1);
qry.Append(" where ");
qry.Append("flm_id = "+flm_idDbString);
}
...
{
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "DELETE Flm where flm_id = "+ flm_id;
...
public static void DeleteFlms(string where)
{
ConnectionFactory.ExecuteQuery("delete Flm where " + where);
}
...
info_passed_to= 256,
time_of_info= 512,
time_of_reaching_site= 1024,
time_of_solving= 2048,
attended_by= 4096,
reject_receipt_counter_type1= 8192,
reject_receipt_counter_type2= 16384,
reject_receipt_counter_type3= 32768,
reject_receipt_counter_type4= 65536,
physical_reject_counter_type1= 131072,
physical_reject_counter_type2= 262144,
physical_reject_counter_type3= 524288,
physical_reject_counter_type4= 1048576,
actual_problem_found= 2097152,
action_taken= 4194304,
bna_jammed_counter= 8388608,
te_bag_no_and_count= 16777216,
time_of_call_to_bank= 33554432,
sno= 67108864,
uploaded_by= 134217728,
upload_datetime= 268435456,
type_of_call= 536870912
}
#endregion
public void BulkSave(List<Flm> dataArray,SqlTransaction dbTrx)
{
DataTable dt = new DataTable();
CreateDataTable(dt);
AddToDataTable(dataArray, ref dt);
SqlBulkCopy bulk = new SqlBulkCopy(dbTrx.Connection, SqlBulkCopyOptions.Default, dbTrx);
bulk.DestinationTableName = "Flm";
bulk.WriteToServer(dt);
}
public void CreateDataTable(DataTable dt)
{
string[] colNames = Enum.GetNames(typeof(Flm.Columns));
for (int i = 0; i < colNames.Length; i++)
{
dt.Columns.Add(colNames[i]);
}
}
public void AddToDataTable(List <Flm> transList,ref DataTable dt)
{
foreach (Flm tran in transList)
{
DataRow Row;
Row = dt.NewRow();
Row["flm_id"] =ConnectionFactory.GetNextId();
