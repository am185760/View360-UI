...
{
[Serializable()]
public class CcmsVaultDayBalance
{
...
public CcmsVaultDayBalance() { }
public CcmsVaultDayBalance( DateTime ledger_date,int vault_id,string vault_transaction_type,string transaction_type,string denomination_name,int total_quantity,int total_value,int vault_note_type_id,int denomination_id,DateTime generated_at )
{
...
private CcmsVaultDayBalance( int id,DateTime ledger_date,int vault_id,string vault_transaction_type,string transaction_type,string denomination_name,int total_quantity,int total_value,int vault_note_type_id,int denomination_id,DateTime generated_at )
{
this.id = id;
this.idChanged = true;
this.ledger_date = ledger_date;
...
#region CcmsVaultDayBalanceReader
public class CcmsVaultDayBalanceReader:IEntityReader, IEnumerator, IEnumerable 
{
IDataReader reader;
IDbConnection conn;
CcmsVaultDayBalance currentCcmsVaultDayBalance;
Columns columns;
bool partialRead = false;
private CcmsVaultDayBalanceReader() { }
/// 
///
///
...
public CcmsVaultDayBalanceReader(IDataReader reader,IDbConnection conn)
{
this.reader = reader;
this.conn = conn;
}
public CcmsVaultDayBalanceReader(IDataReader reader, IDbConnection conn, Columns columns)
{
this.reader = reader;
this.conn = conn;
...
{
get { return currentCcmsVaultDayBalance; }
...
currentCcmsVaultDayBalance.isNewEntity = false;
return true;
}
else
return false;
...
public CcmsVaultDayBalance CurrentCcmsVaultDayBalance
{
get{ return currentCcmsVaultDayBalance; }
}
...
#region CcmsVaultDayBalance functions

public static CcmsVaultDayBalanceReader ExecuteReader(string where, IDbConnection conn, Columns columns)
{
...
qry.Append("from Ccms_vault_day_balance ");
...
return new CcmsVaultDayBalanceReader(cmd.ExecuteReader(), conn, columns);
}

static public CcmsVaultDayBalanceReader ExecuteReader(string where,Columns columns)
{
...
/// 
/// 
public static CcmsVaultDayBalanceReader ExecuteReader(string where,IDbConnection conn)
{
if (conn.State != ConnectionState.Open)
conn.Open();
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
cmd.ExecuteNonQuery();
cmd.CommandText = "Select id,ledger_date,vault_id,vault_transaction_type,transaction_type,denomination_name,total_quantity,total_value,vault_note_type_id,denomination_id,generated_at from Ccms_vault_day_balance ";
if (where != null && where.Trim().Length > 0)
cmd.CommandText = string.Format("{0} where {1}", cmd.CommandText, where);

return new CcmsVaultDayBalanceReader(cmd.ExecuteReader(), conn);
}

static public CcmsVaultDayBalanceReader ExecuteReader(string where)
{
return ExecuteReader(where, ConnectionFactory.GetNewConnection());
}

public static CcmsVaultDayBalance LoadCcmsVaultDayBalance(string where)
{
CcmsVaultDayBalanceReader reader = CcmsVaultDayBalance.ExecuteReader(where);
CcmsVaultDayBalance _ccmsvaultdaybalance = null;
if (reader.Read())
_ccmsvaultdaybalance = reader.CurrentCcmsVaultDayBalance;
reader.Close();
return _ccmsvaultdaybalance;
}

public static CcmsVaultDayBalance LoadCcmsVaultDayBalance(string where, IDbConnection conn)
{
CcmsVaultDayBalanceReader reader = CcmsVaultDayBalance.ExecuteReader(where, conn);
CcmsVaultDayBalance _ccmsvaultdaybalance = null;
if (reader.Read())
_ccmsvaultdaybalance = reader.CurrentCcmsVaultDayBalance;
reader.Close(false);
...
public static CcmsVaultDayBalance LoadCcmsVaultDayBalanceByPk( int id )
{
return LoadCcmsVaultDayBalance( " id="+id );
}

public static CcmsVaultDayBalance LoadCcmsVaultDayBalanceByPk( int id , IDbConnection conn)
{
return LoadCcmsVaultDayBalance(" id="+id , conn);
}
...
if (this.isNewEntity)
{
qry.Append(@"insert into Ccms_vault_day_balance( id,ledger_date,vault_id,vault_transaction_type,transaction_type,denomination_name,total_quantity,total_value,vault_note_type_id,denomination_id,generated_at ) values(");
...
if (!(idChanged || ledger_dateChanged || vault_idChanged || vault_transaction_typeChanged || transaction_typeChanged || denomination_nameChanged || total_quantityChanged || total_valueChanged || vault_note_type_idChanged || denomination_idChanged || generated_atChanged ))
return;
qry.Append("UPDATE Ccms_vault_day_balance set "); if ( ledger_dateChanged )
{
...
{
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "DELETE Ccms_vault_day_balance where id = "+ id;
...
public static void DeleteCcmsVaultDayBalances(string where)
{
ConnectionFactory.ExecuteQuery("delete Ccms_vault_day_balance where " + where);
}
...
#endregion
public void BulkSave(List<CcmsVaultDayBalance> dataArray,SqlTransaction dbTrx)
{
DataTable dt = new DataTable();
CreateDataTable(dt);
AddToDataTable(dataArray, ref dt);
SqlBulkCopy bulk = new SqlBulkCopy(dbTrx.Connection, SqlBulkCopyOptions.Default, dbTrx);
bulk.DestinationTableName = "Ccms_vault_day_balance";
bulk.WriteToServer(dt);
}
public void CreateDataTable(DataTable dt)
{
string[] colNames = Enum.GetNames(typeof(CcmsVaultDayBalance.Columns));
for (int i = 0; i < colNames.Length; i++)
{
dt.Columns.Add(colNames[i]);
}
}
public void AddToDataTable(List <CcmsVaultDayBalance> transList,ref DataTable dt)
{
foreach (CcmsVaultDayBalance tran in transList)
{
DataRow Row;
Row = dt.NewRow();
