...
{
    [Serializable()]
    public class ReconciliationBnaHostData
    {
        ...
        public ReconciliationBnaHostData() { }
        public ReconciliationBnaHostData(int reconciliation_bna_host_data_id, int reconciliation_batch_id, string atm_id, string card_number, string transaction_date, string transaction_time)
        {
        ...
        private ReconciliationBnaHostData(int reconciliation_bna_host_data_id, int reconciliation_batch_id, string atm_id, string card_number, string customer_account_no, string transaction_date, string transaction_time, decimal? transaction_amount, string transaction_currency, string transaction_sequence, string transaction_type, string transaction_response, DateTime? transaction_settlement_date, string card_network, string card_issuer)
        {
            this.reconciliation_bna_host_data_id = reconciliation_bna_host_data_id;
            this.reconciliation_bna_host_data_idChanged = true;
            this.reconciliation_batch_id = reconciliation_batch_id;
            this.reconciliation_batch_idChanged = true;
            this.atm_id = atm_id;
            this.atm_idChanged = true;
        ...
        #region ReconciliationBnaHostDataId
        private bool reconciliation_bna_host_data_idChanged = false;
        private int reconciliation_bna_host_data_id;
        public int ReconciliationBnaHostDataId
        {
            get { return reconciliation_bna_host_data_id; }
            set
            {
                reconciliation_bna_host_data_id = value;
                reconciliation_bna_host_data_idChanged = true;
            }
        }
        private string reconciliation_bna_host_data_idDbString
        {
            get
            {
                return reconciliation_bna_host_data_id.ToString();
            }
        }
        #endregion
        ...
        #region ReconciliationBnaHostDataReader
        public class ReconciliationBnaHostDataReader : IEntityReader, IEnumerator, IEnumerable
        {
            IDataReader reader;
            IDbConnection conn;
            ReconciliationBnaHostData currentReconciliationBnaHostData;
            Columns columns;
            bool partialRead = false;
            private ReconciliationBnaHostDataReader() { }
            /// 
            ///
            ///
            ...
            public ReconciliationBnaHostDataReader(IDataReader reader, IDbConnection conn)
            {
                this.reader = reader;
                this.conn = conn;
            }
            public ReconciliationBnaHostDataReader(IDataReader reader, IDbConnection conn, Columns columns)
            {
                this.reader = reader;
                this.conn = conn;
            ...
            {
                get { return currentReconciliationBnaHostData; }
            ...
            public bool Read()
            {
                if (reader.Read())
                {
                    currentReconciliationBnaHostData = new ReconciliationBnaHostData();
                    if (partialRead)
                    {
                        if ((columns & Columns.reconciliation_bna_host_data_id) == Columns.reconciliation_bna_host_data_id && reader["reconciliation_bna_host_data_id"] != DBNull.Value)
                            currentReconciliationBnaHostData.reconciliation_bna_host_data_id = (int)reader["reconciliation_bna_host_data_id"];
                        if ((columns & Columns.reconciliation_batch_id) == Columns.reconciliation_batch_id && reader["reconciliation_batch_id"] != DBNull.Value)
                            currentReconciliationBnaHostData.reconciliation_batch_id = (int)reader["reconciliation_batch_id"];
                        if ((columns & Columns.atm_id) == Columns.atm_id && reader["atm_id"] != DBNull.Value)
                            currentReconciliationBnaHostData.atm_id = (string)reader["atm_id"];
                        if ((columns & Columns.card_number) == Columns.card_number && reader["card_number"] != DBNull.Value)
                            currentReconciliationBnaHostData.card_number = (string)reader["card_number"];
                        if ((columns & Columns.customer_account_no) == Columns.customer_account_no && reader["customer_account_no"] != DBNull.Value)
                            currentReconciliationBnaHostData.customer_account_no = (string)reader["customer_account_no"];
                        if ((columns & Columns.transaction_date) == Columns.transaction_date && reader["transaction_date"] != DBNull.Value)
                            currentReconciliationBnaHostData.transaction_date = (string)reader["transaction_date"];
                        if ((columns & Columns.transaction_time) == Columns.transaction_time && reader["transaction_time"] != DBNull.Value)
                            currentReconciliationBnaHostData.transaction_time = (string)reader["transaction_time"];
                        if ((columns & Columns.transaction_amount) == Columns.transaction_amount && reader["transaction_amount"] != DBNull.Value)
                            currentReconciliationBnaHostData.transaction_amount = (decimal?)reader["transaction_amount"];
                        if ((columns & Columns.transaction_currency) == Columns.transaction_currency && reader["transaction_currency"] != DBNull.Value)
                            currentReconciliationBnaHostData.transaction_currency = (string)reader["transaction_currency"];
                        if ((columns & Columns.transaction_sequence) == Columns.transaction_sequence && reader["transaction_sequence"] != DBNull.Value)
                            currentReconciliationBnaHostData.transaction_sequence = (string)reader["transaction_sequence"];
                        if ((columns & Columns.transaction_type) == Columns.transaction_type && reader["transaction_type"] != DBNull.Value)
                            currentReconciliationBnaHostData.transaction_type = (string)reader["transaction_type"];
                        if ((columns & Columns.transaction_response) == Columns.transaction_response && reader["transaction_response"] != DBNull.Value)
                            currentReconciliationBnaHostData.transaction_response = (string)reader["transaction_response"];
                        if ((columns & Columns.transaction_settlement_date) == Columns.transaction_settlement_date && reader["transaction_settlement_date"] != DBNull.Value)
                            currentReconciliationBnaHostData.transaction_settlement_date = (DateTime?)reader["transaction_settlement_date"];
                        if ((columns & Columns.card_network) == Columns.card_network && reader["card_network"] != DBNull.Value)
                            currentReconciliationBnaHostData.card_network = (string)reader["card_network"];
                        if ((columns & Columns.card_issuer) == Columns.card_issuer && reader["card_issuer"] != DBNull.Value)
                            currentReconciliationBnaHostData.card_issuer = (string)reader["card_issuer"];

                    }
                    else
                    {
                        if (reader["reconciliation_bna_host_data_id"] != DBNull.Value)
                            currentReconciliationBnaHostData.reconciliation_bna_host_data_id = (int)reader["reconciliation_bna_host_data_id"];
                    ...
                    currentReconciliationBnaHostData.isNewEntity = false;
                    return true;
                }
                else
                    return false;
            ...
            public ReconciliationBnaHostData CurrentReconciliationBnaHostData
            {
                get { return currentReconciliationBnaHostData; }
            }
        ...
        #region ReconciliationBnaHostData functions

        public static ReconciliationBnaHostDataReader ExecuteReader(string where, IDbConnection conn, Columns columns)
        {
            StringBuilder qry = new StringBuilder(200);
            qry.Append("select ");
            if (Columns.reconciliation_bna_host_data_id == (Columns.reconciliation_bna_host_data_id & columns))
                qry.Append("reconciliation_bna_host_data_id,");
            ...
            qry.Append("from Reconciliation_bna_host_data ");
            ...
            return new ReconciliationBnaHostDataReader(cmd.ExecuteReader(), conn, columns);
        }

        static public ReconciliationBnaHostDataReader ExecuteReader(string where, Columns columns)
        {
        ...
        /// 
        /// 
        public static ReconciliationBnaHostDataReader ExecuteReader(string where, IDbConnection conn)
        {
            if (conn.State != ConnectionState.Open)
                conn.Open();
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
            cmd.ExecuteNonQuery();
            cmd.CommandText = "Select reconciliation_bna_host_data_id,reconciliation_batch_id,atm_id,card_number,customer_account_no,transaction_date,transaction_time,transaction_amount,transaction_currency,transaction_sequence,transaction_type,transaction_response,transaction_settlement_date,card_network,card_issuer from Reconciliation_bna_host_data ";
            if (where != null && where.Trim().Length > 0)
                cmd.CommandText = string.Format("{0} where {1}", cmd.CommandText, where);

            return new ReconciliationBnaHostDataReader(cmd.ExecuteReader(), conn);
        }

        static public ReconciliationBnaHostDataReader ExecuteReader(string where)
        {
            return ExecuteReader(where, ConnectionFactory.GetNewConnection());
        }

        public static ReconciliationBnaHostData LoadReconciliationBnaHostData(string where)
        {
            ReconciliationBnaHostDataReader reader = ReconciliationBnaHostData.ExecuteReader(where);
            ReconciliationBnaHostData _reconciliationbnahostdata = null;
            if (reader.Read())
                _reconciliationbnahostdata = reader.CurrentReconciliationBnaHostData;
            reader.Close();
            return _reconciliationbnahostdata;
        }

        public static ReconciliationBnaHostData LoadReconciliationBnaHostData(string where, IDbConnection conn)
        {
            ReconciliationBnaHostDataReader reader = ReconciliationBnaHostData.ExecuteReader(where, conn);
            ReconciliationBnaHostData _reconciliationbnahostdata = null;
            if (reader.Read())
                _reconciliationbnahostdata = reader.CurrentReconciliationBnaHostData;
            reader.Close(false);
        ...
        public static ReconciliationBnaHostData LoadReconciliationBnaHostDataByPk(int reconciliation_bna_host_data_id)
        {
            return LoadReconciliationBnaHostData("reconciliation_bna_host_data_id=" + reconciliation_bna_host_data_id);
        }

        public static ReconciliationBnaHostData LoadReconciliationBnaHostDataByPk(int reconciliation_bna_host_data_id, IDbConnection conn)
        {
            return LoadReconciliationBnaHostData(" reconciliation_bna_host_data_id=" + reconciliation_bna_host_data_id, conn);
        }

        public void Save()
        {
            if (reconciliation_bna_host_data_idChanged || reconciliation_batch_idChanged || atm_idChanged || card_numberChanged || customer_account_noChanged || transaction_dateChanged || transaction_timeChanged || transaction_amountChanged || transaction_currencyChanged || transaction_sequenceChanged || transaction_typeChanged || transaction_responseChanged || transaction_settlement_dateChanged || card_networkChanged || card_issuerChanged)
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (reconciliation_bna_host_data_idChanged || reconciliation_batch_idChanged || atm_idChanged || card_numberChanged || customer_account_noChanged || transaction_dateChanged || transaction_timeChanged || transaction_amountChanged || transaction_currencyChanged || transaction_sequenceChanged || transaction_typeChanged || transaction_responseChanged || transaction_settlement_dateChanged || card_networkChanged || card_issuerChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    qry.Append(@"insert into Reconciliation_bna_host_data(reconciliation_bna_host_data_id,reconciliation_batch_id,atm_id,card_number,customer_account_no,transaction_date,transaction_time,transaction_amount,transaction_currency,transaction_sequence,transaction_type,transaction_response,transaction_settlement_date,card_network,card_issuer) values(");
                    lock (ConnectionFactory.connectionString)
                    {
                        this.reconciliation_bna_host_data_id = ConnectionFactory.GetNextId();
                        qry.Append(this.reconciliation_bna_host_data_id);
                    } qry.Append(",");
                ...
                else
                {
                    if (!(reconciliation_bna_host_data_idChanged || reconciliation_batch_idChanged || atm_idChanged || card_numberChanged || customer_account_noChanged || transaction_dateChanged || transaction_timeChanged || transaction_amountChanged || transaction_currencyChanged || transaction_sequenceChanged || transaction_typeChanged || transaction_responseChanged || transaction_settlement_dateChanged || card_networkChanged || card_issuerChanged))
                        return;
                    qry.Append("UPDATE Reconciliation_bna_host_data set "); if (reconciliation_batch_idChanged)
                    {
                    ...
                    qry.Replace(',', ' ', qry.Length - 1, 1);
                    qry.Append(" where ");
                    qry.Append("reconciliation_bna_host_data_id = " + reconciliation_bna_host_data_idDbString);
                }
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE Reconciliation_bna_host_data where reconciliation_bna_host_data_id= " + reconciliation_bna_host_data_id;
        ...
        public static void DeleteReconciliationBnaHostDatas(string where)
        {
            ConnectionFactory.ExecuteQuery("delete Reconciliation_bna_host_data where " + where);
        }
        ...
        {
            reconciliation_bna_host_data_id = 1,
        ...
        #endregion
        public DataTable BulkSave(List<ReconciliationBnaHostData> dataArray, SqlTransaction dbTrx)
        {
            DataTable dt = new DataTable();
            CreateDataTable(dt);
            AddToDataTable(dataArray, ref dt);
            SqlBulkCopy bulk = new SqlBulkCopy(dbTrx.Connection, SqlBulkCopyOptions.Default, dbTrx);
            bulk.DestinationTableName = "Reconciliation_bna_host_data";
            bulk.WriteToServer(dt); return dt;
        }
        public void CreateDataTable(DataTable dt)
        {
            string[] colNames = Enum.GetNames(typeof(ReconciliationBnaHostData.Columns));
            for (int i = 0; i < colNames.Length; i++)
            {
                dt.Columns.Add(colNames[i]);
            }
        }
        public void AddToDataTable(List<ReconciliationBnaHostData> transList, ref DataTable dt)
        {
            foreach (ReconciliationBnaHostData tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
                Row["reconciliation_bna_host_data_id"] = ConnectionFactory.GetNextId();
