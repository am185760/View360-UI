...
namespace Avanza.CCMS.DAL
{
    ...
    public class AtmAlert
    {
        ...
        public AtmAlert() { }
        public AtmAlert(int atm_alert_id, DateTime generated_at, int alert_type_id, int generate_at_retry_remaining, int resolve_at_retry_remaining, bool generate_notification_sent)
        {
        ...
        public AtmAlert(int? atm_id, DateTime generated_at, DateTime? resolve_at, int alert_type_id, DateTime? expiration_time, int generate_at_retry_remaining, int resolve_at_retry_remaining, DateTime? last_invoked_at, bool generate_notification_sent, bool? resolve_notification_sent, string failure_reason, string alert_msg, int? task_id, string entity_type, int? entity_id)
        {
            this.atm_id = atm_id;
        ...
        private AtmAlert(int? atm_id, int atm_alert_id, DateTime generated_at, DateTime? resolve_at, int alert_type_id, DateTime? expiration_time, int generate_at_retry_remaining, int resolve_at_retry_remaining, DateTime? last_invoked_at, bool generate_notification_sent, bool? resolve_notification_sent, string failure_reason, string alert_msg, int? task_id, string entity_type, int? entity_id)
        {
            this.atm_id = atm_id;
            this.atm_idChanged = true;
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.atm_id) == Columns.atm_id && reader["atm_id"] != DBNull.Value)
                            currentAtmAlert.atm_id = (int?)reader["atm_id"];
                        if ((columns & Columns.atm_alert_id) == Columns.atm_alert_id && reader["atm_alert_id"] != DBNull.Value)
                            currentAtmAlert.atm_alert_id = (int)reader["atm_alert_id"];
                        ...
                        if ((columns & Columns.alert_type_id) == Columns.alert_type_id && reader["alert_type_id"] != DBNull.Value)
                            currentAtmAlert.alert_type_id = (int)reader["alert_type_id"];
                        ...
                        if ((columns & Columns.task_id) == Columns.task_id && reader["task_id"] != DBNull.Value)
                            currentAtmAlert.task_id = (int?)reader["task_id"];
                        ...
                        if ((columns & Columns.entity_id) == Columns.entity_id && reader["entity_id"] != DBNull.Value)
                            currentAtmAlert.entity_id = (int?)reader["entity_id"];
        ...
        public static AtmAlertReader ExecuteReader(string where, IDbConnection conn)
        {
            ...
            cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
            cmd.ExecuteNonQuery();
            cmd.CommandText = "Select atm_id,atm_alert_id,generated_at,resolve_at,alert_type_id,expiration_time,generate_at_retry_remaining,resolve_at_retry_remaining,last_invoked_at,generate_notification_sent,resolve_notification_sent,failure_reason,alert_msg,task_id,entity_type,entity_id from Atm_alert ";
            if (where != null && where.Trim().Length > 0)
        ...
        public static AtmAlert LoadAtmAlertByPk(int atm_alert_id)
        {
            return LoadAtmAlert(" atm_alert_id=" + atm_alert_id);
        }

        public static AtmAlert LoadAtmAlertByPk(int atm_alert_id, IDbConnection conn)
        {
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (atm_idChanged || atm_alert_idChanged || generated_atChanged || resolve_atChanged || alert_type_idChanged || expiration_timeChanged || generate_at_retry_remainingChanged || resolve_at_retry_remainingChanged || last_invoked_atChanged || generate_notification_sentChanged || resolve_notification_sentChanged || failure_reasonChanged || alert_msgChanged || task_idChanged || entity_typeChanged || entity_idChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    qry.Append(@"insert into Atm_alert( atm_id,atm_alert_id,generated_at,resolve_at,alert_type_id,expiration_time,generate_at_retry_remaining,resolve_at_retry_remaining,last_invoked_at,generate_notification_sent,resolve_notification_sent,failure_reason,alert_msg,task_id,entity_type,entity_id ) values(");
                    qry.Append(atm_idDbString + ",");
                    lock (ConnectionFactory.connectionString)
                    {
                        this.atm_alert_id = ConnectionFactory.GetNextId();
        ...
        {
            ConnectionFactory.ExecuteQuery("delete Atm_alert where " + where);
        }
        ...
        public enum Columns : uint
        {
            ...
            alert_type_id = 16,
            expiration_time = 32,
            generate_at_retry_remaining = 64,
            resolve_at_retry_remaining = 128,
            last_invoked_at = 256,
            generate_notification_sent = 512,
            resolve_notification_sent = 1024,
            failure_reason = 2048,
            alert_msg = 4096,
            task_id = 8192,
            entity_type = 16384,
        ...
        #endregion
        public void BulkSave(List<AtmAlert> dataArray, SqlTransaction dbTrx)
        {
        ...
        public void AddToDataTable(List<AtmAlert> transList, ref DataTable dt)
        {
            foreach (AtmAlert tran in transList)
            {
                ...
                Row["atm_alert_id"] = ConnectionFactory.GetNextId();
