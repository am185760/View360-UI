...
namespace Avanza.CCMS.DAL
{
    ...
    public class EjParsedBnaTransaction
    {
        ...
        public EjParsedBnaTransaction(int ej_parsed_bna_transaction_id, int atm_id, DateTime generated_at, int start_index, int end_index, int task_id, bool is_eligible)
        {
            this.atm_id = atm_id;
        ...
        public EjParsedBnaTransaction(DateTime? trxn_datetime, string terminal_id, string seq, string account_type, string pan, string consumer_message_id, string dispute_status, decimal? amount_authorized, string status, string comment, string processed_tran, int atm_id, DateTime generated_at, int start_index, int end_index, int task_id, bool is_eligible, DateTime? transaction_start_time, string transaction_end_time, string card_taken_time, bool? is_disputed_transaction, string host_tsn, string account_no, DateTime? posting_date, string currency, int? transaction_type_id, string network, bool? is_cardless, string bank_name, int? customer_id, string eida_name)
        {
        ...
        private EjParsedBnaTransaction(int ej_parsed_bna_transaction_id, DateTime? trxn_datetime, string terminal_id, string seq, string account_type, string pan, string consumer_message_id, string dispute_status, decimal? amount_authorized, string status, string comment, string processed_tran, int atm_id, DateTime generated_at, int start_index, int end_index, int task_id, bool is_eligible, DateTime? transaction_start_time, string transaction_end_time, string card_taken_time, bool? is_disputed_transaction, string host_tsn, string account_no, DateTime? posting_date, string currency, int? transaction_type_id, string network, bool? is_cardless, string bank_name, int? customer_id, string eida_name)
        {
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.ej_parsed_bna_transaction_id) == Columns.ej_parsed_bna_transaction_id && reader["ej_parsed_bna_transaction_id"] != DBNull.Value)
                            currentEjParsedBnaTransaction.ej_parsed_bna_transaction_id = (int)reader["ej_parsed_bna_transaction_id"];
                        ...
                        if ((columns & Columns.atm_id) == Columns.atm_id && reader["atm_id"] != DBNull.Value)
                            currentEjParsedBnaTransaction.atm_id = (int)reader["atm_id"];
                        ...
                        if ((columns & Columns.task_id) == Columns.task_id && reader["task_id"] != DBNull.Value)
                            currentEjParsedBnaTransaction.task_id = (int)reader["task_id"];
                        ...
                        if ((columns & Columns.transaction_type_id) == Columns.transaction_type_id && reader["transaction_type_id"] != DBNull.Value)
                            currentEjParsedBnaTransaction.transaction_type_id = (int?)reader["transaction_type_id"];
                        ...
                        if ((columns & Columns.customer_id) == Columns.customer_id && reader["customer_id"] != DBNull.Value)
                            currentEjParsedBnaTransaction.customer_id = (int?)reader["customer_id"];
                        if ((columns & Columns.eida_name) == Columns.eida_name && reader["eida_name"] != DBNull.Value)
                            currentEjParsedBnaTransaction.eida_name = (string)reader["eida_name"];
        ...
        public static EjParsedBnaTransaction LoadEjParsedBnaTransactionByPk(int ej_parsed_bna_transaction_id)
        {
            return LoadEjParsedBnaTransaction("ej_parsed_bna_transaction_id=" + ej_parsed_bna_transaction_id);
        }

        public static EjParsedBnaTransaction LoadEjParsedBnaTransactionByPk(int ej_parsed_bna_transaction_id, IDbConnection conn)
        {
            return LoadEjParsedBnaTransaction(" ej_parsed_bna_transaction_id=" + ej_parsed_bna_transaction_id, conn);
        }

        public void Save()
        {
            if (ej_parsed_bna_transaction_idChanged || trxn_datetimeChanged || terminal_idChanged || seqChanged || account_typeChanged || panChanged || consumer_message_idChanged || dispute_statusChanged || amount_authorizedChanged || statusChanged || commentChanged || processed_tranChanged || atm_idChanged || generated_atChanged || start_indexChanged || end_indexChanged || task_idChanged || is_eligibleChanged || transaction_start_timeChanged || transaction_end_timeChanged || card_taken_timeChanged || is_disputed_transactionChanged || host_tsnChanged || account_noChanged || posting_dateChanged || currencyChanged || transaction_type_idChanged || networkChanged || is_cardlessChanged || bank_nameChanged || customer_idChanged || eida_nameChanged)
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (ej_parsed_bna_transaction_idChanged || trxn_datetimeChanged || terminal_idChanged || seqChanged || account_typeChanged || panChanged || consumer_message_idChanged || dispute_statusChanged || amount_authorizedChanged || statusChanged || commentChanged || processed_tranChanged || atm_idChanged || generated_atChanged || start_indexChanged || end_indexChanged || task_idChanged || is_eligibleChanged || transaction_start_timeChanged || transaction_end_timeChanged || card_taken_timeChanged || is_disputed_transactionChanged || host_tsnChanged || account_noChanged || posting_dateChanged || currencyChanged || transaction_type_idChanged || networkChanged || is_cardlessChanged || bank_nameChanged || customer_idChanged || eida_nameChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    ...
                    {
                        this.ej_parsed_bna_transaction_id = ConnectionFactory.GetNextId();
                ...
                else
                {
                    if (!(ej_parsed_bna_transaction_idChanged || trxn_datetimeChanged || terminal_idChanged || seqChanged || account_typeChanged || panChanged || consumer_message_idChanged || dispute_statusChanged || amount_authorizedChanged || statusChanged || commentChanged || processed_tranChanged || atm_idChanged || generated_atChanged || start_indexChanged || end_indexChanged || task_idChanged || is_eligibleChanged || transaction_start_timeChanged || transaction_end_timeChanged || card_taken_timeChanged || is_disputed_transactionChanged || host_tsnChanged || account_noChanged || posting_dateChanged || currencyChanged || transaction_type_idChanged || networkChanged || is_cardlessChanged || bank_nameChanged || customer_idChanged || eida_nameChanged))
                        return;
                    qry.Append("UPDATE Ej_parsed_bna_transaction set "); if (trxn_datetimeChanged)
                    {
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE Ej_parsed_bna_transaction where ej_parsed_bna_transaction_id= " + ej_parsed_bna_transaction_id;
        ...
        {
            ConnectionFactory.ExecuteQuery("delete Ej_parsed_bna_transaction where " + where);
        }
        ...
        public enum Columns : uint
        {
            ...
            bank_name = 536870912,
            customer_id = 1073741824,
            eida_name = 2147483648
        }
        #endregion
        ...
        public void AddToDataTable(List<EjParsedBnaTransaction> transList, ref DataTable dt)
        {
            foreach (EjParsedBnaTransaction tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
                Row["ej_parsed_bna_transaction_id"] = ConnectionFactory.GetNextId();
