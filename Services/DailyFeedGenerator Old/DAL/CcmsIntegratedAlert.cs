...
{
    [Serializable()]
    public class CcmsIntegratedAlert
    {
        ...
        public CcmsIntegratedAlert() { }
        public CcmsIntegratedAlert(long? id, bool generate_notification_sent, bool resolve_notification_sent)
        {
        ...
        public CcmsIntegratedAlert(int? alert_type_id, string alert_type, int? entity_id, string entity_type, string alert_level, string alert_status, DateTime? generated_at, string alert_text, DateTime? expiration_time, int? generate_retry_remaining, int? resolve_retry_remaining, DateTime? last_invoked_at, string module_type, int? ftp_file_info_id, string failure_reason, string alert_resolution_text, int? organization_id, DateTime? resolved_at, bool generate_notification_sent, bool resolve_notification_sent, int? atm_id, int? atm_alert_id, int? event_count, bool? trap_generate_notification_sent, int? trap_generate_retry_remaining, bool? trap_resolve_notification_sent, int? trap_resolve_retry_remaining, string trap_failure_reason)
        {
            ...
            this.trap_generate_notification_sent = trap_generate_notification_sent;
            this.trap_generate_notification_sentChanged = true;
            this.trap_generate_retry_remaining = trap_generate_retry_remaining;
            this.trap_generate_retry_remainingChanged = true;
            this.trap_resolve_notification_sent = trap_resolve_notification_sent;
            this.trap_resolve_notification_sentChanged = true;
            this.trap_resolve_retry_remaining = trap_resolve_retry_remaining;
            this.trap_resolve_retry_remainingChanged = true;
            this.trap_failure_reason = trap_failure_reason;
            this.trap_failure_reasonChanged = true;
        }
        private CcmsIntegratedAlert(long? id, int? alert_type_id, string alert_type, int? entity_id, string entity_type, string alert_level, string alert_status, DateTime? generated_at, string alert_text, DateTime? expiration_time, int? generate_retry_remaining, int? resolve_retry_remaining, DateTime? last_invoked_at, string module_type, int? ftp_file_info_id, string failure_reason, string alert_resolution_text, int? organization_id, DateTime? resolved_at, bool generate_notification_sent, bool resolve_notification_sent, int? atm_id, int? atm_alert_id, int? event_count, bool? trap_generate_notification_sent, int? trap_generate_retry_remaining, bool? trap_resolve_notification_sent, int? trap_resolve_retry_remaining, string trap_failure_reason)
        {
            this.id = id;
            this.idChanged = true;
            this.alert_type_id = alert_type_id;
        ...
        private string alert_typeDbString
        {
            get
            {
                if (this.alert_type != null)
                    return string.Format("'{0}'", alert_type);
                else
                    return "null";
            }
        }
        ...
        private string alert_levelDbString
        {
            get
            {
                if (this.alert_level != null)
                    return string.Format("'{0}'", alert_level);
                else
                    return "null";
            }
        }
        ...
        private string alert_statusDbString
        {
            get
            {
                if (this.alert_status != null)
                    return string.Format("'{0}'", alert_status);
                else
                    return "null";
            }
        }
        ...
        private string alert_textDbString
        {
            get
            {
                if (this.alert_text != null)
                    return string.Format("'{0}'", alert_text);
                else
                    return "null";
            }
        }
        ...
        private int? generate_retry_remaining;
        public int? GenerateRetryRemaining
        {
        ...
        private int? resolve_retry_remaining;
        public int? ResolveRetryRemaining
        {
        ...
        private string module_typeDbString
        {
            get
            {
                if (this.module_type != null)
                    return string.Format("'{0}'", module_type);
                else
                    return "null";
            }
        }
        ...
        private string alert_resolution_textDbString
        {
            get
            {
                if (this.alert_resolution_text != null)
                    return string.Format("'{0}'", alert_resolution_text);
                else
                    return "null";
            }
        }
        ...
        #endregion
        #region TrapGenerateNotificationSent
        private bool trap_generate_notification_sentChanged = false;
        private bool? trap_generate_notification_sent;
        public bool? TrapGenerateNotificationSent
        {
            get { return trap_generate_notification_sent; }
            set
            {
                trap_generate_notification_sent = value;
                trap_generate_notification_sentChanged = true;
            }
        }
        private string trap_generate_notification_sentDbString
        {
            get
            {
                if (this.trap_generate_notification_sent.HasValue)
                    return trap_generate_notification_sent.Value ? "1" : "0";
                else
                    return "null";
            }
        }
        #endregion
        #region TrapGenerateRetryRemaining
        private bool trap_generate_retry_remainingChanged = false;
        private int? trap_generate_retry_remaining;
        public int? TrapGenerateRetryRemaining
        {
            get { return trap_generate_retry_remaining; }
            set
            {
                trap_generate_retry_remaining = value;
                trap_generate_retry_remainingChanged = true;
            }
        }
        private string trap_generate_retry_remainingDbString
        {
            get
            {
                if (this.trap_generate_retry_remaining.HasValue)
                    return trap_generate_retry_remaining.ToString();
                else
                    return "null";
            }
        }
        #endregion
        #region TrapResolveNotificationSent
        private bool trap_resolve_notification_sentChanged = false;
        private bool? trap_resolve_notification_sent;
        public bool? TrapResolveNotificationSent
        {
            get { return trap_resolve_notification_sent; }
            set
            {
                trap_resolve_notification_sent = value;
                trap_resolve_notification_sentChanged = true;
            }
        }
        private string trap_resolve_notification_sentDbString
        {
            get
            {
                if (this.trap_resolve_notification_sent.HasValue)
                    return trap_resolve_notification_sent.Value ? "1" : "0";
                else
                    return "null";
            }
        }
        #endregion
        #region TrapResolveRetryRemaining
        private bool trap_resolve_retry_remainingChanged = false;
        private int? trap_resolve_retry_remaining;
        public int? TrapResolveRetryRemaining
        {
            get { return trap_resolve_retry_remaining; }
            set
            {
                trap_resolve_retry_remaining = value;
                trap_resolve_retry_remainingChanged = true;
            }
        }
        private string trap_resolve_retry_remainingDbString
        {
            get
            {
                if (this.trap_resolve_retry_remaining.HasValue)
                    return trap_resolve_retry_remaining.ToString();
                else
                    return "null";
            }
        }
        #endregion
        #region TrapFailureReason
        private bool trap_failure_reasonChanged = false;
        private string trap_failure_reason;
        public string TrapFailureReason
        {
            get { return trap_failure_reason; }
            set
            {
                trap_failure_reason = value;
                trap_failure_reasonChanged = true;
            }
        }
        private string trap_failure_reasonDbString
        {
            get
            {
                if (this.trap_failure_reason != null)
                    return string.Format("'{0}'", trap_failure_reason);
                else
                    return "null";
            }
        }
        ...
        #region CcmsIntegratedAlertReader
        public class CcmsIntegratedAlertReader : IEntityReader, IEnumerator, IEnumerable
        {
            IDataReader reader;
            IDbConnection conn;
            CcmsIntegratedAlert currentCcmsIntegratedAlert;
            Columns columns;
            bool partialRead = false;
            private CcmsIntegratedAlertReader() { }
            /// 
            ///
            ///
            ...
            public CcmsIntegratedAlertReader(IDataReader reader, IDbConnection conn)
            {
                this.reader = reader;
                this.conn = conn;
            }
            public CcmsIntegratedAlertReader(IDataReader reader, IDbConnection conn, Columns columns)
            {
                this.reader = reader;
                this.conn = conn;
            ...
            {
                get { return currentCcmsIntegratedAlert; }
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        ...
                        if ((columns & Columns.generate_notification_sent) == Columns.generate_notification_sent && reader["generate_notification_sent"] != DBNull.Value)
                            currentCcmsIntegratedAlert.generate_notification_sent = (bool)reader["generate_notification_sent"];
                        if ((columns & Columns.resolve_notification_sent) == Columns.resolve_notification_sent && reader["resolve_notification_sent"] != DBNull.Value)
                            currentCcmsIntegratedAlert.resolve_notification_sent = (bool)reader["resolve_notification_sent"];
                        if ((columns & Columns.atm_id) == Columns.atm_id && reader["atm_id"] != DBNull.Value)
                            currentCcmsIntegratedAlert.atm_id = (int?)reader["atm_id"];
                        if ((columns & Columns.atm_alert_id) == Columns.atm_alert_id && reader["atm_alert_id"] != DBNull.Value)
                            currentCcmsIntegratedAlert.atm_alert_id = (int?)reader["atm_alert_id"];
                        if ((columns & Columns.event_count) == Columns.event_count && reader["event_count"] != DBNull.Value)
                            currentCcmsIntegratedAlert.event_count = (int?)reader["event_count"];
                        if ((columns & Columns.trap_generate_notification_sent) == Columns.trap_generate_notification_sent && reader["trap_generate_notification_sent"] != DBNull.Value)
                            currentCcmsIntegratedAlert.trap_generate_notification_sent = (bool?)reader["trap_generate_notification_sent"];
                        if ((columns & Columns.trap_generate_retry_remaining) == Columns.trap_generate_retry_remaining && reader["trap_generate_retry_remaining"] != DBNull.Value)
                            currentCcmsIntegratedAlert.trap_generate_retry_remaining = (int?)reader["trap_generate_retry_remaining"];
                        if ((columns & Columns.trap_resolve_notification_sent) == Columns.trap_resolve_notification_sent && reader["trap_resolve_notification_sent"] != DBNull.Value)
                            currentCcmsIntegratedAlert.trap_resolve_notification_sent = (bool?)reader["trap_resolve_notification_sent"];
                        if ((columns & Columns.trap_resolve_retry_remaining) == Columns.trap_resolve_retry_remaining && reader["trap_resolve_retry_remaining"] != DBNull.Value)
                            currentCcmsIntegratedAlert.trap_resolve_retry_remaining = (int?)reader["trap_resolve_retry_remaining"];
                        if ((columns & Columns.trap_failure_reason) == Columns.trap_failure_reason && reader["trap_failure_reason"] != DBNull.Value)
                            currentCcmsIntegratedAlert.trap_failure_reason = (string)reader["trap_failure_reason"];
                    ...
                    else
                    {
                        ...
                        if (reader["alert_type_id"] != DBNull.Value)
                            currentCcmsIntegratedAlert.alert_type_id = int.Parse(reader["alert_type_id"].ToString());
                        if (reader["alert_type"] != DBNull.Value)
                            currentCcmsIntegratedAlert.alert_type = (string)reader["alert_type"];
                        if (reader["entity_id"] != DBNull.Value)
                            currentCcmsIntegratedAlert.entity_id = int.Parse(reader["entity_id"].ToString());
                        ...
                        if (reader["organization_id"] != DBNull.Value)
                            currentCcmsIntegratedAlert.organization_id = int.Parse(reader["organization_id"].ToString());
                        ...
                        if (reader["atm_alert_id"] != DBNull.Value)
                            currentCcmsIntegratedAlert.atm_alert_id = int.Parse(reader["atm_alert_id"].ToString());
                        if (reader["event_count"] != DBNull.Value)
                            currentCcmsIntegratedAlert.event_count = int.Parse(reader["event_count"].ToString());
                        if (reader["trap_generate_notification_sent"] != DBNull.Value)
                            currentCcmsIntegratedAlert.trap_generate_notification_sent = (bool?)reader["trap_generate_notification_sent"];
                        if (reader["trap_generate_retry_remaining"] != DBNull.Value)
                            currentCcmsIntegratedAlert.trap_generate_retry_remaining = (int?)reader["trap_generate_retry_remaining"];
                        if (reader["trap_resolve_notification_sent"] != DBNull.Value)
                            currentCcmsIntegratedAlert.trap_resolve_notification_sent = (bool?)reader["trap_resolve_notification_sent"];
                        if (reader["trap_resolve_retry_remaining"] != DBNull.Value)
                            currentCcmsIntegratedAlert.trap_resolve_retry_remaining = (int?)reader["trap_resolve_retry_remaining"];
                        if (reader["trap_failure_reason"] != DBNull.Value)
                            currentCcmsIntegratedAlert.trap_failure_reason = (string)reader["trap_failure_reason"];
                    }

                    currentCcmsIntegratedAlert.isNewEntity = false;
                    return true;
                }
                else
                    return false;
            ...
            public CcmsIntegratedAlert CurrentCcmsIntegratedAlert
            {
                get { return currentCcmsIntegratedAlert; }
            }
        ...
        #region CcmsIntegratedAlert functions

        public static CcmsIntegratedAlertReader ExecuteReader(string where, IDbConnection conn, Columns columns)
        {
            ...
            if (Columns.trap_generate_notification_sent == (Columns.trap_generate_notification_sent & columns))
                qry.Append("trap_generate_notification_sent,");
            if (Columns.trap_generate_retry_remaining == (Columns.trap_generate_retry_remaining & columns))
                qry.Append("trap_generate_retry_remaining,");
            if (Columns.trap_resolve_notification_sent == (Columns.trap_resolve_notification_sent & columns))
                qry.Append("trap_resolve_notification_sent,");
            if (Columns.trap_resolve_retry_remaining == (Columns.trap_resolve_retry_remaining & columns))
                qry.Append("trap_resolve_retry_remaining,");
            if (Columns.trap_failure_reason == (Columns.trap_failure_reason & columns))
                qry.Append("trap_failure_reason,");
            qry.Replace(',', ' ', qry.Length - 1, 1);
            qry.Append("from Ccms_integrated_alert ");
            ...
            return new CcmsIntegratedAlertReader(cmd.ExecuteReader(), conn, columns);
        }

        static public CcmsIntegratedAlertReader ExecuteReader(string where, Columns columns)
        {
        ...
        /// 
        /// 
        public static CcmsIntegratedAlertReader ExecuteReader(string where, IDbConnection conn)
        {
            if (conn.State != ConnectionState.Open)
                conn.Open();
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
            cmd.ExecuteNonQuery();
            cmd.CommandText = "Select id,alert_type_id,alert_type,entity_id,entity_type,alert_level,alert_status,generated_at,alert_text,expiration_time,generate_retry_remaining,resolve_retry_remaining,last_invoked_at,module_type,ftp_file_info_id,failure_reason,alert_resolution_text,organization_id,resolved_at,generate_notification_sent,resolve_notification_sent,atm_id,atm_alert_id,event_count,trap_generate_notification_sent,trap_generate_retry_remaining,trap_resolve_notification_sent,trap_resolve_retry_remaining,trap_failure_reason from Ccms_integrated_alert ";
            if (where != null && where.Trim().Length > 0)
                cmd.CommandText = string.Format("{0} where {1}", cmd.CommandText, where);

            return new CcmsIntegratedAlertReader(cmd.ExecuteReader(), conn);
        }

        static public CcmsIntegratedAlertReader ExecuteReader(string where)
        {
            return ExecuteReader(where, ConnectionFactory.GetNewConnection());
        }

        public static CcmsIntegratedAlert LoadCcmsIntegratedAlert(string where)
        {
            CcmsIntegratedAlertReader reader = CcmsIntegratedAlert.ExecuteReader(where);
            CcmsIntegratedAlert _ccmsintegratedalert = null;
            if (reader.Read())
                _ccmsintegratedalert = reader.CurrentCcmsIntegratedAlert;
            reader.Close();
            return _ccmsintegratedalert;
        }

        public static CcmsIntegratedAlert LoadCcmsIntegratedAlert(string where, IDbConnection conn)
        {
            CcmsIntegratedAlertReader reader = CcmsIntegratedAlert.ExecuteReader(where, conn);
            CcmsIntegratedAlert _ccmsintegratedalert = null;
            if (reader.Read())
                _ccmsintegratedalert = reader.CurrentCcmsIntegratedAlert;
            reader.Close(false);
        ...
        public static CcmsIntegratedAlert LoadCcmsIntegratedAlertByPk(long id)
        {
            return LoadCcmsIntegratedAlert(" id=" + id);
        }

        public static CcmsIntegratedAlert LoadCcmsIntegratedAlertByPk(long id, IDbConnection conn)
        {
            return LoadCcmsIntegratedAlert(" id=" + id, conn);
        }

        public void Save()
        {
            if (idChanged || alert_type_idChanged || alert_typeChanged || entity_idChanged || entity_typeChanged || alert_levelChanged || alert_statusChanged || generated_atChanged || alert_textChanged || expiration_timeChanged || generate_retry_remainingChanged || resolve_retry_remainingChanged || last_invoked_atChanged || module_typeChanged || ftp_file_info_idChanged || failure_reasonChanged || alert_resolution_textChanged || organization_idChanged || resolved_atChanged || generate_notification_sentChanged || resolve_notification_sentChanged || atm_idChanged || atm_alert_idChanged || event_countChanged || trap_generate_notification_sentChanged || trap_generate_retry_remainingChanged || trap_resolve_notification_sentChanged || trap_resolve_retry_remainingChanged || trap_failure_reasonChanged)
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (idChanged || alert_type_idChanged || alert_typeChanged || entity_idChanged || entity_typeChanged || alert_levelChanged || alert_statusChanged || generated_atChanged || alert_textChanged || expiration_timeChanged || generate_retry_remainingChanged || resolve_retry_remainingChanged || last_invoked_atChanged || module_typeChanged || ftp_file_info_idChanged || failure_reasonChanged || alert_resolution_textChanged || organization_idChanged || resolved_atChanged || generate_notification_sentChanged || resolve_notification_sentChanged || atm_idChanged || atm_alert_idChanged || event_countChanged || trap_generate_notification_sentChanged || trap_generate_retry_remainingChanged || trap_resolve_notification_sentChanged || trap_resolve_retry_remainingChanged || trap_failure_reasonChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    qry.Append(@"insert into Ccms_integrated_alert( alert_type_id,alert_type,entity_id,entity_type,alert_level,alert_status,generated_at,alert_text,expiration_time,generate_retry_remaining,resolve_retry_remaining,last_invoked_at,module_type,ftp_file_info_id,failure_reason,alert_resolution_text,organization_id,resolved_at,generate_notification_sent,resolve_notification_sent,atm_id,atm_alert_id,event_count,trap_generate_notification_sent,trap_generate_retry_remaining,trap_resolve_notification_sent,trap_resolve_retry_remaining,trap_failure_reason ) values(");
                    ...
                    qry.Append(trap_generate_notification_sentDbString + ",");
                    qry.Append(trap_generate_retry_remainingDbString + ",");
                    qry.Append(trap_resolve_notification_sentDbString + ",");
                    qry.Append(trap_resolve_retry_remainingDbString + ",");
                    qry.Append(trap_failure_reasonDbString);
                ...
                else
                {
                    if (!(idChanged || alert_type_idChanged || alert_typeChanged || entity_idChanged || entity_typeChanged || alert_levelChanged || alert_statusChanged || generated_atChanged || alert_textChanged || expiration_timeChanged || generate_retry_remainingChanged || resolve_retry_remainingChanged || last_invoked_atChanged || module_typeChanged || ftp_file_info_idChanged || failure_reasonChanged || alert_resolution_textChanged || organization_idChanged || resolved_atChanged || generate_notification_sentChanged || resolve_notification_sentChanged || atm_idChanged || atm_alert_idChanged || event_countChanged || trap_generate_notification_sentChanged || trap_generate_retry_remainingChanged || trap_resolve_notification_sentChanged || trap_resolve_retry_remainingChanged || trap_failure_reasonChanged))
                        return;
                    qry.Append("UPDATE Ccms_integrated_alert set "); if (alert_type_idChanged)
                    {
                    ...
                    if (trap_generate_notification_sentChanged)
                    {
                        qry.Append("trap_generate_notification_sent =" + trap_generate_notification_sentDbString);
                        qry.Append(",");
                    }

                    if (trap_generate_retry_remainingChanged)
                    {
                        qry.Append("trap_generate_retry_remaining =" + trap_generate_retry_remainingDbString);
                        qry.Append(",");
                    }

                    if (trap_resolve_notification_sentChanged)
                    {
                        qry.Append("trap_resolve_notification_sent =" + trap_resolve_notification_sentDbString);
                        qry.Append(",");
                    }

                    if (trap_resolve_retry_remainingChanged)
                    {
                        qry.Append("trap_resolve_retry_remaining =" + trap_resolve_retry_remainingDbString);
                        qry.Append(",");
                    }

                    if (trap_failure_reasonChanged)
                    {
                        qry.Append("trap_failure_reason =" + trap_failure_reasonDbString);
                        qry.Append(",");
                    }
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE Ccms_integrated_alert where id = " + id;
        ...
        public void Delete(IDbConnection conn, SqlTransaction trxn)
        {
            IDbCommand cmd = conn.CreateCommand();
        ...
        public static void DeleteCcmsIntegratedAlerts(string where)
        {
            ConnectionFactory.ExecuteQuery("delete Ccms_integrated_alert where " + where);
        }
        ...
        public enum Columns : uint
        {
            ...
            event_count = 8388608,
            trap_generate_notification_sent = 16777216,
            trap_generate_retry_remaining = 33554432,
            trap_resolve_notification_sent = 67108864,
            trap_resolve_retry_remaining = 134217728,
            trap_failure_reason = 268435456
        }
        #endregion
        public void BulkSave(List<CcmsIntegratedAlert> dataArray, SqlTransaction dbTrx)
        {
            DataTable dt = new DataTable();
            CreateDataTable(dt);
            AddToDataTable(dataArray, ref dt);
            SqlBulkCopy bulk = new SqlBulkCopy(dbTrx.Connection, SqlBulkCopyOptions.Default, dbTrx);
            bulk.DestinationTableName = "Ccms_integrated_alert";
            bulk.WriteToServer(dt);
        }
        public void CreateDataTable(DataTable dt)
        {
            string[] colNames = Enum.GetNames(typeof(CcmsIntegratedAlert.Columns));
            for (int i = 0; i < colNames.Length; i++)
            {
                dt.Columns.Add(colNames[i]);
            }
        }
        public void AddToDataTable(List<CcmsIntegratedAlert> transList, ref DataTable dt)
        {
            foreach (CcmsIntegratedAlert tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
                ...
                Row["trap_generate_notification_sent"] = tran.TrapGenerateNotificationSent;
                Row["trap_generate_retry_remaining"] = tran.TrapGenerateRetryRemaining;
                Row["trap_resolve_notification_sent"] = tran.TrapResolveNotificationSent;
                Row["trap_resolve_retry_remaining"] = tran.TrapResolveRetryRemaining;
                Row["trap_failure_reason"] = tran.TrapFailureReason;
                dt.Rows.Add(Row);
            }
        }
    }
