...
namespace Avanza.CCMS.DAL
{
    ...
    public class DepositPosition
    {
        ...
        public DepositPosition(int atm_id, int deposit_position_id)
        {
            this.atm_id = atm_id;
            this.atm_idChanged = true;
        }
        public DepositPosition(int atm_id, int? cassette1_deposit, int? cassette2_deposit, int? cassette3_deposit, int? cassette4_deposit, int? purge_deposit, int? bin1, int? bin2, int? bin3, int? bin4, DateTime? last_cpm_deposit_at, DateTime? last_bna_deposit_at, string cassette1_deposit_value, string cassette2_deposit_value, string cassette3_deposit_value, string cassette4_deposit_value, string purge_deposit_value)
        {
            this.atm_id = atm_id;
            this.atm_idChanged = true;
        ...
        private DepositPosition(int atm_id, int deposit_position_id, int? cassette1_deposit, int? cassette2_deposit, int? cassette3_deposit, int? cassette4_deposit, int? purge_deposit, int? bin1, int? bin2, int? bin3, int? bin4, DateTime? last_cpm_deposit_at, DateTime? last_bna_deposit_at, string cassette1_deposit_value, string cassette2_deposit_value, string cassette3_deposit_value, string cassette4_deposit_value, string purge_deposit_value)
        {
            this.atm_id = atm_id;
            this.atm_idChanged = true;
        ...
        private int deposit_position_id;
        public int DepositPositionId
        {
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.atm_id) == Columns.atm_id && reader["atm_id"] != DBNull.Value)
                            currentDepositPosition.atm_id = (int)reader["atm_id"];
                        if ((columns & Columns.deposit_position_id) == Columns.deposit_position_id && reader["deposit_position_id"] != DBNull.Value)
                            currentDepositPosition.deposit_position_id = (int)reader["deposit_position_id"];
        ...
        public static DepositPosition LoadDepositPositionByPk(int deposit_position_id)
        {
            return LoadDepositPosition("deposit_position_id=" + deposit_position_id);
        }

        public static DepositPosition LoadDepositPositionByPk(int deposit_position_id, IDbConnection conn)
        {
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (atm_idChanged || deposit_position_idChanged || cassette1_depositChanged || cassette2_depositChanged || cassette3_depositChanged || cassette4_depositChanged || purge_depositChanged || bin1Changed || bin2Changed || bin3Changed || bin4Changed || last_cpm_deposit_atChanged || last_bna_deposit_atChanged || cassette1_deposit_valueChanged || cassette2_deposit_valueChanged || cassette3_deposit_valueChanged || cassette4_deposit_valueChanged || purge_deposit_valueChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    ...
                    {
                        this.deposit_position_id = ConnectionFactory.GetNextId();
        ...
        {
            ConnectionFactory.ExecuteQuery("delete Deposit_position where " + where);
        }
        ...
        {
            atm_id = 1,
            deposit_position_id = 2,
            cassette1_deposit = 4,
            cassette2_deposit = 8,
            cassette3_deposit = 16,
            cassette4_deposit = 32,
            purge_deposit = 64,
            bin1 = 128,
            bin2 = 256,
            bin3 = 512,
            bin4 = 1024,
            last_cpm_deposit_at = 2048,
            last_bna_deposit_at = 4096,
            cassette1_deposit_value = 8192,
            cassette2_deposit_value = 16384,
            cassette3_deposit_value = 32768,
            cassette4_deposit_value = 65536,
            purge_deposit_value = 131072
        }
        #endregion
        ...
        public void AddToDataTable(List<DepositPosition> transList, ref DataTable dt)
        {
            foreach (DepositPosition tran in transList)
            {
                ...
                Row["deposit_position_id"] = ConnectionFactory.GetNextId();
