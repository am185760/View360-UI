...
namespace Avanza.CCMS.DAL
{
    ...
    public class EjParsedReplenishments
    {
        ...
        public EjParsedReplenishments(int ej_parsed_replenishments_id)
        {
        }
        public EjParsedReplenishments(int? atm_id, int? notes_added_type1, int? notes_added_type2, int? notes_added_type3, int? notes_added_type4, DateTime? rep_datetime, int? task_id, DateTime? processing_datetime, int? start_index, int? end_index, int? last_tsn, int? notes_Added_type5, int? notes_Added_type6, int? notes_Added_type7)
        {
            this.atm_id = atm_id;
        ...
        private EjParsedReplenishments(int ej_parsed_replenishments_id, int? atm_id, int? notes_added_type1, int? notes_added_type2, int? notes_added_type3, int? notes_added_type4, DateTime? rep_datetime, int? task_id, DateTime? processing_datetime, int? start_index, int? end_index, int? last_tsn, int? notes_Added_type5, int? notes_Added_type6, int? notes_Added_type7)
        {
        ...
        private int ej_parsed_replenishments_id;
        public int EjParsedReplenishmentsId
        {
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.ej_parsed_replenishments_id) == Columns.ej_parsed_replenishments_id && reader["ej_parsed_replenishments_id"] != DBNull.Value)
                            currentEjParsedReplenishments.ej_parsed_replenishments_id = (int)reader["ej_parsed_replenishments_id"];
                        if ((columns & Columns.atm_id) == Columns.atm_id && reader["atm_id"] != DBNull.Value)
                            currentEjParsedReplenishments.atm_id = (int?)reader["atm_id"];
                        ...
                        if ((columns & Columns.task_id) == Columns.task_id && reader["task_id"] != DBNull.Value)
                            currentEjParsedReplenishments.task_id = (int?)reader["task_id"];
        ...
        public static EjParsedReplenishmentsReader ExecuteReader(string where, IDbConnection conn)
        {
            ...
            cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
            cmd.ExecuteNonQuery();
            cmd.CommandText = "Select ej_parsed_replenishments_id,atm_id,notes_added_type1,notes_added_type2,notes_added_type3,notes_added_type4,rep_datetime,task_id,processing_datetime,start_index,end_index,last_tsn,notes_Added_type5,notes_Added_type6,notes_Added_type7 from Ej_parsed_replenishments ";
            if (where != null && where.Trim().Length > 0)
        ...
        public static EjParsedReplenishments LoadEjParsedReplenishmentsByPk(int ej_parsed_replenishments_id)
        {
            return LoadEjParsedReplenishments(" ej_parsed_replenishments_id=" + ej_parsed_replenishments_id);
        }

        public static EjParsedReplenishments LoadEjParsedReplenishmentsByPk(int ej_parsed_replenishments_id, IDbConnection conn)
        {
            return LoadEjParsedReplenishments(" ej_parsed_replenishments_id=" + ej_parsed_replenishments_id, conn);
        }
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (ej_parsed_replenishments_idChanged || atm_idChanged || notes_added_type1Changed || notes_added_type2Changed || notes_added_type3Changed || notes_added_type4Changed || rep_datetimeChanged || task_idChanged || processing_datetimeChanged || start_indexChanged || end_indexChanged || last_tsnChanged || notes_Added_type5Changed || notes_Added_type6Changed || notes_Added_type7Changed)
            {
                ...
                if (this.isNewEntity)
                {
                    ...
                    {
                        this.ej_parsed_replenishments_id = ConnectionFactory.GetNextId();
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE Ej_parsed_replenishments where ej_parsed_replenishments_id = " + ej_parsed_replenishments_id;
        ...
        {
            ConnectionFactory.ExecuteQuery("delete Ej_parsed_replenishments where " + where);
        }
        ...
        {
            ej_parsed_replenishments_id = 1,
            atm_id = 2,
            notes_added_type1 = 4,
            notes_added_type2 = 8,
            notes_added_type3 = 16,
            notes_added_type4 = 32,
            rep_datetime = 64,
            task_id = 128,
            processing_datetime = 256,
            start_index = 512,
            end_index = 1024,
            last_tsn = 2048,
            notes_Added_type5 = 4096,
            notes_Added_type6 = 8192,
            notes_Added_type7 = 16384
        }
        #endregion
        public void BulkSave(List<EjParsedReplenishments> dataArray, SqlTransaction dbTrx)
        {
        ...
        public void AddToDataTable(List<EjParsedReplenishments> transList, ref DataTable dt)
        {
            foreach (EjParsedReplenishments tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
                Row["ej_parsed_replenishments_id"] = ConnectionFactory.GetNextId();
