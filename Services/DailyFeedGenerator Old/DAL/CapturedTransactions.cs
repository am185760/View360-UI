...
namespace Avanza.CCMS.DAL
{
    ...
    public class CapturedTransactions
    {
        ...
        public CapturedTransactions(int captured_transactions_id, int transaction_rule_id, int transaction_id, DateTime captured_at)
        {
        ...
        public CapturedTransactions(int transaction_rule_id, int transaction_id, DateTime captured_at, DateTime? expiration_time, int? user_id, string comments, int? ej_captured_card_id, int? ej_parsed_transactions_id, int? ej_parsed_bna_transactions_id, int? ej_parsed_cpm_transactions_id, int? task_id, string trxn_status, decimal? amount_claimed, bool? is_locked, decimal? amount_credited, string internal_team_comment, DateTime? locked_datetime, string modified_by, decimal? amount)
        {
            this.transaction_rule_id = transaction_rule_id;
            this.transaction_rule_idChanged = true;
            this.transaction_id = transaction_id;
        ...
        private CapturedTransactions(int captured_transactions_id, int transaction_rule_id, int transaction_id, DateTime captured_at, DateTime? expiration_time, int? user_id, string comments, int? ej_captured_card_id, int? ej_parsed_transactions_id, int? ej_parsed_bna_transactions_id, int? ej_parsed_cpm_transactions_id, int? task_id, string trxn_status, decimal? amount_claimed, bool? is_locked, decimal? amount_credited, string internal_team_comment, DateTime? locked_datetime, string modified_by, decimal? amount)
        {
        ...
        private int? ej_parsed_bna_transactions_id;
        public int? EjParsedBnaTransactionsId
        {
        ...
        private int? ej_parsed_cpm_transactions_id;
        public int? EjParsedCpmTransactionsId
        {
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.captured_transactions_id) == Columns.captured_transactions_id && reader["captured_transactions_id"] != DBNull.Value)
                            currentCapturedTransactions.captured_transactions_id = (int)reader["captured_transactions_id"];
                        if ((columns & Columns.transaction_rule_id) == Columns.transaction_rule_id && reader["transaction_rule_id"] != DBNull.Value)
                            currentCapturedTransactions.transaction_rule_id = (int)reader["transaction_rule_id"];
                        if ((columns & Columns.transaction_id) == Columns.transaction_id && reader["transaction_id"] != DBNull.Value)
                            currentCapturedTransactions.transaction_id = (int)reader["transaction_id"];
                        ...
                        if ((columns & Columns.user_id) == Columns.user_id && reader["user_id"] != DBNull.Value)
                            currentCapturedTransactions.user_id = (int?)reader["user_id"];
                        ...
                        if ((columns & Columns.ej_captured_card_id) == Columns.ej_captured_card_id && reader["ej_captured_card_id"] != DBNull.Value)
                            currentCapturedTransactions.ej_captured_card_id = (int?)reader["ej_captured_card_id"];
                        if ((columns & Columns.ej_parsed_transactions_id) == Columns.ej_parsed_transactions_id && reader["ej_parsed_transactions_id"] != DBNull.Value)
                            currentCapturedTransactions.ej_parsed_transactions_id = (int?)reader["ej_parsed_transactions_id"];
                        if ((columns & Columns.ej_parsed_bna_transactions_id) == Columns.ej_parsed_bna_transactions_id && reader["ej_parsed_bna_transactions_id"] != DBNull.Value)
                            currentCapturedTransactions.ej_parsed_bna_transactions_id = (int?)reader["ej_parsed_bna_transactions_id"];
                        if ((columns & Columns.ej_parsed_cpm_transactions_id) == Columns.ej_parsed_cpm_transactions_id && reader["ej_parsed_cpm_transactions_id"] != DBNull.Value)
                            currentCapturedTransactions.ej_parsed_cpm_transactions_id = (int?)reader["ej_parsed_cpm_transactions_id"];
                        if ((columns & Columns.task_id) == Columns.task_id && reader["task_id"] != DBNull.Value)
                            currentCapturedTransactions.task_id = (int?)reader["task_id"];
        ...
        public static CapturedTransactionsReader ExecuteReader(string where, IDbConnection conn)
        {
            ...
            cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
            cmd.ExecuteNonQuery();
            cmd.CommandText = "Select captured_transactions_id,transaction_rule_id,transaction_id,captured_at,expiration_time,user_id,comments,ej_captured_card_id,ej_parsed_transactions_id,ej_parsed_bna_transactions_id,ej_parsed_cpm_transactions_id,task_id,trxn_status,amount_claimed,is_locked,amount_credited,internal_team_comment,locked_datetime,modified_by,amount from Captured_transactions ";
            if (where != null && where.Trim().Length > 0)
        ...
        public static CapturedTransactions LoadCapturedTransactionsByPk(int captured_transactions_id)
        {
            return LoadCapturedTransactions("captured_transactions_id=" + captured_transactions_id);
        }

        public static CapturedTransactions LoadCapturedTransactionsByPk(int captured_transactions_id, IDbConnection conn)
        {
            return LoadCapturedTransactions(" captured_transactions_id=" + captured_transactions_id, conn);
        }

        public void Save()
        {
            if (captured_transactions_idChanged || transaction_rule_idChanged || transaction_idChanged || captured_atChanged || expiration_timeChanged || user_idChanged || commentsChanged || ej_captured_card_idChanged || ej_parsed_transactions_idChanged || ej_parsed_bna_transactions_idChanged || ej_parsed_cpm_transactions_idChanged || task_idChanged || trxn_statusChanged || amount_claimedChanged || is_lockedChanged || amount_creditedChanged || internal_team_commentChanged || locked_datetimeChanged || modified_byChanged || amountChanged)
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (captured_transactions_idChanged || transaction_rule_idChanged || transaction_idChanged || captured_atChanged || expiration_timeChanged || user_idChanged || commentsChanged || ej_captured_card_idChanged || ej_parsed_transactions_idChanged || ej_parsed_bna_transactions_idChanged || ej_parsed_cpm_transactions_idChanged || task_idChanged || trxn_statusChanged || amount_claimedChanged || is_lockedChanged || amount_creditedChanged || internal_team_commentChanged || locked_datetimeChanged || modified_byChanged || amountChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    qry.Append(@"insert into Captured_transactions(captured_transactions_id,transaction_rule_id,transaction_id,captured_at,expiration_time,user_id,comments,ej_captured_card_id,ej_parsed_transactions_id,ej_parsed_bna_transactions_id,ej_parsed_cpm_transactions_id,task_id,trxn_status,amount_claimed,is_locked,amount_credited,internal_team_comment,locked_datetime,modified_by,amount) values(");
                    lock (ConnectionFactory.connectionString)
                    {
                        this.captured_transactions_id = ConnectionFactory.GetNextId();
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE Captured_transactions where captured_transactions_id= " + captured_transactions_id;
        ...
        {
            ConnectionFactory.ExecuteQuery("delete Captured_transactions where " + where);
        }
        ...
        public enum Columns : uint
        {
            ...
            ej_parsed_cpm_transactions_id = 10,
            task_id = 11,
            trxn_status = 12,
            amount_claimed = 13,
            is_locked = 14,
        ...
        public void AddToDataTable(List<CapturedTransactions> transList, ref DataTable dt)
        {
            foreach (CapturedTransactions tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
                Row["captured_transactions_id"] = ConnectionFactory.GetNextId();
