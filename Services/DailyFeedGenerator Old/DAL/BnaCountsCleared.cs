...
namespace Avanza.CCMS.DAL
{
    ...
    public class BnaCountsCleared
    {
        ...
        public BnaCountsCleared(int bna_counts_cleared_id, int atm_id, DateTime counts_cleared_at, DateTime recorded_at)
        {
            this.atm_id = atm_id;
        ...
        public BnaCountsCleared(int atm_id, DateTime counts_cleared_at, DateTime recorded_at, int? task_id)
        {
            this.atm_id = atm_id;
            this.atm_idChanged = true;
            this.counts_cleared_at = counts_cleared_at;
        ...
        private BnaCountsCleared(int bna_counts_cleared_id, int atm_id, DateTime counts_cleared_at, DateTime recorded_at, int? task_id)
        {
        ...
        private int bna_counts_cleared_id;
        public int BnaCountsClearedId
        {
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.bna_counts_cleared_id) == Columns.bna_counts_cleared_id && reader["bna_counts_cleared_id"] != DBNull.Value)
                            currentBnaCountsCleared.bna_counts_cleared_id = (int)reader["bna_counts_cleared_id"];
                        if ((columns & Columns.atm_id) == Columns.atm_id && reader["atm_id"] != DBNull.Value)
                            currentBnaCountsCleared.atm_id = (int)reader["atm_id"];
                        ...
                        if ((columns & Columns.task_id) == Columns.task_id && reader["task_id"] != DBNull.Value)
                            currentBnaCountsCleared.task_id = (int?)reader["task_id"];
        ...
        public static BnaCountsCleared LoadBnaCountsClearedByPk(int bna_counts_cleared_id)
        {
            return LoadBnaCountsCleared(" bna_counts_cleared_id=" + bna_counts_cleared_id);
        }

        public static BnaCountsCleared LoadBnaCountsClearedByPk(int bna_counts_cleared_id, IDbConnection conn)
        {
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (bna_counts_cleared_idChanged || atm_idChanged || counts_cleared_atChanged || recorded_atChanged || task_idChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    ...
                    {
                        this.bna_counts_cleared_id = ConnectionFactory.GetNextId();
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE Bna_counts_cleared where bna_counts_cleared_id = " + bna_counts_cleared_id;
        ...
        {
            ConnectionFactory.ExecuteQuery("delete Bna_counts_cleared where " + where);
        }
        ...
        #endregion
        public DataTable BulkSave(List<BnaCountsCleared> dataArray, SqlTransaction dbTrx)
        {
        ...
        public void AddToDataTable(List<BnaCountsCleared> transList, ref DataTable dt)
        {
            foreach (BnaCountsCleared tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
                Row["bna_counts_cleared_id"] = ConnectionFactory.GetNextId();
