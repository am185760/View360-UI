...
{
[Serializable()]
public class ReconciliationHostData
{
...
public ReconciliationHostData() { }
public ReconciliationHostData( int reconciliation_host_data_id,int reconciliation_batch_id,string atm_id,string card_number,string transaction_date,string transaction_time ) 
{
...
private ReconciliationHostData( int reconciliation_host_data_id,int reconciliation_batch_id,string atm_id,string card_number,string customer_account_no,string transaction_date,string transaction_time,decimal? transaction_amount,string transaction_currency,string transaction_sequence,string transaction_type,string transaction_response,DateTime? transaction_settlement_date,string card_network,string card_issuer )
{
...
#region ReconciliationHostDataReader
public class ReconciliationHostDataReader:IEntityReader, IEnumerator, IEnumerable 
{
IDataReader reader;
IDbConnection conn;
ReconciliationHostData currentReconciliationHostData;
Columns columns;
bool partialRead = false;
private ReconciliationHostDataReader() { }
/// 
///
///
...
public ReconciliationHostDataReader(IDataReader reader,IDbConnection conn)
{
this.reader = reader;
this.conn = conn;
}
public ReconciliationHostDataReader(IDataReader reader, IDbConnection conn, Columns columns)
{
this.reader = reader;
this.conn = conn;
...
{
get { return currentReconciliationHostData; }
...
if (reader.Read())
{
currentReconciliationHostData = new ReconciliationHostData();
if (partialRead)
...
currentReconciliationHostData.transaction_settlement_date =(DateTime?) reader["transaction_settlement_date"]; 
if ((columns & Columns.card_network) == Columns.card_network && reader["card_network"]!=DBNull.Value)
currentReconciliationHostData.card_network =(string) reader["card_network"]; 
if ((columns & Columns.card_issuer) == Columns.card_issuer && reader["card_issuer"]!=DBNull.Value)
currentReconciliationHostData.card_issuer =(string) reader["card_issuer"]; 
...
currentReconciliationHostData.isNewEntity = false;
return true;
}
else
return false;
...
public ReconciliationHostData CurrentReconciliationHostData
{
get{ return currentReconciliationHostData; }
}
...
#region ReconciliationHostData functions

public static ReconciliationHostDataReader ExecuteReader(string where, IDbConnection conn, Columns columns)
{
...
qry.Append("from Reconciliation_host_data ");
...
return new ReconciliationHostDataReader(cmd.ExecuteReader(), conn, columns);
}

static public ReconciliationHostDataReader ExecuteReader(string where,Columns columns)
{
...
/// 
/// 
public static ReconciliationHostDataReader ExecuteReader(string where,IDbConnection conn)
{
if (conn.State != ConnectionState.Open)
conn.Open();
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
cmd.ExecuteNonQuery();
cmd.CommandText = "Select reconciliation_host_data_id,reconciliation_batch_id,atm_id,card_number,customer_account_no,transaction_date,transaction_time,transaction_amount,transaction_currency,transaction_sequence,transaction_type,transaction_response,transaction_settlement_date,card_network,card_issuer from Reconciliation_host_data ";
if (where != null && where.Trim().Length > 0)
cmd.CommandText = string.Format("{0} where {1}", cmd.CommandText, where);

return new ReconciliationHostDataReader(cmd.ExecuteReader(), conn);
}

static public ReconciliationHostDataReader ExecuteReader(string where)
{
return ExecuteReader(where, ConnectionFactory.GetNewConnection());
}

public static ReconciliationHostData LoadReconciliationHostData(string where)
{
ReconciliationHostDataReader reader = ReconciliationHostData.ExecuteReader(where);
ReconciliationHostData _reconciliationhostdata = null;
if (reader.Read())
_reconciliationhostdata = reader.CurrentReconciliationHostData;
reader.Close();
return _reconciliationhostdata;
}

public static ReconciliationHostData LoadReconciliationHostData(string where, IDbConnection conn)
{
ReconciliationHostDataReader reader = ReconciliationHostData.ExecuteReader(where, conn);
ReconciliationHostData _reconciliationhostdata = null;
if (reader.Read())
_reconciliationhostdata = reader.CurrentReconciliationHostData;
reader.Close(false);
...
public static ReconciliationHostData LoadReconciliationHostDataByPk( int reconciliation_host_data_id )
{
return LoadReconciliationHostData( " reconciliation_host_data_id="+reconciliation_host_data_id );
}

public static ReconciliationHostData LoadReconciliationHostDataByPk( int reconciliation_host_data_id , IDbConnection conn)
{
return LoadReconciliationHostData(" reconciliation_host_data_id="+reconciliation_host_data_id , conn);
}

public void Save()
{
if (reconciliation_host_data_idChanged || reconciliation_batch_idChanged || atm_idChanged || card_numberChanged || customer_account_noChanged || transaction_dateChanged || transaction_timeChanged || transaction_amountChanged || transaction_currencyChanged || transaction_sequenceChanged || transaction_typeChanged || transaction_responseChanged || transaction_settlement_dateChanged || card_networkChanged || card_issuerChanged )
...
if (this.isNewEntity)
{
qry.Append(@"insert into Reconciliation_host_data( reconciliation_host_data_id,reconciliation_batch_id,atm_id,card_number,customer_account_no,transaction_date,transaction_time,transaction_amount,transaction_currency,transaction_sequence,transaction_type,transaction_response,transaction_settlement_date,card_network,card_issuer ) values(");
lock (ConnectionFactory.connectionString) { this.reconciliation_host_data_id = ConnectionFactory.GetNextId();
qry.Append(this.reconciliation_host_data_id);
} qry.Append(",");
...
if (!(reconciliation_host_data_idChanged || reconciliation_batch_idChanged || atm_idChanged || card_numberChanged || customer_account_noChanged || transaction_dateChanged || transaction_timeChanged || transaction_amountChanged || transaction_currencyChanged || transaction_sequenceChanged || transaction_typeChanged || transaction_responseChanged || transaction_settlement_dateChanged || card_networkChanged || card_issuerChanged ))
return;
qry.Append("UPDATE Reconciliation_host_data set "); if ( reconciliation_batch_idChanged )
{
...
{
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "DELETE Reconciliation_host_data where reconciliation_host_data_id = "+ reconciliation_host_data_id;
...
public static void DeleteReconciliationHostDatas(string where)
{
ConnectionFactory.ExecuteQuery("delete Reconciliation_host_data where " + where);
}
...
#endregion
public void BulkSave(List<ReconciliationHostData> dataArray,SqlTransaction dbTrx)
{
DataTable dt = new DataTable();
CreateDataTable(dt);
AddToDataTable(dataArray, ref dt);
SqlBulkCopy bulk = new SqlBulkCopy(dbTrx.Connection, SqlBulkCopyOptions.Default, dbTrx);
bulk.DestinationTableName = "Reconciliation_host_data";
bulk.WriteToServer(dt);
}
public void CreateDataTable(DataTable dt)
{
string[] colNames = Enum.GetNames(typeof(ReconciliationHostData.Columns));
for (int i = 0; i < colNames.Length; i++)
{
dt.Columns.Add(colNames[i]);
}
}
public void AddToDataTable(List <ReconciliationHostData> transList,ref DataTable dt)
{
foreach (ReconciliationHostData tran in transList)
{
DataRow Row;
Row = dt.NewRow();
Row["reconciliation_host_data_id"] =ConnectionFactory.GetNextId();
