...
namespace Avanza.CCMS.Parser
{
    ...
    public class Parser
    {
        int TrxnsMarginalIntervalInSeconds = int.Parse(System.Configuration.ConfigurationManager.AppSettings["TrxnsMarginalIntervalInSeconds"]);

        string AddCashReplenishmentHandling = System.Configuration.ConfigurationManager.AppSettings["AddCashReplenishmentHandling"];
        ...
        string enableBillCashWithdrawalParsing = System.Configuration.ConfigurationManager.AppSettings["EnableBillWithdrawalParsing"];
        string ignoreWithdrawalAndReplenishmentProcessingForRecyclers = System.Configuration.ConfigurationManager.AppSettings["ignoreWithdrawalAndReplenishmentProcessingForRecyclers"];
        ...
        {
            string addCashReportedDatetime = "";
            ...
            List<Replenishment> listReplenishment = new List<Replenishment>();
            ...
            List<string> listAddCashDatetime = new List<string>();
            DateTime testCashDateTime = DateTime.MinValue;
            ...
            int[] billNotesAdded = new int[7];
            bool isSwap = false;
            bool isBillSwap = false;
            ...
            int[] denominations = { noteSetType.DenominationType1.Value,noteSetType.DenominationType2.Value,noteSetType.DenominationType3.Value,
                        noteSetType.DenominationType4.Value,noteSetType.DenominationType5.Value,noteSetType.DenominationType6.Value,noteSetType.DenominationType7.Value};
            string depositSummaryAfterModeChanged = "";
            try
            {
                for (int i = 0; i < parts.Length; i++)
                {
                    ...
                    if (ignoreWithdrawalAndReplenishmentProcessingForRecyclers == "1" &&
                        ATM.IsRecycler != null && ATM.IsRecycler.Value && (subParts[1].ToLower().Contains("withdrawal") || subParts[1].ToLower().Contains("replenishment")))
                    {
                        task.Log(MethodBase.GetCurrentMethod(), TraceLevel.Info, "ignored " + parts[i] + " because processing is forcefully stopped for the recyclers machines");
                        continue;


                    }

                    task.Log(MethodBase.GetCurrentMethod(), TraceLevel.Info, "Processing " + parts[i]);
                        ...
                        subParts[1] == "EODBalance" || subParts[1] == "BillEODBalance" || subParts[1] == "BNAEODBalance")
                    {
                        ...
                        if (subParts[1] == "DepositSummaryAfterModeChanged")
                            depositSummaryAfterModeChanged = parts[i];

                    }

                    if (AddCashReplenishmentHandling == "1")
                    {
                        if ((subParts[1] != "AddCash" && subParts[1] != "BillAddCash") && isReplenishmentExpected)
                        {
                            ...
                            bool _IsBillDispenser = false;
                            if (subParts[1] == "BillAddCash")
                                _IsBillDispenser = true;
                            else
                                _IsBillDispenser = false;

                            if (ATM.AtmType.ToLower().Contains("brm"))
                                _IsBillDispenser = true;


                            if (CheckForDuplicates("GetReplenishmentRow", string.Format("rep_datetime>=convert(datetime,'{0}:00:00',101) and rep_datetime<=convert(datetime,'{2}:59:59',101)  and atm_id={1} and cash_added1={3} and cash_added2={4} and cash_added3={5} and cash_added4={6} and is_swap={7} and IsBillDispenser={8}",
                                      DateTime.ParseExact(addCashReportedDatetime, "MM/dd/yyyy HH:mm:ss", null).AddMinutes(-20).ToString("MM/dd/yyyy HH"),
                                      ATMID, DateTime.ParseExact(addCashReportedDatetime, "MM/dd/yyyy HH:mm:ss", null).ToString("MM/dd/yyyy HH"), notesAdded[0], notesAdded[1], notesAdded[2], notesAdded[3], isSwap ? 1 : 0, _IsBillDispenser ? 1 : 0)))
                            {
                            ...
                            replenishment.AtmId = ATMID;
                            replenishment.RepDatetime = DateTime.ParseExact(addCashReportedDatetime, "MM/dd/yyyy HH:mm:ss", null);
                            ...
                            parserPostProcessingTask.EventInfo = replenishment.RepDatetime.ToString("MM/dd/yyyy HH:mm:ss") + "|Replenishment|OrderMissing|20180513132747|20180513132747|-1|1564|0|81|1183|0|0|0|2416|0|883|783|0|0|0|20|0|36|34|0|0|0|" + replenishment.CashAdded1 + "|" + replenishment.CashAdded2 + "|" + replenishment.CashAdded3 + "|" + replenishment.CashAdded4 + "|0|0|0";
                    ...
                    else if (subParts[1] == "BNACountsCleared")
                    {
                    ...
                    {
                        LogableTask.LogMonoActivityTask("parsing", MethodBase.GetCurrentMethod(), TraceLevel.Info, "Processing " + parts[i]);
                        ...
                        notesDepositInCassette = parsedBnaCounter.Cassette1Counter1.Value;
                        ...
                        notesDepositInCassette += parsedBnaCounter.Cassette2Counter1.Value;
                        ...
                        notesDepositInCassette += parsedBnaCounter.Cassette3Counter1.Value;
                        ...
                        notesDepositInCassette += parsedBnaCounter.Cassette4Counter1.Value;
                        ...
                        if (notesDepositInCassette > 0)
                        {
                            //Changes done by IK on 15-Nov-2023..
                            //On UAT parser crashing to parse the counter file on line 
                            if (i + 1 < parts.Length)

                            //                           if (i + 1 <= parts.Length)
                            {
                                string nextEvent = parts[i + 1];
                                string[] nextEventParts = nextEvent.Split('|');
                                if (DateTime.ParseExact(nextEventParts[0], "MM/dd/yyyy HH:mm:ss", null) == parsedBnaCounter.LastDepositAt)
                                {
                                    task.Log(MethodBase.GetCurrentMethod(), TraceLevel.Info, "Ignoring because of same datetime logic");
                                    continue;

                                }
                                if ((DateTime.ParseExact(nextEventParts[0], "MM/dd/yyyy HH:mm:ss", null) - parsedBnaCounter.LastDepositAt).TotalSeconds <= 3
                                     && nextEventParts[1].ToLower().Contains("testcash"))
                                {
                                    task.Log(MethodBase.GetCurrentMethod(), TraceLevel.Info, "Ignoring because of next test cash event");
                                    continue;
                    ...
                    #endregion
                    #region TestCash
                    else if (subParts[1] == "TestCash" || subParts[1] == "BillTestCash")
                    {
                        //if (appSetting.IsDuplicateCheckingEnabled.Value)
                        //{
                    ...
                    else if (subParts[1] == "CashWithdrawal" || (enableBillCashWithdrawalParsing == "1" && subParts[1] == "BillCashWithdrawal"))
                    {
                        j = 0;
                        ...
                        if (subParts[1] == "BillCashWithdrawal")
                        {
                            ...
                            //     if (parsedTransaction.CashDispensed1 + parsedTransaction.CashDispensed2 + parsedTransaction.CashDispensed3 + parsedTransaction.CashDispensed4 == 0)
                            //     {
                            //         task.Log(MethodBase.GetCurrentMethod(), TraceLevel.Info, "Ignoring " + parts[i] + ".because dispensed amount is zero");
                            //         continue;
                        ...
                        parsedTransaction.TrxnDatetime.AddSeconds(-TrxnsMarginalIntervalInSeconds).ToString("MM/dd/yyyy HH:mm:ss"), ATMID, parsedTransaction.Amount,
                        parsedTransaction.TrxnDatetime.AddSeconds(TrxnsMarginalIntervalInSeconds).ToString("MM/dd/yyyy HH:mm:ss")), 2, null);
                    ...
                    #region AddCash
                    else if (subParts[1] == "CountsCleared" || subParts[1] == "BillCountsCleared")
                    {
                        if (subParts[1] == "CountsCleared")
                            isSwap = true;
                        else
                            isBillSwap = true;
                    ...
                    else if (subParts[1] == "AddCash" || subParts[1] == "BillAddCash")
                    {
                        addCashReportedDatetime = subParts[0];
                        listAddCashDatetime.Add(addCashReportedDatetime);
                        ...
                        for (int n = 0; n < 7; n++)
                        {
                            if (subParts[1] == "AddCash")
                            {
                                ...
                                {
                                    notesAdded[n] += int.Parse(subParts[n + 9]);
                                }
                            }
                            else
                            {
                                if (isBillSwap)
                                {
                                    billNotesAdded[n] = int.Parse(subParts[n + 2]) + int.Parse(subParts[n + 9]);
                                }
                                else
                                {
                                    billNotesAdded[n] += int.Parse(subParts[n + 9]);
                                }
                            }
                        }
                        ////////////////////////////////////////////////////////
                        //Handling end of file cases for the add cash scenarios.
                        //Add empty string so that logic that looks for event after add cash gets executed.
                        ////////////////////////////////////////////////////////
                        if (i + 1 == parts.Length)
                        {
                            string[] temp = new string[parts.Length + 1];
                            Array.Copy(parts, temp, 0);
                            temp[temp.Length - 1] = "EOF|";
                            parts = temp;
                        }
                        continue;
                    }
                    ...
                    else if (subParts[1] == "Replenishment" || subParts[1] == "BillReplenishment") //replenishment started.
                    {
                        ...
                        DateTime repDateTime = DateTime.ParseExact(subParts[0], "MM/dd/yyyy HH:mm:ss", null);
                        if ((repStartDate.Date - repEndDate.Date).Hours >= 24)
                        {
                        ...
                        // in progress
                        //DateTime addCashDateTime = DateTime.ParseExact(addCashReportedDatetime, "MM/dd/yyyy HH:mm:ss", null);
                        //if ((repDateTime - addCashDateTime).TotalMinutes <= 5)
                        //{
                        //    task.Log(MethodBase.GetCurrentMethod(), TraceLevel.Info, "Ignoring " + parts[i] + ".because addCash fall within 5 min range ");
                        //    continue;
                        //}
                        ...
                        //          DateTime.ParseExact(subParts[0], "MM/dd/yyyy HH:mm:ss", null).AddMinutes(-20).ToString("MM/dd/yyyy HH"), ATMID, 
                        //          DateTime.ParseExact(subParts[0], "MM/dd/yyyy HH:mm:ss", null).ToString("MM/dd/yyyy HH"), subParts[27], subParts[28], subParts[29], subParts[30])))
                        //{
                        //    task.Log(MethodBase.GetCurrentMethod(), TraceLevel.Info, "Ignoring " + parts[i] + ".because this already exists in Replenishment table");
                        //    continue;
                        //}
                        ...
                        //BillDispenserHandlingWhileCheckingDuplicate -  09/08/2026 -- Jabbar.
                        ...
                        bool isSwapCommon = subParts[1] == "BillReplenishment" ? isBillSwap : isSwap;

                        if (CheckForDuplicates("GetReplenishmentRow", string.Format("rep_datetime>=convert(datetime,'{0}',101) and rep_datetime<=convert(datetime,'{2}',101)  and atm_id={1} and cash_added1={3} and cash_added2={4} and cash_added3={5} and cash_added4={6} and is_swap={7} and IsBillDispenser={8}",
                                 repDateTime.AddMinutes(-20).ToString("MM/dd/yyyy HH:mm:ss"), ATMID,
                                 repDateTime.ToString("MM/dd/yyyy HH:mm:ss"), subParts[27], subParts[28], subParts[29], subParts[30], isSwapCommon ? 1 : 0, _IsBillDispenser ? 1 : 0)))
                        {
                            task.Log(MethodBase.GetCurrentMethod(), TraceLevel.Info, "Ignoring " + parts[i] + ".because this already exists in Replenishment table");
                            continue;
                        }
                        ...
                        for (int l = 0; l < listReplenishment.Count; l++)
                        {
                            if ((repDateTime - listReplenishment[l].RepDatetime).Minutes <= 25 &&
                               listReplenishment[l].CashAdded1 == int.Parse(subParts[27]) &&
                               listReplenishment[l].CashAdded2 == int.Parse(subParts[28]) &&
                               listReplenishment[l].CashAdded3 == int.Parse(subParts[29]) &&
                               listReplenishment[l].CashAdded4 == int.Parse(subParts[30]) && listReplenishment[l].IsSwap == isSwapCommon
                        ...
                        {
                            replenishment.CashAdded1 = int.Parse(subParts[j++]);
                            if (replenishment.CashAdded1 <= 0 || (billNotesAdded[0] > replenishment.CashAdded1 && isBillSwap))
                            {
                                LogableTask.LogMonoActivityTask("CheckRep", MethodBase.GetCurrentMethod(), TraceLevel.Verbose,
                                        " type1 <=0 so updating replenishment with notes added counter " + replenishment.CashAdded1 + " updating with " + billNotesAdded[0]);

                                replenishment.CashAdded1 = billNotesAdded[0];
                            }

                            replenishment.CashAdded2 = int.Parse(subParts[j++]);
                            if (replenishment.CashAdded2 <= 0 || (billNotesAdded[1] > replenishment.CashAdded2 && isBillSwap))
                            {
                                LogableTask.LogMonoActivityTask("CheckRep", MethodBase.GetCurrentMethod(), TraceLevel.Verbose,
                                            " type2 <=0 so updating replenishment with notes added counter " + replenishment.CashAdded2 + " updating with " + billNotesAdded[1]);
                                replenishment.CashAdded2 = billNotesAdded[1];
                            }

                            replenishment.CashAdded3 = int.Parse(subParts[j++]);
                            if (replenishment.CashAdded3 <= 0 || (billNotesAdded[2] > replenishment.CashAdded3 && isBillSwap))
                            {
                                LogableTask.LogMonoActivityTask("CheckRep", MethodBase.GetCurrentMethod(), TraceLevel.Verbose,
                               " type3 <=0 so updating replenishment with notes added counter " + replenishment.CashAdded3 + " updating with " + billNotesAdded[2]);
                                replenishment.CashAdded3 = billNotesAdded[2];
                            }

                            replenishment.CashAdded4 = int.Parse(subParts[j++]);
                            if (replenishment.CashAdded4 <= 0 || (billNotesAdded[3] > replenishment.CashAdded4 && isBillSwap))
                            {
                                LogableTask.LogMonoActivityTask("CheckRep", MethodBase.GetCurrentMethod(), TraceLevel.Verbose,
                                " type4 <=0 so updating replenishment with notes added counter " + replenishment.CashAdded4 + " updating with " + billNotesAdded[3]);
                                replenishment.CashAdded4 = billNotesAdded[3];
                            }
                        }
                        ...
                        replenishment.IsSwap = isSwapCommon;
                        ...
                        */
                        ...
                        notesAdded = new int[7];
                        //After consuming status reset isSwap to False so that counts clear will make it again true 
                ...
                if (AddCashReplenishmentHandling == "1")
                {
                    for (int i = 0; i < listAddCashDatetime.Count; i++)
                    {
                        DateTime addCashDateTime = DateTime.ParseExact(listAddCashDatetime[i], "MM/dd/yyyy HH:mm:ss", null);
                        ConnectionFactory.ExecuteQuery(
                           string.Format("delete replenishment where atm_Id = {0} and rep_datetime>=convert(datetime,'{1}',103) and rep_datetime<convert(datetime,'{2}',103) ",
                         ATMId, addCashDateTime.AddMinutes(-5).ToString("dd/MM/yyyy HH:mm:ss"), addCashDateTime.ToString("dd/MM/yyyy HH:mm:ss")), DatabaseName.Cash);

                        ConnectionFactory.ExecuteQuery(
                            string.Format("delete parser_post_processing_task where atm_Id = {0} and event_type='Replenishment' and event_occured_at>=convert(datetime,'{1}',103) " +
                            "and event_occured_at<convert(datetime,'{2}',103) ",
                            ATMId, addCashDateTime.AddMinutes(-5).ToString("dd/MM/yyyy HH:mm:ss"), addCashDateTime.ToString("dd/MM/yyyy HH:mm:ss")), DatabaseName.Cash);
                    }
                }
