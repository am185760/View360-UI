...
namespace ATM360
{
    public partial class EView360ATMStreaming : ServiceBase
    {
        ...
        static string logFilePath = AppDomain.CurrentDomain.BaseDirectory + "\\Logs";
        public static string TransportQueuePath = AppDomain.CurrentDomain.BaseDirectory + "\\TransportQueue";
        public static string uploadedFilePath = @"C:\program files\ncr aptra\advance NDC\CCMSAgent\Uploaded";
        ...
        public static string tempFilePath = AppDomain.CurrentDomain.BaseDirectory + "\\temp\\";
        ...
        static int uploadRefreshInterval = 5;
        static int cutOverIntervalDays = 5;
        static int heartBeatRefreshInterval = 5;
        Timer timerCutOver;
        Timer timerDoWork;
        ...
        {
            watcher = new FileSystemWatcher("C:\\program files\\ncr aptra\\advance NDC\\CCMSAgent");
        ...
        //    //ReadCounterFile(e.FullPath);

        //}
        protected override void OnStart(string[] args)
        {
            timerSchedular = new Timer(Schedular, null, new TimeSpan(0, 0, 5), new TimeSpan(0, 0, 0, 0, -1));
        }

        private void DoInit()
        {
            RegistryKey key = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\NCR\CCMSAgent");

            if (key == null)
            {
                regKey = @"SOFTWARE\WOW6432Node\NCR\CCMSAgent";
                key = Registry.LocalMachine.OpenSubKey(regKey);
            }

            if (key == null)
            {
                throw new Exception("CCMSAgent registry key not found.");
            }
        ...
        // 20-07-26 this function handle the case: If service restarted and a file already there in ccmsagent folder
        private void Schedular(object state)
        {
            try
            {
                ...
                // Scan for existing files in case the service was stopped when they were created
                try
                {
                    string ccmsAgentPath = @"C:\program files\ncr aptra\advance NDC\CCMSAgent";
                    if (Directory.Exists(ccmsAgentPath))
                    {
                        string[] existingFiles = Directory.GetFiles(ccmsAgentPath, "counter*.dll");
                        foreach (string file in existingFiles)
                        {
                            queue.Add(file);
                            LogableTask.LogMonoActivityTask("Schedular", System.Reflection.MethodBase.GetCurrentMethod(), TraceLevel.Info, "Added pre-existing file to queue: " + file);
                        }
                    }
                }
                catch (Exception ex)
                {
                    LogableTask.LogMonoActivityTask("Schedular", System.Reflection.MethodBase.GetCurrentMethod(), TraceLevel.Error, ex);
                }

                Task.Run(() => ReadCounterFile());
        ...
        // 20-07-26 this function do not handle the case: If service restarted and a file already there in ccmsagent folder
        //private void Schedular(object state)
        //{
        //    try
        //    {
        ...
        private static string ReadFile(string sourceFilePath)
        {
            string data = null;

            if (sourceFilePath.Length > 0)
            {
                string filename = Path.GetFileName(sourceFilePath) + "_" + DateTime.Now.ToString("ddMMyyHHmmss");
                File.Move(sourceFilePath, tempFilePath + "\\" + filename);
                data = File.ReadAllText(tempFilePath + "\\" + filename, System.Text.Encoding.ASCII);
                File.Delete(tempFilePath + "\\" + filename); //dangerous call.
        ...
        static void CreateZipFile(string file, string data)
        {
            ...
            string baseName = file.Contains("1.dll") ? "Counters1_" : "Counters_";
            string zipFileName = Path.Combine(uploadedFilePath, baseName + datetimeStr + ".zip");

            FileStream fileStream = new FileStream(zipFileName, FileMode.Create, FileAccess.Write);
            ZipOutputStream stream = new ZipOutputStream(fileStream);
            ZipEntry entry;
            entry = new ZipEntry(Path.GetFileName(file));
            byte[] filebytes = System.Text.Encoding.ASCII.GetBytes(data);
        ...
        private void ReadCounterFile()
        {
            ...
            while (!queue.IsCompleted)
            {
                try
                {
                    ...
                    if (processedFileMap.TryGetValue(fileToRead, out DateTime processedWithModDate))
                    {
                        if (processedWithModDate == lastWriteTime)
                        {
                            LogableTask.LogMonoActivityTask("DoFileParse", System.Reflection.MethodBase.GetCurrentMethod(), TraceLevel.Info, $"Ignoring duplicate change event for file: {fileToRead}");
                            //Console.WriteLine($"Ignoring duplicate change event for file: {fileToRead}");
                            continue;
                        }

                        //It's a new change, so process it, then update mod date.
                        //Console.WriteLine($"Processed file again: {fileToRead}");
                        //processedFileMap[fileToRead] = fileInfo.LastWriteTimeUtc;
                    }
                    else
                    {
                        //We haven't processed this file before. Process it, then save the mod date.
                        LogableTask.LogMonoActivityTask("DoFileParse", System.Reflection.MethodBase.GetCurrentMethod(), TraceLevel.Info, $"Processed file for the first time: {fileToRead}.");

                        //Console.WriteLine($"Processed file for the first time: {fileToRead}.");
                        processedFileMap.TryAdd(fileToRead, lastWriteTime);
                    }
                    ...
                    if (File.Exists(fileToRead))
                    {
                        ...
                        if (data.Length > 0)
                        {
                            if (data.Length > 0)
                            {
                                ...
                                FileSplitter(chunkFileCompletePath);
                                PushPendingFiles();
                                //                      manualResetEvent.Set();
                                CreateZipFile(fileToRead, data);
        ...
        {
            LogableTask.LogMonoActivityTask("FileProcessor", System.Reflection.MethodBase.GetCurrentMethod(), TraceLevel.Info, "Entered");
        ...
        private void PushPendingFiles()
        {
            ...
            communicator.Connect(serverIP, dataStreamingServerPort);
            DirectoryInfo dirInfo = new DirectoryInfo(EView360ATMStreaming.processorPath);
            FileInfo[] fileInfos = dirInfo.GetFiles().OrderBy(f => f.LastWriteTime).ToArray();
            foreach (FileInfo fileInfo in fileInfos)
                communicator.Upload(fileInfo.FullName);
