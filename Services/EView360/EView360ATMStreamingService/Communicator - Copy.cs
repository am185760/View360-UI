...
namespace ATM360
{
    class Communicator
    {
        ...
        public void ProcessCommands()
        {
            ...
            while (true)
            {
                DirectoryInfo dirInfo = new DirectoryInfo(SecuredTransportLayer.TransportQueuePath);
                FileInfo[] fileInfos = dirInfo.GetFiles().OrderBy(f => f.CreationTime).ToArray();
                foreach (FileInfo fileInfo in fileInfos)
                {
                    LogableTask.LogMonoActivityTask("DoUpload", System.Reflection.MethodBase.GetCurrentMethod(), TraceLevel.Info, "going to send messages " + fileInfo.FullName);
                    ...
                    if (command.StartsWith("OK"))
                    {
                        try
                        {
                            ...
                            while ((bytesRead = fileStream.Read(fileData, 0, 1023)) > 0)
                            {
                                ...
                                if (!SecuredTransportLayer.isSSLEnabled)
                ...
                //{
                //    File.Move(SecuredTransportLayer.TransportQueuePath + "\\" + filename, SecuredTransportLayer.uploadedFilePath + "\\" + filename);
                //    if (socket != null)
                //        socket.Close();
                //    else
                //    {
                ...
                //    byte[] fileData = new byte[1024];
                //    string[] parts = command.Split(';');
                //    string fileToSend = SecuredTransportLayer.TransportQueuePath + "\\" + filename;
        ...
        public void Talk(string ip, int port)
        {
            try
            {
                ...
                if (SecuredTransportLayer.isSSLEnabled)
                {
                    ...
                    sslStream.AuthenticateAsClient(SecuredTransportLayer.hostname, null, SslProtocols.Tls12, false);
                }
                else
                {
                    socket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
            ...
            catch (Exception ex)
            {
               ...
               SecuredTransportLayer.timerUploader.Change(new TimeSpan(0, 5, 0), new TimeSpan(0, 1, 0));
            }
            finally
            {
                if (tcpClient != null)
