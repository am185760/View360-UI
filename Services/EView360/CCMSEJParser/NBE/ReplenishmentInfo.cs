...
namespace Avanza.CCMS.Parser
{
    public class ReplenishmentExtractor
    {
        ...
        private Regex regex = new Regex(@"((?<CashAddedTime>\d+/\d+/\d+\*\d+:\d+)\*[ ]*\r?\n?(C(?<Type>\d+))?[ ]*CASH[ ]ADDED[ ]*\r?\n?[ ]*(TYPE[ ]\d+[ ]=[ ]+(?<Added>\d+)[ ]*\r?\n?)+)|((?<CountsClearingTime>\d+/\d+/\d+\*\d+:\d+)\*\s*CASH[ ]COUNTS[ ]CLEARED\s*(CASH[ ]DISPENSED[ ](CDM[ ]\d)?\s*(TYPE[ ]\d[ ]=[ ](?<Dispensed>\d+)\s+)+CASH[ ]REMAINING\s*(TYPE[ ]\d[ ]=[ ](?<Remaining>\d+)\s+)+)?)|((DATE-TIME[ ]+=(?<PrintCountersDateTime>\d{2}/\d{2}/\d{2}[ ]\d{2}:\d{2}))?(\r[ ]+TYPE[ ]\d[ ]+TYPE[ ]\d[ ]*\r[ ]CASSETTE[ ]+(?<Cassette1>\d+)[ ]+(?<Cassette2>\d+)\r\+REJECTED[ ]+(?<Rejected1>\d+)[ ]+(?<Rejected2>\d+)\r=REMAINING[ |]+(?<Remaining1>\d+)[ ]+(?<Remaining2>\d+)\r\+DISPENSED[ ]+(?<Dispensed1>\d+)[ ]+(?<Dispensed2>\d+)\r=TOTAL[ ]+(?<Total1>\d+)[ ]+(?<Total2>\d+)\r)*[ ]LAST[ ]CLEARED[ ]+(?<LastClearedDateTime>\d+/\d+/\d+[ ]\d+:\d+)\r{3}[ ]CARDS[ ]CAPTURED[ ]+(?<CardsCapturedCount>\d+))");
        ...
        Regex CountsClearingRegex = new Regex(@"((?<CountsClearingTime>\d+/\d+/\d+\*\d+:\d+)\*\s*CASH[ ]COUNTS[ ]CLEARED\s*CASH[ ]DISPENSED([ ]CDM[ ]\d+)?\s*(TYPE[ ]\d[ ]=[ ](?<Dispensed>\d+)\s+)+CASH[ ]REMAINING\s*(TYPE[ ]\d[ ]=[ ](?<Remaining>\d+)\s+)+)");
        ...
        Match match;
        Replenishment lastProcessedReplenishment;

        private Replenishment AddReplenishmentInCCMS(ref ServicesDAL.Task downloadTask, ref EjParsedReplenishments cashAdded, ref bool isSwap, ref Atm atm)
        {
            //this will be used for accumulating counters of dispenser and recycler even both are swap replenishments.
            bool tempIsSwap = isSwap;
            ...
            replenishment2.Save();
            LogableTask.LogMonoActivityTask("Extracting ej from db", MethodBase.GetCurrentMethod(), TraceLevel.Info, "ATM ID[" + downloadTask.ATMId + "],Datetime[" + replenishment2.RepDatetime + "],swap[" + replenishment2.IsSwap + "] saved");
            ...
            //cdm1//cdm2 
            if (!cashAdded.IsBillDispenser.Value) // this means dispenser
            {
                if (lastProcessedReplenishment != null)
                {
                    if ((lastProcessedReplenishment.CashAdded1 > 0 || lastProcessedReplenishment.CashAdded2 > 0 ||
                        lastProcessedReplenishment.CashAdded3 > 0 || lastProcessedReplenishment.CashAdded4 > 0) && lastProcessedReplenishment.IsBillDispenser.Value)
                    {
                        if (isSwap)
                            tempIsSwap = false;
                    }
            ...
            LogableTask.LogMonoActivityTask("Extracting ej from db", MethodBase.GetCurrentMethod(), TraceLevel.Info, "ATM ID[" + parserPostProcessingTask.AtmId + "],Datetime[" + parserPostProcessingTask.EventOccuredAt + "] business rules processor event saved");
     ...
     //               task.Log(MethodBase.GetCurrentMethod(), TraceLevel.Info, "cash order monitoring updated for atm : " + replenishment.AtmId);
            ...
            */
            ...
            if (replenishment2.IsBillDispenser.Value)
                lastProcessedReplenishment = replenishment2;

            return replenishment2;
        }

        EjParsedReplenishments ExtractCashAdd(ref Atm atm)
        {
            ...
            try
            {
                ...
                while (match.Success)
                {
                    ...
                    else
                    {
                        ...
                        {
                            cashAdded = ExtractCashAdd(ref atm);
                            ...
                            if ((int)cmd.ExecuteScalar() > 0)
                            {
                                ...
                                //If bill dispenser counters are read then set is swap to false;
                                if (atm.AtmType.ToLower().Contains("gbru"))
                                {
                                    if (!cashAdded.IsBillDispenser.Value)
                                        isSwap = false;
                                }
                                else
                                    isSwap = false;
                            ...
                            {
                                //Did not understand below logic.....
                                //02-11-2023
                                //cmd.CommandType = CommandType.Text;
                                //cmd.CommandText = "delete ej_parsed_replenishments where atm_id =  " + cashAdded.AtmId
                                //    + " and rep_Datetime<='" + cashAdded.RepDatetime.Value.ToString("yyyy-MM-dd HH:mm:ss") +
                                //    "' and rep_datetime>='" + cashAdded.RepDatetime.Value.AddHours(-2).ToString("yyyy-MM-dd HH:mm:ss") + "' and is_bill_dispenser = " + (cashAdded.IsBillDispenser.Value ? 1 : 0);
...
//                            cashAdded.Save(trxn.Connection, trxn);
                            ...
                            {
                                lastSavedReplenishment = this.AddReplenishmentInCCMS(ref downloadTask, ref cashAdded, ref isSwap, ref atm);
