                ...
                while (match.Success)
                {
                    ...
                    if (match.Groups["TotalDepositDetails"].Success)
                    {
                        ...
                        for (int i = 0; i < match.Groups["Denomination"].Captures.Count; i++)
                        {
                            ...
                            Capture denomCapture = match.Groups["Denomination"].Captures[i];

                            if (match.Groups["Denomination"].Success)
                                ejParsedTotalBnaTransactionDetail.NoteType = int.Parse(denomCapture.Value);

                            int nextDenomIndex = (i + 1 < match.Groups["Denomination"].Captures.Count)
                                ? match.Groups["Denomination"].Captures[i + 1].Index
                                : int.MaxValue;

                            foreach (Capture noteCapture in match.Groups["Notes"].Captures)
                            {
                                if (noteCapture.Index > denomCapture.Index && noteCapture.Index < nextDenomIndex)
                                {
                                    ejParsedTotalBnaTransactionDetail.NotesCount += int.Parse(noteCapture.Value);
                                }
                            }
