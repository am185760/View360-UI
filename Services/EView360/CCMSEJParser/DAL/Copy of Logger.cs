/* Author : Zubaer Naseem 
 * Dated : Nov 11,2005
 * Completed : Nov 14,2005
 * any use of this code in softwares not relatd to Avanza Solutions' is not allowed
 */
...
//namespace Avanza.Tools
//{
	/// <summary>
	/// represents one activity in a task, every  call to the 
	/// function LogableTask.Log() adds a new instance to the activities in a LogableTask instance
	/// <seealso cref="LogableTask"/>
	/// </summary>
		...
		public class XmlLogWriter
		{
            ...
            public static void InitXmlLogWriter(string logFileName)
            {
                ...
                lock (logfilelock)
                {
                    if (xmlWriter != null)
                    {
                        FileStream stream = xmlWriter.BaseStream as FileStream;
                    ...
                    {
                        xmlWriter = new XmlTextWriter(new FileStream(fileName, FileMode.Open, FileAccess.ReadWrite, FileShare.Read), System.Text.Encoding.ASCII);
                        xmlWriter.BaseStream.Position = xmlWriter.BaseStream.Length;
                    }
                    else
                    {

                        xmlWriter = new XmlTextWriter(new FileStream(fileName, FileMode.Create, FileAccess.ReadWrite), System.Text.Encoding.ASCII);
                        ...
                        xmlWriter.WriteElementString("Time", DateTime.Now.ToString("MMM-dd HH:mm:ss"));
                        xmlWriter.WriteElementString("TraceLevel", TraceLevel.Info.ToString());
                        xmlWriter.WriteElementString("FunctionName", "Logwriter..ctor ");
                        xmlWriter.WriteElementString("Msg", "File path " + fileName);
            ...
            static public void WriteTask(LogableTask task)
            {
                ...
                lock (logfilelock)
                {
                    ...
                    if (xmlWriter.BaseStream.Position > 0)
                        xmlWriter.BaseStream.Position = xmlWriter.BaseStream.Position - 6;
                    xmlWriter.WriteStartElement("Task");
                    xmlWriter.WriteElementString("RefNo", task.Taskno.ToString());
                    if (task.TaskName != null)
                        xmlWriter.WriteElementString("Name", task.TaskName);
                    ...
                    for (int i = 0; i < task.activities.Count; i++)
                    {
                        ...
                        xmlWriter.WriteStartElement("Activity", "");
                        xmlWriter.WriteElementString("Time", act.Time.ToString("ddMMM_HH:mm:ss"));
                        xmlWriter.WriteElementString("TraceLevel", act.level.ToString());
                        xmlWriter.WriteElementString("FunctionName", act.FunctionName);
                        if (act.stackTrace != null)
                            xmlWriter.WriteElementString("StackTrace", act.stackTrace);
                        xmlWriter.WriteElementString("Msg", act.msg);
                        xmlWriter.WriteEndElement();
                    }
                    xmlWriter.WriteEndElement();
