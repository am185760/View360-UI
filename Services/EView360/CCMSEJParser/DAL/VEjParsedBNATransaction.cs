...
{
    [Serializable()]
    public class VEjParsedBNATransaction
    {
        ...
        public VEjParsedBNATransaction() { }
        public VEjParsedBNATransaction(int ej_parsed_bna_transaction_id, int atm_id, DateTime generated_at, int start_index, int end_index, int task_id)
        {
        ...
        public VEjParsedBNATransaction(int ej_parsed_bna_transaction_id, DateTime? trxn_datetime, string terminal_id, string seq, string account_type, string pan, string consumer_message_id, string dispute_status, decimal? amount_authorized, string status, string comment, string processed_tran, int atm_id, DateTime generated_at, int start_index, int end_index, int task_id, bool? is_eligible, DateTime? transaction_start_time, string transaction_end_time, string card_taken_time, bool? is_disputed_transaction, string host_tsn, string account_no, DateTime? posting_date, string currency, int? transaction_type_id, string network, bool? is_cardless)
        {
            this.ej_parsed_bna_transaction_id = ej_parsed_bna_transaction_id;
            this.ej_parsed_bna_transaction_idChanged = true;
        ...
        #region VEjParsedBNATransactionReader
        public class VEjParsedBNATransactionReader : IEntityReader, IEnumerator, IEnumerable
        {
            IDataReader reader;
            IDbConnection conn;
            VEjParsedBNATransaction currentVEjParsedBNATransaction;
            Columns columns;
            bool partialRead = false;
            private VEjParsedBNATransactionReader() { }
            /// 
            ///
            ///
            ...
            public VEjParsedBNATransactionReader(IDataReader reader, IDbConnection conn)
            {
                this.reader = reader;
                this.conn = conn;
            }
            public VEjParsedBNATransactionReader(IDataReader reader, IDbConnection conn, Columns columns)
            {
                this.reader = reader;
                this.conn = conn;
            ...
            {
                get { return currentVEjParsedBNATransaction; }
            ...
            public bool Read()
            {
                if (reader.Read())
                {
                    currentVEjParsedBNATransaction = new VEjParsedBNATransaction();
                    if (partialRead)
                    {
                    ...
                    currentVEjParsedBNATransaction.isNewEntity = false;
                    return true;
                }
                else
                    return false;
            ...
            public VEjParsedBNATransaction CurrentVEjParsedBNATransaction
            {
                get { return currentVEjParsedBNATransaction; }
            }
        ...
        #region VEjParsedBNATransaction functions

        public static VEjParsedBNATransactionReader ExecuteReader(string where, IDbConnection conn, Columns columns)
        {
            ...
            qry.Append("from VEjParsedBNATransaction ");
            ...
            return new VEjParsedBNATransactionReader(cmd.ExecuteReader(), conn, columns);
        }

        static public VEjParsedBNATransactionReader ExecuteReader(string where, Columns columns)
        {
        ...
        /// 
        /// 
        public static VEjParsedBNATransactionReader ExecuteReader(string where, IDbConnection conn)
        {
            if (conn.State != ConnectionState.Open)
                conn.Open();
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
            cmd.ExecuteNonQuery();
            cmd.CommandText = "Select ej_parsed_bna_transaction_id,trxn_datetime,terminal_id,seq,account_type,pan,consumer_message_id,dispute_status,amount_authorized,status,comment,processed_tran,atm_id,generated_at,start_index,end_index,task_id,is_eligible,transaction_start_time,transaction_end_time,card_taken_time,is_disputed_transaction,host_tsn,account_no,posting_date,currency,transaction_type_id,network,is_cardless from VEjParsedBNATransaction ";
            if (where != null && where.Trim().Length > 0)
                cmd.CommandText = string.Format("{0} where {1}", cmd.CommandText, where);

            return new VEjParsedBNATransactionReader(cmd.ExecuteReader(), conn);
        }

        static public VEjParsedBNATransactionReader ExecuteReader(string where)
        {
            return ExecuteReader(where, ConnectionFactory.GetNewConnection());
        }

        public static VEjParsedBNATransaction LoadVEjParsedBNATransactionsByPk(int ej_parsed_bna_transactions_id)
        {
            return LoadVEjParsedBNATransaction(" ej_parsed_bna_transaction_id=" + ej_parsed_bna_transactions_id);
        }

        public static VEjParsedBNATransaction LoadVEjParsedBNATransactionsByPk(int ej_parsed_bna_transactions_id, IDbConnection conn)
        {
            return LoadVEjParsedBNATransaction(" ej_parsed_bna_transaction_id=" + ej_parsed_bna_transactions_id, conn);
        }

        public static VEjParsedBNATransaction LoadVEjParsedBNATransaction(string where)
        {
            VEjParsedBNATransactionReader reader = VEjParsedBNATransaction.ExecuteReader(where);
            VEjParsedBNATransaction _vejparsedbnatransaction = null;
            if (reader.Read())
                _vejparsedbnatransaction = reader.CurrentVEjParsedBNATransaction;
            reader.Close();
            return _vejparsedbnatransaction;
        }

        public static VEjParsedBNATransaction LoadVEjParsedBNATransaction(string where, IDbConnection conn)
        {
            VEjParsedBNATransactionReader reader = VEjParsedBNATransaction.ExecuteReader(where, conn);
            VEjParsedBNATransaction _vejparsedbnatransaction = null;
            if (reader.Read())
                _vejparsedbnatransaction = reader.CurrentVEjParsedBNATransaction;
            reader.Close(false);
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (ej_parsed_bna_transaction_idChanged || trxn_datetimeChanged || terminal_idChanged || seqChanged || account_typeChanged || panChanged || consumer_message_idChanged || dispute_statusChanged || amount_authorizedChanged || statusChanged || commentChanged || processed_tranChanged || atm_idChanged || generated_atChanged || start_indexChanged || end_indexChanged || task_idChanged || is_eligibleChanged || transaction_start_timeChanged || transaction_end_timeChanged || card_taken_timeChanged || is_disputed_transactionChanged || host_tsnChanged || account_noChanged || posting_dateChanged || currencyChanged || transaction_type_idChanged || networkChanged || is_cardlessChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    qry.Append(@"insert into VEjParsedBNATransaction( ej_parsed_bna_transaction_id,trxn_datetime,terminal_id,seq,account_type,pan,consumer_message_id,dispute_status,amount_authorized,status,comment,processed_tran,atm_id,generated_at,start_index,end_index,task_id,is_eligible,transaction_start_time,transaction_end_time,card_taken_time,is_disputed_transaction,host_tsn,account_no,posting_date,currency,transaction_type_id,network,is_cardless ) values(");
                ...
                {
                    throw new Exception("No primary key is defined, can not update VEjParsedBNATransaction!");
                }
        ...
        public static void DeleteVEjParsedBNATransactions(string where)
        {
            ConnectionFactory.ExecuteQuery("delete VEjParsedBNATransaction where " + where);
        }
        ...
        public enum Columns : uint
        {
            ...
            account_type = 16,
            pan = 32,
            consumer_message_id = 64,
            dispute_status = 128,
            amount_authorized = 256,
            status = 512,
            comment = 1024,
            processed_tran = 2048,
            atm_id = 4096,
            ...
            task_id = 65536,
            is_eligible = 131072,
            transaction_start_time = 262144,
            transaction_end_time = 524288,
            card_taken_time = 1048576,
            is_disputed_transaction = 2097152,
            host_tsn = 4194304,
            account_no = 8388608,
            posting_date = 16777216,
            currency = 33554432,
            transaction_type_id = 67108864,
            network = 134217728,
            is_cardless = 268435456
        }
        #endregion
        public void BulkSave(List<VEjParsedBNATransaction> dataArray, SqlTransaction dbTrx)
        {
            DataTable dt = new DataTable();
            CreateDataTable(dt);
            AddToDataTable(dataArray, ref dt);
            SqlBulkCopy bulk = new SqlBulkCopy(dbTrx.Connection, SqlBulkCopyOptions.Default, dbTrx);
            bulk.DestinationTableName = "VEjParsedBNATransaction";
            bulk.WriteToServer(dt);
        }
        public void CreateDataTable(DataTable dt)
        {
            string[] colNames = Enum.GetNames(typeof(VEjParsedBNATransaction.Columns));
            for (int i = 0; i < colNames.Length; i++)
            {
                dt.Columns.Add(colNames[i]);
            }
        }
        public void AddToDataTable(List<VEjParsedBNATransaction> transList, ref DataTable dt)
        {
            foreach (VEjParsedBNATransaction tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
