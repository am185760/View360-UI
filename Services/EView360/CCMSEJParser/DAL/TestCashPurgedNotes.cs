...
using System;
 ...
 public TestCashPurgedNotes( DateTime test_cash_datetime,int cash_purged1,int cash_purged2,int cash_purged3,int cash_purged4,int cash_purged5,int cash_purged6,int cash_purged7,int task_id,int replenishment_id,int atm_id,bool is_auto_generated )
 {
 ...
 private TestCashPurgedNotes( int test_cash_purged_notes_id,DateTime test_cash_datetime,int cash_purged1,int cash_purged2,int cash_purged3,int cash_purged4,int cash_purged5,int cash_purged6,int cash_purged7,int task_id,int replenishment_id,int atm_id,bool is_auto_generated )
 {
 ...
 private int test_cash_purged_notes_id;
 public int TestCashPurgedNotesId
 {
...
TestCashPurgedNotes currentTestCashPurgedNotes;
 ...
 currentTestCashPurgedNotes.test_cash_purged_notes_id =(int) reader["test_cash_purged_notes_id"]; 
 ...
 currentTestCashPurgedNotes.task_id =(int) reader["task_id"]; 
 if ((columns & Columns.replenishment_id) == Columns.replenishment_id && reader["replenishment_id"]!=DBNull.Value)
 currentTestCashPurgedNotes.replenishment_id =(int) reader["replenishment_id"]; 
 if ((columns & Columns.atm_id) == Columns.atm_id && reader["atm_id"]!=DBNull.Value)
 currentTestCashPurgedNotes.atm_id =(int) reader["atm_id"]; 
 ...
 cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
 cmd.ExecuteNonQuery();
 cmd.CommandText = "Select test_cash_purged_notes_id,test_cash_datetime,cash_purged1,cash_purged2,cash_purged3,cash_purged4,cash_purged5,cash_purged6,cash_purged7,task_id,replenishment_id,atm_id,is_auto_generated from Test_cash_purged_notes ";
 if (where != null && where.Trim().Length > 0)
...
TestCashPurgedNotes _testcashpurgednotes = null;
 ...
 public static TestCashPurgedNotes LoadTestCashPurgedNotesByPk( int test_cash_purged_notes_id )
 {
 return LoadTestCashPurgedNotes( " test_cash_purged_notes_id="+test_cash_purged_notes_id );
 }

 public static TestCashPurgedNotes LoadTestCashPurgedNotesByPk( int test_cash_purged_notes_id , IDbConnection conn)
 {
 return LoadTestCashPurgedNotes(" test_cash_purged_notes_id="+test_cash_purged_notes_id , conn);
 }

 public void Save()
 {
 if (test_cash_purged_notes_idChanged || test_cash_datetimeChanged || cash_purged1Changed || cash_purged2Changed || cash_purged3Changed || cash_purged4Changed || cash_purged5Changed || cash_purged6Changed || cash_purged7Changed || task_idChanged || replenishment_idChanged || atm_idChanged || is_auto_generatedChanged )
 ...
 if (this.isNewEntity)
 {
 qry.Append(@"insert into Test_cash_purged_notes( test_cash_purged_notes_id,test_cash_datetime,cash_purged1,cash_purged2,cash_purged3,cash_purged4,cash_purged5,cash_purged6,cash_purged7,task_id,replenishment_id,atm_id,is_auto_generated ) values(");
 lock (ConnectionFactory.connectionString) { this.test_cash_purged_notes_id = ConnectionFactory.GetNextId();
 ...
 if (!(test_cash_purged_notes_idChanged || test_cash_datetimeChanged || cash_purged1Changed || cash_purged2Changed || cash_purged3Changed || cash_purged4Changed || cash_purged5Changed || cash_purged6Changed || cash_purged7Changed || task_idChanged || replenishment_idChanged || atm_idChanged || is_auto_generatedChanged ))
 return;
 qry.Append("UPDATE Test_cash_purged_notes set "); if ( test_cash_datetimeChanged )
 {
 qry.Append("test_cash_datetime ="+test_cash_datetimeDbString);
 qry.Append(",");
 }
 ...
 {
 IDbCommand cmd = conn.CreateCommand();
 cmd.CommandText = "DELETE Test_cash_purged_notes where test_cash_purged_notes_id = "+ test_cash_purged_notes_id;
 ...
 {
 ConnectionFactory.ExecuteQuery("delete Test_cash_purged_notes where " + where);
 }
 ...
 {
test_cash_purged_notes_id= 1,
test_cash_datetime= 2,
cash_purged1= 4,
cash_purged2= 8,
cash_purged3= 16,
cash_purged4= 32,
...
atm_id= 2048,
is_auto_generated= 4096
 }
 #endregion
 public DataTable BulkSave(List<TestCashPurgedNotes> dataArray,SqlTransaction dbTrx)
 {
 ...
 DataRow Row;
 Row = dt.NewRow();
 Row["test_cash_purged_notes_id"] =ConnectionFactory.GetNextId();
