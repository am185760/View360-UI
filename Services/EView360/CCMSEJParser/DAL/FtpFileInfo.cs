...
public FtpFileInfo() { }
public FtpFileInfo( int ftp_file_info_id,DateTime creation_time,string status,int task_type_id,int retry_count,string ftp_filename,int region_id ) 
{
...
public FtpFileInfo( DateTime creation_time,DateTime? end_time,string status,int task_type_id,int retry_count,string failure_reason,string ftp_filename,int region_id,DateTime? last_invoked_at )
{
this.creation_time = creation_time;
this.creation_timeChanged = true;
...
private FtpFileInfo( int ftp_file_info_id,DateTime creation_time,DateTime? end_time,string status,int task_type_id,int retry_count,string failure_reason,string ftp_filename,int region_id,DateTime? last_invoked_at )
{
...
return string.Format("'{0}'",ftp_filename); else
return "null";
}
}
#endregion
...
currentFtpFileInfo.ftp_file_info_id =(int) reader["ftp_file_info_id"]; 
...
currentFtpFileInfo.task_type_id =(int) reader["task_type_id"]; 
...
currentFtpFileInfo.region_id =(int) reader["region_id"]; 
...
public static FtpFileInfo LoadFtpFileInfoByPk( int ftp_file_info_id )
{
return LoadFtpFileInfo( " ftp_file_info_id="+ftp_file_info_id );
}

public static FtpFileInfo LoadFtpFileInfoByPk( int ftp_file_info_id , IDbConnection conn)
{
...
lock (ConnectionFactory.connectionString) { this.ftp_file_info_id = ConnectionFactory.GetNextId();
...
{
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "DELETE Ftp_file_info where ftp_file_info_id = "+ ftp_file_info_id;
...
{
ConnectionFactory.ExecuteQuery("delete Ftp_file_info where " + where);
}
...
#endregion
public void BulkSave(List<FtpFileInfo> dataArray,SqlTransaction dbTrx)
{
...
DataRow Row;
Row = dt.NewRow();
Row["ftp_file_info_id"] =ConnectionFactory.GetNextId();
