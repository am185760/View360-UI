...
public CcmsAtmLog() { }
public CcmsAtmLog( int id,int task_id,DateTime processing_datetime ) 
{
...
public CcmsAtmLog( string event_name,DateTime? event_occured_at,string event_mode,string event_info,int? atm_id,string order_number,int task_id,DateTime processing_datetime )
{
...
private CcmsAtmLog( int id,string event_name,DateTime? event_occured_at,string event_mode,string event_info,int? atm_id,string order_number,int task_id,DateTime processing_datetime )
{
this.id = id;
this.idChanged = true;
this.event_name = event_name;
...
get
{
if (this.event_occured_at.HasValue)
return string.Format("Convert(datetime,'{0}',121)",event_occured_at.Value.ToString("yyyy-MM-dd HH:mm:ss:fff"));
else
return "null";
}
}
...
return string.Format("'{0}'",event_mode); else
return "null";
}
}
#endregion
...
return string.Format("'{0}'",event_info); else
return "null";
}
}
#endregion
...
currentCcmsAtmLog.id =(int) reader["id"]; 
...
currentCcmsAtmLog.atm_id =(int?) reader["atm_id"]; 
...
currentCcmsAtmLog.task_id =(int) reader["task_id"]; 
...
currentCcmsAtmLog.id = int.Parse(reader["id"].ToString()); 
...
currentCcmsAtmLog.atm_id = int.Parse(reader["atm_id"].ToString()); 
...
cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
cmd.ExecuteNonQuery();
cmd.CommandText = "Select id,event_name,event_occured_at,event_mode,event_info,atm_id,order_number,task_id,processing_datetime from Ccms_atm_log ";
if (where != null && where.Trim().Length > 0)
...
public static CcmsAtmLog LoadCcmsAtmLogByPk( int id )
{
return LoadCcmsAtmLog( " id="+id );
}

public static CcmsAtmLog LoadCcmsAtmLogByPk( int id , IDbConnection conn)
{
return LoadCcmsAtmLog(" id="+id , conn);
}
...
if (this.isNewEntity)
{
qry.Append(@"insert into Ccms_atm_log( event_name,event_occured_at,event_mode,event_info,atm_id,order_number,task_id,processing_datetime ) values(");
...
{
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "DELETE Ccms_atm_log where id = "+ id;
...
{
ConnectionFactory.ExecuteQuery("delete Ccms_atm_log where " + where);
}
...
#endregion
public void BulkSave(List<CcmsAtmLog> dataArray,SqlTransaction dbTrx)
{
