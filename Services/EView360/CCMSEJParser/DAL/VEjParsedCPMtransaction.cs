...
{
    [Serializable()]
    public class VEjParsedCPMtransaction
    {
        ...
        public VEjParsedCPMtransaction() { }
        public VEjParsedCPMtransaction(int ej_parsed_cpm_transaction_id, int atm_id, DateTime generated_at, int start_index, int end_index, int task_id, bool is_eligible)
        {
        ...
        #region VEjParsedCPMtransactionReader
        public class VEjParsedCPMtransactionReader : IEntityReader, IEnumerator, IEnumerable
        {
            IDataReader reader;
            IDbConnection conn;
            VEjParsedCPMtransaction currentVEjParsedCPMtransaction;
            Columns columns;
            bool partialRead = false;
            private VEjParsedCPMtransactionReader() { }
            /// 
            ///
            ///
            ...
            public VEjParsedCPMtransactionReader(IDataReader reader, IDbConnection conn)
            {
                this.reader = reader;
                this.conn = conn;
            }
            public VEjParsedCPMtransactionReader(IDataReader reader, IDbConnection conn, Columns columns)
            {
                this.reader = reader;
                this.conn = conn;
            ...
            {
                get { return currentVEjParsedCPMtransaction; }
            ...
            public bool Read()
            {
                if (reader.Read())
                {
                    currentVEjParsedCPMtransaction = new VEjParsedCPMtransaction();
                    if (partialRead)
                    {
                    ...
                    currentVEjParsedCPMtransaction.isNewEntity = false;
                    return true;
                }
                else
                    return false;
            ...
            public VEjParsedCPMtransaction CurrentVEjParsedCPMtransaction
            {
                get { return currentVEjParsedCPMtransaction; }
            }
        ...
        #region VEjParsedCPMtransaction functions

        public static VEjParsedCPMtransactionReader ExecuteReader(string where, IDbConnection conn, Columns columns)
        {
            ...
            qry.Append("from VEjParsedCPMtransaction ");
            ...
            return new VEjParsedCPMtransactionReader(cmd.ExecuteReader(), conn, columns);
        }

        static public VEjParsedCPMtransactionReader ExecuteReader(string where, Columns columns)
        {
        ...
        /// 
        /// 
        public static VEjParsedCPMtransactionReader ExecuteReader(string where, IDbConnection conn)
        {
            if (conn.State != ConnectionState.Open)
                conn.Open();
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
            cmd.ExecuteNonQuery();
            cmd.CommandText = "Select ej_parsed_cpm_transaction_id,trxn_datetime,terminal_id,seq,account_type,pan,deposit_amount,result,consumer_message_id,dispute_status,status,comment,reject_reason,processed_tran,atm_id,generated_at,dispense_amount,start_index,end_index,task_id,is_eligible,is_disputed_transaction,host_tsn,account_no,micr,transaction_type_id,is_cardless,bank_name,eida_name from VEjParsedCPMtransaction ";
            if (where != null && where.Trim().Length > 0)
                cmd.CommandText = string.Format("{0} where {1}", cmd.CommandText, where);

            return new VEjParsedCPMtransactionReader(cmd.ExecuteReader(), conn);
        }

        static public VEjParsedCPMtransactionReader ExecuteReader(string where)
        {
            return ExecuteReader(where, ConnectionFactory.GetNewConnection());
        }

        public static VEjParsedCPMtransaction LoadVEjParsedCPMtransaction(string where)
        {
            VEjParsedCPMtransactionReader reader = VEjParsedCPMtransaction.ExecuteReader(where);
            VEjParsedCPMtransaction _vejparsedcpmtransaction = null;
            if (reader.Read())
                _vejparsedcpmtransaction = reader.CurrentVEjParsedCPMtransaction;
            reader.Close();
            return _vejparsedcpmtransaction;
        }

        public static VEjParsedCPMtransaction LoadVEjParsedCPMtransaction(string where, IDbConnection conn)
        {
            VEjParsedCPMtransactionReader reader = VEjParsedCPMtransaction.ExecuteReader(where, conn);
            VEjParsedCPMtransaction _vejparsedcpmtransaction = null;
            if (reader.Read())
                _vejparsedcpmtransaction = reader.CurrentVEjParsedCPMtransaction;
            reader.Close(false);
        ...
        public static VEjParsedCPMtransaction LoadVEjParsedCPMtransactionByPk(int ej_parsed_cpm_transaction_id)
        {
            return LoadVEjParsedCPMtransaction("ej_parsed_cpm_transaction_id=" + ej_parsed_cpm_transaction_id);
        }

        public static VEjParsedCPMtransaction LoadVEjParsedCPMtransactionByPk(int ej_parsed_cpm_transaction_id, IDbConnection conn)
        {
            return LoadVEjParsedCPMtransaction(" ej_parsed_cpm_transaction_id=" + ej_parsed_cpm_transaction_id, conn);
        }
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (ej_parsed_cpm_transaction_idChanged || trxn_datetimeChanged || terminal_idChanged || seqChanged || account_typeChanged || panChanged || deposit_amountChanged || resultChanged || consumer_message_idChanged || dispute_statusChanged || statusChanged || commentChanged || reject_reasonChanged || processed_tranChanged || atm_idChanged || generated_atChanged || dispense_amountChanged || start_indexChanged || end_indexChanged || task_idChanged || is_eligibleChanged || is_disputed_transactionChanged || host_tsnChanged || account_noChanged || micrChanged || transaction_type_idChanged || is_cardlessChanged || bank_nameChanged || eida_nameChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    qry.Append(@"insert into VEjParsedCPMtransaction(ej_parsed_cpm_transaction_id,trxn_datetime,terminal_id,seq,account_type,pan,deposit_amount,result,consumer_message_id,dispute_status,status,comment,reject_reason,processed_tran,atm_id,generated_at,dispense_amount,start_index,end_index,task_id,is_eligible,is_disputed_transaction,host_tsn,account_no,micr,transaction_type_id,is_cardless,bank_name,eida_name) values(");
                ...
                else
                {
                    if (!(ej_parsed_cpm_transaction_idChanged || trxn_datetimeChanged || terminal_idChanged || seqChanged || account_typeChanged || panChanged || deposit_amountChanged || resultChanged || consumer_message_idChanged || dispute_statusChanged || statusChanged || commentChanged || reject_reasonChanged || processed_tranChanged || atm_idChanged || generated_atChanged || dispense_amountChanged || start_indexChanged || end_indexChanged || task_idChanged || is_eligibleChanged || is_disputed_transactionChanged || host_tsnChanged || account_noChanged || micrChanged || transaction_type_idChanged || is_cardlessChanged || bank_nameChanged || eida_nameChanged))
                        return;
                    qry.Append("UPDATE VEjParsedCPMtransaction set "); if (trxn_datetimeChanged)
                    {
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE VEjParsedCPMtransaction where ej_parsed_cpm_transaction_id= " + ej_parsed_cpm_transaction_id;
        ...
        public static void DeleteVEjParsedCPMtransactions(string where)
        {
            ConnectionFactory.ExecuteQuery("delete VEjParsedCPMtransaction where " + where);
        }
        ...
        #endregion
        public DataTable BulkSave(List<VEjParsedCPMtransaction> dataArray, SqlTransaction dbTrx)
        {
            DataTable dt = new DataTable();
            CreateDataTable(dt);
            AddToDataTable(dataArray, ref dt);
            SqlBulkCopy bulk = new SqlBulkCopy(dbTrx.Connection, SqlBulkCopyOptions.Default, dbTrx);
            bulk.DestinationTableName = "VEjParsedCPMtransaction";
            bulk.WriteToServer(dt); return dt;
        }
        public void CreateDataTable(DataTable dt)
        {
            string[] colNames = Enum.GetNames(typeof(VEjParsedCPMtransaction.Columns));
            for (int i = 0; i < colNames.Length; i++)
            {
                dt.Columns.Add(colNames[i]);
            }
        }
        public void AddToDataTable(List<VEjParsedCPMtransaction> transList, ref DataTable dt)
        {
            foreach (VEjParsedCPMtransaction tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
