...
{
    [Serializable()]
    public class VEjParsedTransactions
    {
        ...
        public VEjParsedTransactions() { }
        public VEjParsedTransactions(int ej_parsed_transactions_id)
        {
        ...
        #region VEjParsedTransactionsReader
        public class VEjParsedTransactionsReader : IEntityReader, IEnumerator, IEnumerable
        {
            IDataReader reader;
            IDbConnection conn;
            VEjParsedTransactions currentVEjParsedTransactions;
            Columns columns;
            bool partialRead = false;
            private VEjParsedTransactionsReader() { }
            /// 
            ///
            ///
            ...
            public VEjParsedTransactionsReader(IDataReader reader, IDbConnection conn)
            {
                this.reader = reader;
                this.conn = conn;
            }
            public VEjParsedTransactionsReader(IDataReader reader, IDbConnection conn, Columns columns)
            {
                this.reader = reader;
                this.conn = conn;
            ...
            {
                get { return currentVEjParsedTransactions; }
            ...
            public bool Read()
            {
                if (reader.Read())
                {
                    currentVEjParsedTransactions = new VEjParsedTransactions();
                    if (partialRead)
                    {
                    ...
                    currentVEjParsedTransactions.isNewEntity = false;
                    return true;
                }
                else
                    return false;
            ...
            public VEjParsedTransactions CurrentVEjParsedTransactions
            {
                get { return currentVEjParsedTransactions; }
            }
        ...
        #region VEjParsedTransactions functions

        public static VEjParsedTransactionsReader ExecuteReader(string where, IDbConnection conn, Columns columns)
        {
            ...
            qry.Append("from VEjParsedTransactions ");
            ...
            return new VEjParsedTransactionsReader(cmd.ExecuteReader(), conn, columns);
        }

        static public VEjParsedTransactionsReader ExecuteReader(string where, Columns columns)
        {
        ...
        /// 
        /// 
        public static VEjParsedTransactionsReader ExecuteReader(string where, IDbConnection conn)
        {
            if (conn.State != ConnectionState.Open)
                conn.Open();
            IDbCommand cmd = conn.CreateCommand();
        ...
        public static VEjParsedTransactions LoadVEjParsedTransactionsByPk(int ej_parsed_transactions_id)
        {
            return LoadVEjParsedTransactions("ej_parsed_transactions_id=" + ej_parsed_transactions_id);
        }

        public static VEjParsedTransactions LoadVEjParsedTransactionsByPk(int ej_parsed_transactions_id, IDbConnection conn)
        {
            return LoadVEjParsedTransactions(" ej_parsed_transactions_id=" + ej_parsed_transactions_id, conn);
        }
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (ej_parsed_transactions_idChanged || tsnChanged || panChanged || trxn_datetimeChanged || amountChanged || notes_dispensed_type1Changed || notes_dispensed_type2Changed || notes_dispensed_type3Changed || notes_dispensed_type4Changed || atm_idChanged || task_idChanged || mstate_idChanged || comment_idChanged || transaction_type_idChanged || available_balanceChanged || processing_datetimeChanged || start_indexChanged || end_indexChanged || statusChanged || donation_amountChanged || transferred_amountChanged || notes_remaining_type1Changed || notes_remaining_type2Changed || notes_remaining_type3Changed || notes_remaining_type4Changed || notes_dispensed_type5Changed || notes_dispensed_type6Changed || notes_dispensed_type7Changed || transaction_start_timeChanged || transaction_end_timeChanged || card_taken_timeChanged || account_typeChanged || resultChanged || consumer_message_idChanged || dispute_statusChanged || terminal_idChanged || is_disputed_transactionChanged || posting_dateChanged || currencyChanged || is_eligibleChanged || networkChanged || notes_rejected_type1Changed || notes_rejected_type2Changed || notes_rejected_type3Changed || notes_rejected_type4Changed || host_tsnChanged || is_cardlessChanged || notes_remaining_type5Changed || notes_remaining_type6Changed || notes_remaining_type7Changed || notes_rejected_type5Changed || notes_rejected_type6Changed || notes_rejected_type7Changed || account_noChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    qry.Append(@"insert into VEjParsedTransactions(ej_parsed_transactions_id,tsn,pan,trxn_datetime,amount,notes_dispensed_type1,notes_dispensed_type2,notes_dispensed_type3,notes_dispensed_type4,atm_id,task_id,mstate_id,comment_id,transaction_type_id,available_balance,processing_datetime,start_index,end_index,status,donation_amount,transferred_amount,notes_remaining_type1,notes_remaining_type2,notes_remaining_type3,notes_remaining_type4,notes_dispensed_type5,notes_dispensed_type6,notes_dispensed_type7,transaction_start_time,transaction_end_time,card_taken_time,account_type,result,consumer_message_id,dispute_status,terminal_id,is_disputed_transaction,posting_date,currency,is_eligible,network,notes_rejected_type1,notes_rejected_type2,notes_rejected_type3,notes_rejected_type4,host_tsn,is_cardless,notes_remaining_type5,notes_remaining_type6,notes_remaining_type7,notes_rejected_type5,notes_rejected_type6,notes_rejected_type7,account_no) values(");
                ...
                else
                {
                    if (!(ej_parsed_transactions_idChanged || tsnChanged || panChanged || trxn_datetimeChanged || amountChanged || notes_dispensed_type1Changed || notes_dispensed_type2Changed || notes_dispensed_type3Changed || notes_dispensed_type4Changed || atm_idChanged || task_idChanged || mstate_idChanged || comment_idChanged || transaction_type_idChanged || available_balanceChanged || processing_datetimeChanged || start_indexChanged || end_indexChanged || statusChanged || donation_amountChanged || transferred_amountChanged || notes_remaining_type1Changed || notes_remaining_type2Changed || notes_remaining_type3Changed || notes_remaining_type4Changed || notes_dispensed_type5Changed || notes_dispensed_type6Changed || notes_dispensed_type7Changed || transaction_start_timeChanged || transaction_end_timeChanged || card_taken_timeChanged || account_typeChanged || resultChanged || consumer_message_idChanged || dispute_statusChanged || terminal_idChanged || is_disputed_transactionChanged || posting_dateChanged || currencyChanged || is_eligibleChanged || networkChanged || notes_rejected_type1Changed || notes_rejected_type2Changed || notes_rejected_type3Changed || notes_rejected_type4Changed || host_tsnChanged || is_cardlessChanged || notes_remaining_type5Changed || notes_remaining_type6Changed || notes_remaining_type7Changed || notes_rejected_type5Changed || notes_rejected_type6Changed || notes_rejected_type7Changed || account_noChanged))
                        return;
                    qry.Append("UPDATE VEjParsedTransactions set "); if (tsnChanged)
                    {
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE VEjParsedTransactions whereej_parsed_transactions_id= " + ej_parsed_transactions_id;
        ...
        public static void DeleteVEjParsedTransactionss(string where)
        {
            ConnectionFactory.ExecuteQuery("delete VEjParsedTransactions where " + where);
        }
        ...
        public enum Columns : ulong
        {
            ...
            notes_rejected_type5 = 1125899906842624,
            notes_rejected_type6 = 2251799813685248,
            notes_rejected_type7 = 4503599627370496,
            account_no = 9007199254740992
        }
        #endregion
        public DataTable BulkSave(List<VEjParsedTransactions> dataArray, SqlTransaction dbTrx)
        {
            DataTable dt = new DataTable();
            CreateDataTable(dt);
            AddToDataTable(dataArray, ref dt);
            SqlBulkCopy bulk = new SqlBulkCopy(dbTrx.Connection, SqlBulkCopyOptions.Default, dbTrx);
            bulk.DestinationTableName = "VEjParsedTransactions";
            bulk.WriteToServer(dt); return dt;
        }
        public void CreateDataTable(DataTable dt)
        {
            string[] colNames = Enum.GetNames(typeof(VEjParsedTransactions.Columns));
            for (int i = 0; i < colNames.Length; i++)
            {
                dt.Columns.Add(colNames[i]);
            }
        }
        public void AddToDataTable(List<VEjParsedTransactions> transList, ref DataTable dt)
        {
            foreach (VEjParsedTransactions tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
