...
{
[Serializable()]
public class ReconciliationTransactionHistory
{
...
public ReconciliationTransactionHistory() { }
public ReconciliationTransactionHistory( int reconciliation_transactions_history_id,int reconciliation_switch_data_id,int reconciliation_host_data_id,int ej_parsed_transactions_id,int parsed_transactions_id,int comparison_type,int user_id,DateTime generated_at,int reconciliation_batch_id,string status ) 
{
...
private ReconciliationTransactionHistory( int reconciliation_transactions_history_id,int reconciliation_switch_data_id,int reconciliation_host_data_id,int ej_parsed_transactions_id,int parsed_transactions_id,int comparison_type,bool? is_reconciled,bool? is_reconciled_manually,int user_id,DateTime generated_at,DateTime? updated_at,string reason,int reconciliation_batch_id,string status,string user_comments )
{
this.reconciliation_transactions_history_id = reconciliation_transactions_history_id;
this.reconciliation_transactions_history_idChanged = true;
this.reconciliation_switch_data_id = reconciliation_switch_data_id;
this.reconciliation_switch_data_idChanged = true;
this.reconciliation_host_data_id = reconciliation_host_data_id;
this.reconciliation_host_data_idChanged = true;
...
#region ReconciliationTransactionsHistoryId
private bool reconciliation_transactions_history_idChanged = false;
private int reconciliation_transactions_history_id;
public int ReconciliationTransactionsHistoryId
{
get { return reconciliation_transactions_history_id; }
set { 
reconciliation_transactions_history_id = value;
reconciliation_transactions_history_idChanged = true;
}
}
private string reconciliation_transactions_history_idDbString
{
get
{
return reconciliation_transactions_history_id.ToString();
}
}
#endregion
...
#region ReconciliationTransactionHistoryReader
public class ReconciliationTransactionHistoryReader:IEntityReader, IEnumerator, IEnumerable 
{
IDataReader reader;
IDbConnection conn;
ReconciliationTransactionHistory currentReconciliationTransactionHistory;
Columns columns;
bool partialRead = false;
private ReconciliationTransactionHistoryReader() { }
/// 
///
///
...
public ReconciliationTransactionHistoryReader(IDataReader reader,IDbConnection conn)
{
this.reader = reader;
this.conn = conn;
}
public ReconciliationTransactionHistoryReader(IDataReader reader, IDbConnection conn, Columns columns)
{
this.reader = reader;
this.conn = conn;
...
{
get { return currentReconciliationTransactionHistory; }
...
if (reader.Read())
{
currentReconciliationTransactionHistory = new ReconciliationTransactionHistory();
if (partialRead)
{ if ((columns & Columns.reconciliation_transactions_history_id) == Columns.reconciliation_transactions_history_id && reader["reconciliation_transactions_history_id"]!=DBNull.Value)
currentReconciliationTransactionHistory.reconciliation_transactions_history_id =(int) reader["reconciliation_transactions_history_id"]; 
...
currentReconciliationTransactionHistory.reconciliation_switch_data_id =(int) reader["reconciliation_switch_data_id"]; 
...
currentReconciliationTransactionHistory.reconciliation_host_data_id =(int) reader["reconciliation_host_data_id"]; 
...
currentReconciliationTransactionHistory.ej_parsed_transactions_id =(int) reader["ej_parsed_transactions_id"]; 
if ((columns & Columns.parsed_transactions_id) == Columns.parsed_transactions_id && reader["parsed_transactions_id"]!=DBNull.Value)
currentReconciliationTransactionHistory.parsed_transactions_id =(int) reader["parsed_transactions_id"]; 
if ((columns & Columns.comparison_type) == Columns.comparison_type && reader["comparison_type"]!=DBNull.Value)
currentReconciliationTransactionHistory.comparison_type =(int) reader["comparison_type"]; 
if ((columns & Columns.is_reconciled) == Columns.is_reconciled && reader["is_reconciled"]!=DBNull.Value)
currentReconciliationTransactionHistory.is_reconciled =(bool?) reader["is_reconciled"]; 
if ((columns & Columns.is_reconciled_manually) == Columns.is_reconciled_manually && reader["is_reconciled_manually"]!=DBNull.Value)
currentReconciliationTransactionHistory.is_reconciled_manually =(bool?) reader["is_reconciled_manually"]; 
if ((columns & Columns.user_id) == Columns.user_id && reader["user_id"]!=DBNull.Value)
currentReconciliationTransactionHistory.user_id =(int) reader["user_id"]; 
if ((columns & Columns.generated_at) == Columns.generated_at && reader["generated_at"]!=DBNull.Value)
currentReconciliationTransactionHistory.generated_at =(DateTime) reader["generated_at"]; 
if ((columns & Columns.updated_at) == Columns.updated_at && reader["updated_at"]!=DBNull.Value)
currentReconciliationTransactionHistory.updated_at =(DateTime?) reader["updated_at"]; 
if ((columns & Columns.reason) == Columns.reason && reader["reason"]!=DBNull.Value)
currentReconciliationTransactionHistory.reason =(string) reader["reason"]; 
if ((columns & Columns.reconciliation_batch_id) == Columns.reconciliation_batch_id && reader["reconciliation_batch_id"]!=DBNull.Value)
currentReconciliationTransactionHistory.reconciliation_batch_id =(int) reader["reconciliation_batch_id"]; 
if ((columns & Columns.status) == Columns.status && reader["status"]!=DBNull.Value)
currentReconciliationTransactionHistory.status =(string) reader["status"]; 
if ((columns & Columns.user_comments) == Columns.user_comments && reader["user_comments"]!=DBNull.Value)
currentReconciliationTransactionHistory.user_comments =(string) reader["user_comments"]; 

} else
{
if (reader["reconciliation_transactions_history_id"] != DBNull.Value)
currentReconciliationTransactionHistory.reconciliation_transactions_history_id = (int) reader["reconciliation_transactions_history_id"]; 
...
currentReconciliationTransactionHistory.isNewEntity = false;
return true;
}
else
return false;
...
public ReconciliationTransactionHistory CurrentReconciliationTransactionHistory
{
get{ return currentReconciliationTransactionHistory; }
}
...
#region ReconciliationTransactionHistory functions

public static ReconciliationTransactionHistoryReader ExecuteReader(string where, IDbConnection conn, Columns columns)
{
StringBuilder qry = new StringBuilder(200);
qry.Append("select ");
if (Columns.reconciliation_transactions_history_id == (Columns.reconciliation_transactions_history_id & columns))
qry.Append("reconciliation_transactions_history_id,");
...
qry.Append("from Reconciliation_transaction_history ");
...
return new ReconciliationTransactionHistoryReader(cmd.ExecuteReader(), conn, columns);
}

static public ReconciliationTransactionHistoryReader ExecuteReader(string where,Columns columns)
{
...
/// 
/// 
public static ReconciliationTransactionHistoryReader ExecuteReader(string where,IDbConnection conn)
{
if (conn.State != ConnectionState.Open)
conn.Open();
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
cmd.ExecuteNonQuery();
cmd.CommandText = "Select reconciliation_transactions_history_id,reconciliation_switch_data_id,reconciliation_host_data_id,ej_parsed_transactions_id,parsed_transactions_id,comparison_type,is_reconciled,is_reconciled_manually,user_id,generated_at,updated_at,reason,reconciliation_batch_id,status,user_comments from Reconciliation_transaction_history ";
if (where != null && where.Trim().Length > 0)
cmd.CommandText = string.Format("{0} where {1}", cmd.CommandText, where);

return new ReconciliationTransactionHistoryReader(cmd.ExecuteReader(), conn);
}

static public ReconciliationTransactionHistoryReader ExecuteReader(string where)
{
return ExecuteReader(where, ConnectionFactory.GetNewConnection());
}

public static ReconciliationTransactionHistory LoadReconciliationTransactionHistory(string where)
{
ReconciliationTransactionHistoryReader reader = ReconciliationTransactionHistory.ExecuteReader(where);
ReconciliationTransactionHistory _reconciliationtransactionhistory = null;
if (reader.Read())
_reconciliationtransactionhistory = reader.CurrentReconciliationTransactionHistory;
reader.Close();
return _reconciliationtransactionhistory;
}

public static ReconciliationTransactionHistory LoadReconciliationTransactionHistory(string where, IDbConnection conn)
{
ReconciliationTransactionHistoryReader reader = ReconciliationTransactionHistory.ExecuteReader(where, conn);
ReconciliationTransactionHistory _reconciliationtransactionhistory = null;
if (reader.Read())
_reconciliationtransactionhistory = reader.CurrentReconciliationTransactionHistory;
reader.Close(false);
...
public static ReconciliationTransactionHistory LoadReconciliationTransactionHistoryByPk( int reconciliation_transactions_history_id )
{
return LoadReconciliationTransactionHistory( " reconciliation_transactions_history_id="+reconciliation_transactions_history_id );
}

public static ReconciliationTransactionHistory LoadReconciliationTransactionHistoryByPk( int reconciliation_transactions_history_id , IDbConnection conn)
{
return LoadReconciliationTransactionHistory(" reconciliation_transactions_history_id="+reconciliation_transactions_history_id , conn);
}

public void Save()
{
if (reconciliation_transactions_history_idChanged || reconciliation_switch_data_idChanged || reconciliation_host_data_idChanged || ej_parsed_transactions_idChanged || parsed_transactions_idChanged || comparison_typeChanged || is_reconciledChanged || is_reconciled_manuallyChanged || user_idChanged || generated_atChanged || updated_atChanged || reasonChanged || reconciliation_batch_idChanged || statusChanged || user_commentsChanged )
...
if (this.isNewEntity)
{
qry.Append(@"insert into Reconciliation_transaction_history( reconciliation_transactions_history_id,reconciliation_switch_data_id,reconciliation_host_data_id,ej_parsed_transactions_id,parsed_transactions_id,comparison_type,is_reconciled,is_reconciled_manually,user_id,generated_at,updated_at,reason,reconciliation_batch_id,status,user_comments ) values(");
lock (ConnectionFactory.connectionString) { this.reconciliation_transactions_history_id = ConnectionFactory.GetNextId();
qry.Append(this.reconciliation_transactions_history_id);
} qry.Append(",");
...
if (!(reconciliation_transactions_history_idChanged || reconciliation_switch_data_idChanged || reconciliation_host_data_idChanged || ej_parsed_transactions_idChanged || parsed_transactions_idChanged || comparison_typeChanged || is_reconciledChanged || is_reconciled_manuallyChanged || user_idChanged || generated_atChanged || updated_atChanged || reasonChanged || reconciliation_batch_idChanged || statusChanged || user_commentsChanged ))
return;
qry.Append("UPDATE Reconciliation_transaction_history set "); if ( reconciliation_switch_data_idChanged )
{
...
qry.Replace(',', ' ', qry.Length - 1,1);
qry.Append(" where ");
qry.Append("reconciliation_transactions_history_id = "+reconciliation_transactions_history_idDbString);
}
...
{
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "DELETE Reconciliation_transaction_history where reconciliation_transactions_history_id = "+ reconciliation_transactions_history_id;
...
public static void DeleteReconciliationTransactionHistorys(string where)
{
ConnectionFactory.ExecuteQuery("delete Reconciliation_transaction_history where " + where);
}
...
{
reconciliation_transactions_history_id= 1,
...
#endregion
public void BulkSave(List<ReconciliationTransactionHistory> dataArray,SqlTransaction dbTrx)
{
DataTable dt = new DataTable();
CreateDataTable(dt);
AddToDataTable(dataArray, ref dt);
SqlBulkCopy bulk = new SqlBulkCopy(dbTrx.Connection, SqlBulkCopyOptions.Default, dbTrx);
bulk.DestinationTableName = "Reconciliation_transaction_history";
bulk.WriteToServer(dt);
}
public void CreateDataTable(DataTable dt)
{
string[] colNames = Enum.GetNames(typeof(ReconciliationTransactionHistory.Columns));
for (int i = 0; i < colNames.Length; i++)
{
dt.Columns.Add(colNames[i]);
}
}
public void AddToDataTable(List <ReconciliationTransactionHistory> transList,ref DataTable dt)
{
foreach (ReconciliationTransactionHistory tran in transList)
{
DataRow Row;
Row = dt.NewRow();
Row["reconciliation_transactions_history_id"] =ConnectionFactory.GetNextId();
