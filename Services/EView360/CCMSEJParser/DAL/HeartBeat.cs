...
public HeartBeat() { }
public HeartBeat( int heart_beat_id,int atm_id,DateTime heart_beat_received_at ) 
{
this.atm_id = atm_id;
...
public HeartBeat( int atm_id,DateTime heart_beat_received_at,bool? is_service_manager )
{
this.atm_id = atm_id;
this.atm_idChanged = true;
this.heart_beat_received_at = heart_beat_received_at;
...
private HeartBeat( int heart_beat_id,int atm_id,DateTime heart_beat_received_at,bool? is_service_manager )
{
...
currentHeartBeat.heart_beat_id =(int) reader["heart_beat_id"]; 
if ((columns & Columns.atm_id) == Columns.atm_id && reader["atm_id"]!=DBNull.Value)
currentHeartBeat.atm_id =(int) reader["atm_id"]; 
...
public static HeartBeat LoadHeartBeatByPk( int heart_beat_id )
{
return LoadHeartBeat( " heart_beat_id="+heart_beat_id );
}

public static HeartBeat LoadHeartBeatByPk( int heart_beat_id , IDbConnection conn)
{
...
lock (ConnectionFactory.connectionString) { this.heart_beat_id = ConnectionFactory.GetNextId();
...
{
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "DELETE Heart_beat where heart_beat_id = "+ heart_beat_id;
...
{
ConnectionFactory.ExecuteQuery("delete Heart_beat where " + where);
}
...
heart_beat_id= 1,
atm_id= 2,
heart_beat_received_at= 4,
is_service_manager= 8
}
#endregion
public void BulkSave(List<HeartBeat> dataArray,SqlTransaction dbTrx)
{
...
DataRow Row;
Row = dt.NewRow();
Row["heart_beat_id"] =ConnectionFactory.GetNextId();
