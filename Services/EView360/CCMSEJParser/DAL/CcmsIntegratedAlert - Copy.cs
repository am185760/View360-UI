...
namespace Avanza.CCMS.DAL
{
    ...
    public class CcmsIntegratedAlert
    {
        ...
        private CcmsIntegratedAlert(long? id, long? alert_type_id, string alert_type, long? entity_id, string entity_type, string alert_level, string alert_status, DateTime? generated_at, string alert_text, DateTime? expiration_time, int? generate_retry_remaining, int? resolve_retry_remaining, DateTime? last_invoked_at, string module_type, int? ftp_file_info_id, string failure_reason, string alert_resolution_text, long? organization_id, DateTime? resolved_at, bool generate_notification_sent, bool resolve_notification_sent, int? atm_id, int? atm_alert_id)
        {
            this.id = id;
            this.idChanged = true;
            this.alert_type_id = alert_type_id;
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.id) == Columns.id && reader["id"] != DBNull.Value)
                            currentCcmsIntegratedAlert.id = (long?)reader["id"];
                        if ((columns & Columns.alert_type_id) == Columns.alert_type_id && reader["alert_type_id"] != DBNull.Value)
                            currentCcmsIntegratedAlert.alert_type_id = (long?)reader["alert_type_id"];
                        ...
                        if ((columns & Columns.entity_id) == Columns.entity_id && reader["entity_id"] != DBNull.Value)
                            currentCcmsIntegratedAlert.entity_id = (long?)reader["entity_id"];
                        ...
                        if ((columns & Columns.organization_id) == Columns.organization_id && reader["organization_id"] != DBNull.Value)
                            currentCcmsIntegratedAlert.organization_id = (long?)reader["organization_id"];
                    ...
                    else
                    {
                        ...
                        if (reader["alert_type_id"] != DBNull.Value)
                            currentCcmsIntegratedAlert.alert_type_id = long.Parse(reader["alert_type_id"].ToString());
                        if (reader["alert_type"] != DBNull.Value)
                            currentCcmsIntegratedAlert.alert_type = (string)reader["alert_type"];
                        if (reader["entity_id"] != DBNull.Value)
                            currentCcmsIntegratedAlert.entity_id = long.Parse(reader["entity_id"].ToString());
                        ...
                        if (reader["organization_id"] != DBNull.Value)
                            currentCcmsIntegratedAlert.organization_id = long.Parse(reader["organization_id"].ToString());
        ...
        public void Delete(SqlTransaction trxn)
        {
            Delete(ConnectionFactory.GetNewConnection(), trxn);
        }
