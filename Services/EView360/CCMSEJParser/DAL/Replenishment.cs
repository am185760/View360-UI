...
namespace Avanza.CCMS.DAL
{
    ...
    public class Replenishment
    {
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        ...
                        if ((columns & Columns.is_bill_dispenser) == Columns.is_bill_dispenser && reader["is_bill_dispenser"] != DBNull.Value)
                            currentReplenishment.is_bill_dispenser = (bool?)reader["is_bill_dispenser"];
        ...
        public static ReplenishmentReader ExecuteReader(string where, IDbConnection conn)
        {
            ...
            cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
            cmd.ExecuteNonQuery();
            cmd.CommandText = "Select atm_id,cash_added1,cash_added2,cash_added3,cash_added4,cash_added5,cash_added6,cash_added7,rep_datetime,rep_status,replenishment_id,task_id,cash_order_id,is_swap,generated_at,is_updated,modified_by,modified_datetime,generated_by,reason,rep_amount,last_tsn,is_bill_dispenser from Replenishment ";
            if (where != null && where.Trim().Length > 0)
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (atm_idChanged || cash_added1Changed || cash_added2Changed || cash_added3Changed || cash_added4Changed || cash_added5Changed || cash_added6Changed || cash_added7Changed || rep_datetimeChanged || rep_statusChanged || replenishment_idChanged || task_idChanged || cash_order_idChanged || is_swapChanged || generated_atChanged || is_updatedChanged || modified_byChanged || modified_datetimeChanged || generated_byChanged || reasonChanged || rep_amountChanged || last_tsnChanged || is_bill_dispenserChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    qry.Append(@"insert into Replenishment(atm_id,cash_added1,cash_added2,cash_added3,cash_added4,cash_added5,cash_added6,cash_added7,rep_datetime,rep_status,replenishment_id,task_id,cash_order_id,is_swap,generated_at,is_updated,modified_by,modified_datetime,generated_by,reason,rep_amount,last_tsn,is_bill_dispenser) values(");
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE Replenishment wherereplenishment_id= " + replenishment_id;
