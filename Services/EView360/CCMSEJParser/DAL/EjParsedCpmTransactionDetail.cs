...
namespace Avanza.CCMS.DAL
{
    ...
    public class EjParsedCpmTransactionDetail
    {
        ...
        public EjParsedCpmTransactionDetail(int ej_parsed_cpm_transaction_id, decimal check_amount)
        {
        ...
        private EjParsedCpmTransactionDetail(int ej_parsed_cpm_transaction_detail_id, int ej_parsed_cpm_transaction_id, decimal check_amount)
        {
        ...
        private int ej_parsed_cpm_transaction_detail_id;
        public int EjParsedCpmTransactionDetailId
        {
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.ej_parsed_cpm_transaction_detail_id) == Columns.ej_parsed_cpm_transaction_detail_id && reader["ej_parsed_cpm_transaction_detail_id"] != DBNull.Value)
                            currentEjParsedCpmTransactionDetail.ej_parsed_cpm_transaction_detail_id = (int)reader["ej_parsed_cpm_transaction_detail_id"];
                        if ((columns & Columns.ej_parsed_cpm_transaction_id) == Columns.ej_parsed_cpm_transaction_id && reader["ej_parsed_cpm_transaction_id"] != DBNull.Value)
                            currentEjParsedCpmTransactionDetail.ej_parsed_cpm_transaction_id = (int)reader["ej_parsed_cpm_transaction_id"];
        ...
        public static EjParsedCpmTransactionDetailReader ExecuteReader(string where, IDbConnection conn)
        {
            ...
            cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
            cmd.ExecuteNonQuery();
            cmd.CommandText = "Select ej_parsed_cpm_transaction_detail_id,ej_parsed_cpm_transaction_id,check_amount from Ej_parsed_cpm_transaction_detail ";
            if (where != null && where.Trim().Length > 0)
        ...
        public static EjParsedCpmTransactionDetail LoadEjParsedCpmTransactionDetailByPk(int ej_parsed_cpm_transaction_detail_id)
        {
            return LoadEjParsedCpmTransactionDetail(" ej_parsed_cpm_transaction_detail_id=" + ej_parsed_cpm_transaction_detail_id);
        }

        public static EjParsedCpmTransactionDetail LoadEjParsedCpmTransactionDetailByPk(int ej_parsed_cpm_transaction_detail_id, IDbConnection conn)
        {
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (ej_parsed_cpm_transaction_detail_idChanged || ej_parsed_cpm_transaction_idChanged || check_amountChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    ...
                    {
                        this.ej_parsed_cpm_transaction_detail_id = ConnectionFactory.GetNextId();
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE Ej_parsed_cpm_transaction_detail where ej_parsed_cpm_transaction_detail_id = " + ej_parsed_cpm_transaction_detail_id;
        ...
        {
            ConnectionFactory.ExecuteQuery("delete Ej_parsed_cpm_transaction_detail where " + where);
        }
        ...
        {
            ej_parsed_cpm_transaction_detail_id = 1,
            ej_parsed_cpm_transaction_id = 2,
            check_amount = 4
        }
        #endregion
        public void BulkSave(List<EjParsedCpmTransactionDetail> dataArray, SqlTransaction dbTrx)
        {
        ...
        public void AddToDataTable(List<EjParsedCpmTransactionDetail> transList, ref DataTable dt)
        {
            foreach (EjParsedCpmTransactionDetail tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
                Row["ej_parsed_cpm_transaction_detail_id"] = ConnectionFactory.GetNextId();
