...
namespace Avanza.CCMS.DAL
{
    ...
    public class Task
    {
        ...
        public Task() { }
        public Task(int task_id, int bytes_transferred, int aTM_id, DateTime creation_time, string status, int created_by, byte retry_Remaining, int task_type_id)
        {
        ...
        private Task(int task_id, bool? parsed, int bytes_transferred, string file_path_at_ATM, int aTM_id, int? file_type_id, DateTime creation_time, DateTime? download_time, DateTime? upload_time, DateTime? end_time, string status, int? zipped_file_size, int created_by, int? unZipped_file_size, DateTime? last_invoked, byte retry_Remaining, string failure_reason, string server_filepath, int task_type_id, int? cash_order_id, int? downloading_schedule_id, int? failed_to_parse_count, string archive_file_path_at_atm, string task_info)
        {
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.task_id) == Columns.task_id && reader["task_id"] != DBNull.Value)
                            currentTask.task_id = (int)reader["task_id"];
                        ...
                        if ((columns & Columns.created_by) == Columns.created_by && reader["created_by"] != DBNull.Value)
                            currentTask.created_by = (int)reader["created_by"];
        ...
        static public TaskReader ExecuteReader(string where, Columns columns)
        {
        ...
        static public TaskReader ExecuteReader(string where)
        {
            return ExecuteReader(where, ConnectionFactory.GetNewConnection());
        }

        public static Task LoadTask(string where)
        {
            TaskReader reader = Task.ExecuteReader(where);
            Task _task = null;
            if (reader.Read())
        ...
        public static Task LoadTaskByPk(int task_id)
        {
            return LoadTask(" task_id=" + task_id);
        }

        public static Task LoadTaskByPk(int task_id, IDbConnection conn)
        {
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            ...
            if (task_idChanged || parsedChanged || bytes_transferredChanged || file_path_at_ATMChanged || aTM_idChanged || file_type_idChanged || creation_timeChanged || download_timeChanged || upload_timeChanged || end_timeChanged || statusChanged || zipped_file_sizeChanged || created_byChanged || unZipped_file_sizeChanged || last_invokedChanged || retry_RemainingChanged || failure_reasonChanged || server_filepathChanged || task_type_idChanged || cash_order_idChanged || downloading_schedule_idChanged || failed_to_parse_countChanged || archive_file_path_at_atmChanged || task_infoChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    ...
                    {
                        this.task_id = ConnectionFactory.GetNextId();
        ...
        public static void DeleteTasks(string where)
        {
            ConnectionFactory.ExecuteQuery("delete Task where " + where);
        }
        ...
        #endregion
        public void BulkSave(List<Task> dataArray, SqlTransaction dbTrx)
        {
        ...
        public void AddToDataTable(List<Task> transList, ref DataTable dt)
        {
            foreach (Task tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
                Row["task_id"] = ConnectionFactory.GetNextId();
