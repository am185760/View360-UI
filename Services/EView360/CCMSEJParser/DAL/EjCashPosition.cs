...
public EjCashPosition() { }
public EjCashPosition( int ej_cash_position_id,int atm_id,int task_id,DateTime last_trxn_at ) 
{
this.atm_id = atm_id;
...
private EjCashPosition( int ej_cash_position_id,int atm_id,int? cassette1_notes,int? cassette2_notes,int? cassette3_notes,int? cassette4_notes,int? cassette5_notes,int? cassette6_notes,int? cassette7_notes,int task_id,DateTime last_trxn_at,int? purge_cassette1_notes,int? purge_cassette2_notes,int? purge_cassette3_notes,int? purge_cassette4_notes,int? purge_cassette5_notes,int? purge_cassette6_notes,int? purge_cassette7_notes )
{
this.ej_cash_position_id = ej_cash_position_id;
this.ej_cash_position_idChanged = true;
this.atm_id = atm_id;
this.atm_idChanged = true;
this.cassette1_notes = cassette1_notes;
this.cassette1_notesChanged = true;
...
#region EjCashPositionId
private bool ej_cash_position_idChanged = false;
private int ej_cash_position_id;
public int EjCashPositionId
{
get { return ej_cash_position_id; }
set { 
ej_cash_position_id = value;
ej_cash_position_idChanged = true;
}
}
private string ej_cash_position_idDbString
{
get
{
return ej_cash_position_id.ToString();
}
}
#endregion
#region AtmId
...
#region EjCashPositionReader
public class EjCashPositionReader:IEntityReader, IEnumerator, IEnumerable 
{
IDataReader reader;
IDbConnection conn;
EjCashPosition currentEjCashPosition;
Columns columns;
bool partialRead = false;
private EjCashPositionReader() { }
/// 
///
///
...
public EjCashPositionReader(IDataReader reader,IDbConnection conn)
{
this.reader = reader;
this.conn = conn;
}
public EjCashPositionReader(IDataReader reader, IDbConnection conn, Columns columns)
{
this.reader = reader;
this.conn = conn;
...
{
get { return currentEjCashPosition; }
...
currentEjCashPosition.isNewEntity = false;
return true;
}
else
return false;
...
public EjCashPosition CurrentEjCashPosition
{
get{ return currentEjCashPosition; }
}
...
#region EjCashPosition functions

public static EjCashPositionReader ExecuteReader(string where, IDbConnection conn, Columns columns)
{
StringBuilder qry = new StringBuilder(200);
qry.Append("select ");
if (Columns.ej_cash_position_id == (Columns.ej_cash_position_id & columns))
qry.Append("ej_cash_position_id,");
...
qry.Append("from Ej_cash_position ");
...
return new EjCashPositionReader(cmd.ExecuteReader(), conn, columns);
}

static public EjCashPositionReader ExecuteReader(string where,Columns columns)
{
...
/// 
/// 
public static EjCashPositionReader ExecuteReader(string where,IDbConnection conn)
{
if (conn.State != ConnectionState.Open)
conn.Open();
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
cmd.ExecuteNonQuery();
cmd.CommandText = "Select ej_cash_position_id,atm_id,cassette1_notes,cassette2_notes,cassette3_notes,cassette4_notes,cassette5_notes,cassette6_notes,cassette7_notes,task_id,last_trxn_at,purge_cassette1_notes,purge_cassette2_notes,purge_cassette3_notes,purge_cassette4_notes,purge_cassette5_notes,purge_cassette6_notes,purge_cassette7_notes from Ej_cash_position ";
if (where != null && where.Trim().Length > 0)
cmd.CommandText = string.Format("{0} where {1}", cmd.CommandText, where);

return new EjCashPositionReader(cmd.ExecuteReader(), conn);
}

static public EjCashPositionReader ExecuteReader(string where)
{
return ExecuteReader(where, ConnectionFactory.GetNewConnection());
}

public static EjCashPosition LoadEjCashPosition(string where)
{
EjCashPositionReader reader = EjCashPosition.ExecuteReader(where);
EjCashPosition _ejcashposition = null;
if (reader.Read())
_ejcashposition = reader.CurrentEjCashPosition;
reader.Close();
return _ejcashposition;
}

public static EjCashPosition LoadEjCashPosition(string where, IDbConnection conn)
{
EjCashPositionReader reader = EjCashPosition.ExecuteReader(where, conn);
EjCashPosition _ejcashposition = null;
if (reader.Read())
_ejcashposition = reader.CurrentEjCashPosition;
reader.Close(false);
...
if (!(ej_cash_position_idChanged || atm_idChanged || cassette1_notesChanged || cassette2_notesChanged || cassette3_notesChanged || cassette4_notesChanged || cassette5_notesChanged || cassette6_notesChanged || cassette7_notesChanged || task_idChanged || last_trxn_atChanged || purge_cassette1_notesChanged || purge_cassette2_notesChanged || purge_cassette3_notesChanged || purge_cassette4_notesChanged || purge_cassette5_notesChanged || purge_cassette6_notesChanged || purge_cassette7_notesChanged ))
return;
qry.Append("UPDATE Ej_cash_position set "); if ( atm_idChanged )
{
...
qry.Replace(',', ' ', qry.Length - 1,1);
qry.Append(" where ");
qry.Append("ej_cash_position_id = "+ej_cash_position_idDbString);
}
...
{
IDbCommand cmd = conn.CreateCommand();
cmd.CommandText = "DELETE Ej_cash_position where ej_cash_position_id = "+ ej_cash_position_id;
...
public static void DeleteEjCashPositions(string where)
{
ConnectionFactory.ExecuteQuery("delete Ej_cash_position where " + where);
}
...
#endregion
public void BulkSave(List<EjCashPosition> dataArray,SqlTransaction dbTrx)
{
DataTable dt = new DataTable();
CreateDataTable(dt);
AddToDataTable(dataArray, ref dt);
SqlBulkCopy bulk = new SqlBulkCopy(dbTrx.Connection, SqlBulkCopyOptions.Default, dbTrx);
bulk.DestinationTableName = "Ej_cash_position";
bulk.WriteToServer(dt);
}
public void CreateDataTable(DataTable dt)
{
string[] colNames = Enum.GetNames(typeof(EjCashPosition.Columns));
for (int i = 0; i < colNames.Length; i++)
{
dt.Columns.Add(colNames[i]);
}
}
public void AddToDataTable(List <EjCashPosition> transList,ref DataTable dt)
{
foreach (EjCashPosition tran in transList)
{
DataRow Row;
Row = dt.NewRow();
Row["ej_cash_position_id"] =ConnectionFactory.GetNextId();
