...
namespace DataRequestor
{
    ...
    public class XmlLogWriter
    {
        ...
        {
            // Local variable to store current file name for comparison
            fileName = logFileName;

            lock (logfilelock)
            {
                // 1. Check if the writer is already initialized
                if (writer != null)
                {
                    FileStream currentStream = writer.BaseStream as FileStream;

                    // If the open file matches the requested name, we are already good to go.
                    if (currentStream != null && string.Equals(currentStream.Name, logFileName, StringComparison.OrdinalIgnoreCase))
                    {
                        return;
                    }

                    // If the name is different (e.g., the date changed), close the old one.
                    ShutDown();
                }

                // 2. Open the file in Append mode with ReadWrite sharing.
                // FileShare.ReadWrite is CRITICAL to prevent the "process cannot access" error.
                ...
                writer.AutoFlush = true; // Ensures logs are written immediately

                // 3. If this is a brand new file (Length is 0), write the initial header line.
                if (fs.Length == 0)
                {
                    string header = DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss").PadRight(paddingLength) +
                                    TraceLevel.Info.ToString().PadRight(10) +
        ...
        public static void InitXmlLogWriter2(string logFileName)
        {
            fileName = logFileName;
            lock (logfilelock)
            {
