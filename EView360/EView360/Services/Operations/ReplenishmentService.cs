...
namespace EView360.Services.Operations
{
    public class ReplenishmentService
    {
        ...
        public ReplenishmentService(HttpClient httpClient, IOptions<ApiUrl> apiUrl, ILogger<ReplenishmentService> logger, INotificationService notificationService, ATMTreeViewRepository treeService, NoteSetTypeService noteSetTypeService, AtmService atmService, ReplenishmentServiceMW service, CommonServices common)
        {
        ...
        public async Task<ReplenishmentViewModel> GetAtmNoteSetType(long atmId)
        {
            ReplenishmentViewModel noteSetType = new();
            try
            {
                noteSetType = await noteSetTypeService.GetAtmNoteSetType(atmId);
            }
            catch (Exception ex)
            {
                _logger.LogError($"Exception at GetAtmNoteSetType as: {ex.Message}");
                await RenderErrorBox("Error", ex.Message);
            }
            return noteSetType;
        }

        public async Task<BaseModel> PostReplenishment(ReplenishmentViewModel postRep)
        {
            try
            {
                //var jsonContent = JsonConvert.SerializeObject(postRep);
                //HttpContent content = new StringContent(jsonContent, Encoding.UTF8, "application/json");
                //HttpResponseMessage response = await client.PutAsync($"{BaseUrl}PostReplenishment", content);
                //string responseBody = await response.Content.ReadAsStringAsync();
                //if (response.IsSuccessStatusCode && !string.IsNullOrEmpty(responseBody))
                //{
                //    var responseModel = JsonConvert.DeserializeObject<BaseModel>(responseBody);
                var responseModel = service.PostReplenishment(postRep);

                if (responseModel.IsSuccess)
                {
                    await RenderSuccessBox("Success", "Replenishment successfully added");
                }
                else
                {
                    _logger.LogError($"API error at Replenishment, PostReplenishment: {responseModel.Message}");
                    await RenderErrorBox("Error", responseModel.Message);
                }
                ...
                //else
                //{
                //    _logger.LogError($"API error at Replenishment, PostReplenishment: {responseBody}");
                //    await RenderErrorBox("Error", responseBody);
                //}
            }
            catch (Exception ex)
            {
                _logger.LogError($"Exception at PostReplenishment: {ex.Message}");
                await RenderErrorBox("Error", ex.Message);
            }
