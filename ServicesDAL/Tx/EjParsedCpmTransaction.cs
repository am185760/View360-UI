...
namespace ServicesDAL
{
    ...
    public class EjParsedCpmTransaction
    {
        ...
        public EjParsedCpmTransaction(long ej_parsed_cpm_transaction_id, DateTime trxn_datetime, long atm_id, DateTime generated_at, int start_index, int end_index, long task_id, bool is_eligible)
        {
        ...
        public EjParsedCpmTransaction(DateTime trxn_datetime, string terminal_id, string seq, string account_type, string pan, decimal? deposit_amount, string result, string consumer_message_id, string dispute_status, string status, string comment, string reject_reason, string processed_tran, long atm_id, DateTime generated_at, decimal? dispense_amount, int start_index, int end_index, long task_id, bool is_eligible, bool? is_disputed_transaction, string host_tsn, string account_no, string micr, long? transaction_type_id, string network, bool? is_cardless, string bank_name)
        {
            this.trxn_datetime = trxn_datetime;
            this.trxn_datetimeChanged = true;
        ...
        private EjParsedCpmTransaction(long ej_parsed_cpm_transaction_id, DateTime trxn_datetime, string terminal_id, string seq, string account_type, string pan, decimal? deposit_amount, string result, string consumer_message_id, string dispute_status, string status, string comment, string reject_reason, string processed_tran, long atm_id, DateTime generated_at, decimal? dispense_amount, int start_index, int end_index, long task_id, bool is_eligible, bool? is_disputed_transaction, string host_tsn, string account_no, string micr, long? transaction_type_id, string network, bool? is_cardless, string bank_name)
        {
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.ej_parsed_cpm_transaction_id) == Columns.ej_parsed_cpm_transaction_id && reader["ej_parsed_cpm_transaction_id"] != DBNull.Value)
                            currentEjParsedCpmTransaction.ej_parsed_cpm_transaction_id = (long)reader["ej_parsed_cpm_transaction_id"];
                        ...
                        if ((columns & Columns.atm_id) == Columns.atm_id && reader["atm_id"] != DBNull.Value)
                            currentEjParsedCpmTransaction.atm_id = (long)reader["atm_id"];
                        ...
                        if ((columns & Columns.task_id) == Columns.task_id && reader["task_id"] != DBNull.Value)
                            currentEjParsedCpmTransaction.task_id = (long)reader["task_id"];
                        ...
                        if ((columns & Columns.transaction_type_id) == Columns.transaction_type_id && reader["transaction_type_id"] != DBNull.Value)
                            currentEjParsedCpmTransaction.transaction_type_id = (long?)reader["transaction_type_id"];
                        if ((columns & Columns.Network) == Columns.Network && reader["Network"] != DBNull.Value)
                            currentEjParsedCpmTransaction.network = (string)reader["Network"];
        ...
        public static EjParsedCpmTransactionReader ExecuteReader(string where, IDbConnection conn)
        {
            ...
            cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
            cmd.ExecuteNonQuery();
            cmd.CommandText = "Selectej_parsed_cpm_transaction_id,trxn_datetime,terminal_id,seq,account_type,pan,deposit_amount,result,consumer_message_id,dispute_status,status,comment,reject_reason,processed_tran,atm_id,generated_at,dispense_amount,start_index,end_index,task_id,is_eligible,is_disputed_transaction,host_tsn,account_no,micr,transaction_type_id,Network,is_cardless,bank_namefrom Ej_parsed_cpm_transaction ";
            if (where != null && where.Trim().Length > 0)
        ...
        public static EjParsedCpmTransaction LoadEjParsedCpmTransactionByPk(long ej_parsed_cpm_transaction_id, DateTime trxn_datetime)
        {
            return LoadEjParsedCpmTransaction("ej_parsed_cpm_transaction_id=" + ej_parsed_cpm_transaction_id + " and trxn_datetime=Convert(datetime,'" + trxn_datetime.ToString("yyyy-MM-dd HH:mm:ss.fff") + "',121)");
        }

        public static EjParsedCpmTransaction LoadEjParsedCpmTransactionByPk(long ej_parsed_cpm_transaction_id, DateTime trxn_datetime, IDbConnection conn)
        {
            return LoadEjParsedCpmTransaction(" ej_parsed_cpm_transaction_id=" + ej_parsed_cpm_transaction_id + " and trxn_datetime=Convert(datetime,'" + trxn_datetime.ToString("yyyy-MM-dd HH:mm:ss.fff") + "',121)", conn);
        }

        public void Save()
        {
            if (ej_parsed_cpm_transaction_idChanged || trxn_datetimeChanged || terminal_idChanged || seqChanged || account_typeChanged || panChanged || deposit_amountChanged || resultChanged || consumer_message_idChanged || dispute_statusChanged || statusChanged || commentChanged || reject_reasonChanged || processed_tranChanged || atm_idChanged || generated_atChanged || dispense_amountChanged || start_indexChanged || end_indexChanged || task_idChanged || is_eligibleChanged || is_disputed_transactionChanged || host_tsnChanged || account_noChanged || micrChanged || transaction_type_idChanged || networkChanged || is_cardlessChanged || bank_nameChanged)
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (ej_parsed_cpm_transaction_idChanged || trxn_datetimeChanged || terminal_idChanged || seqChanged || account_typeChanged || panChanged || deposit_amountChanged || resultChanged || consumer_message_idChanged || dispute_statusChanged || statusChanged || commentChanged || reject_reasonChanged || processed_tranChanged || atm_idChanged || generated_atChanged || dispense_amountChanged || start_indexChanged || end_indexChanged || task_idChanged || is_eligibleChanged || is_disputed_transactionChanged || host_tsnChanged || account_noChanged || micrChanged || transaction_type_idChanged || networkChanged || is_cardlessChanged || bank_nameChanged)
            {
                ...
                else
                {
                    if (!(ej_parsed_cpm_transaction_idChanged || trxn_datetimeChanged || terminal_idChanged || seqChanged || account_typeChanged || panChanged || deposit_amountChanged || resultChanged || consumer_message_idChanged || dispute_statusChanged || statusChanged || commentChanged || reject_reasonChanged || processed_tranChanged || atm_idChanged || generated_atChanged || dispense_amountChanged || start_indexChanged || end_indexChanged || task_idChanged || is_eligibleChanged || is_disputed_transactionChanged || host_tsnChanged || account_noChanged || micrChanged || transaction_type_idChanged || networkChanged || is_cardlessChanged || bank_nameChanged))
                        return;
                    qry.Append("UPDATE Ej_parsed_cpm_transaction set "); if (terminal_idChanged)
                    {
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE Ej_parsed_cpm_transaction whereej_parsed_cpm_transaction_id= " + ej_parsed_cpm_transaction_id + " and trxn_datetime= " + trxn_datetime;
        ...
        {
            ConnectionFactory.ExecuteQuery("delete Ej_parsed_cpm_transaction where " + where, DatabaseName.Tx);
        }
        ...
        {
            ej_parsed_cpm_transaction_id = 0,
            trxn_datetime = 1,
            terminal_id = 2,
            seq = 3,
            account_type = 4,
        ...
        public void AddToDataTable(List<EjParsedCpmTransaction> transList, ref DataTable dt)
        {
            foreach (EjParsedCpmTransaction tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
                Row["ej_parsed_cpm_transaction_id"] = ConnectionFactory.GetNextId(DatabaseName.Tx);
