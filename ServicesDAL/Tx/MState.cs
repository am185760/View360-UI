...
namespace ServicesDAL
{
    ...
    public class MState
    {
        ...
        private MState(long mstate_id, string mstate_desc, string device_id, string mState_code, byte? mstate_status)
        {
        ...
        private string mstate_descDbString
        {
            get
            {
                if (this.mstate_desc != null)
                    return string.Format("'{0}'", mstate_desc);
                else
                    return "null";
            }
        }
        ...
        private string device_idDbString
        {
            get
            {
                if (this.device_id != null)
                    return string.Format("'{0}'", device_id);
                else
                    return "null";
            }
        }
        ...
        private string mState_codeDbString
        {
            get
            {
                if (this.mState_code != null)
                    return string.Format("'{0}'", mState_code);
                else
                    return "null";
            }
        }
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.mstate_id) == Columns.mstate_id && reader["mstate_id"] != DBNull.Value)
                            currentMState.mstate_id = (long)reader["mstate_id"];
        ...
        public static MState LoadMStateByPk(long mstate_id)
        {
            return LoadMState("mstate_id=" + mstate_id);
        }

        public static MState LoadMStateByPk(long mstate_id, IDbConnection conn)
        {
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (mstate_idChanged || mstate_descChanged || device_idChanged || mState_codeChanged || mstate_statusChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    ...
                    {
                        this.mstate_id = ConnectionFactory.GetNextId(DatabaseName.Tx);
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE MState wheremstate_id= " + mstate_id;
        ...
        {
            ConnectionFactory.ExecuteQuery("delete MState where " + where, DatabaseName.Tx);
        }
        ...
        #endregion
        public DataTable BulkSave(List<MState> dataArray, SqlTransaction dbTrx)
        {
        ...
        public void AddToDataTable(List<MState> transList, ref DataTable dt)
        {
            foreach (MState tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
                Row["mstate_id"] = ConnectionFactory.GetNextId(DatabaseName.Tx);
