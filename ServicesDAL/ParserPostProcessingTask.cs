...
namespace ServicesDAL
{
    ...
    public class ParserPostProcessingTask
    {
        ...
        public ParserPostProcessingTask(long parser_post_processing_task_id, string event_type, long entity_id, DateTime event_occured_at, long task_id, long atm_id, DateTime creation_time)
        {
        ...
        public ParserPostProcessingTask(string event_type, string event_info, long entity_id, DateTime event_occured_at, long task_id, long atm_id, DateTime creation_time, DateTime? processed_time)
        {
            this.event_type = event_type;
            this.event_typeChanged = true;
        ...
        private ParserPostProcessingTask(long parser_post_processing_task_id, string event_type, string event_info, long entity_id, DateTime event_occured_at, long task_id, long atm_id, DateTime creation_time, DateTime? processed_time)
        {
        ...
        private long parser_post_processing_task_id;
        public long ParserPostProcessingTaskId
        {
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.parser_post_processing_task_id) == Columns.parser_post_processing_task_id && reader["parser_post_processing_task_id"] != DBNull.Value)
                            currentParserPostProcessingTask.parser_post_processing_task_id = (long)reader["parser_post_processing_task_id"];
                        ...
                        if ((columns & Columns.entity_id) == Columns.entity_id && reader["entity_id"] != DBNull.Value)
                            currentParserPostProcessingTask.entity_id = (long)reader["entity_id"];
                        ...
                        if ((columns & Columns.task_id) == Columns.task_id && reader["task_id"] != DBNull.Value)
                            currentParserPostProcessingTask.task_id = (long)reader["task_id"];
                        if ((columns & Columns.atm_id) == Columns.atm_id && reader["atm_id"] != DBNull.Value)
                            currentParserPostProcessingTask.atm_id = (long)reader["atm_id"];
        ...
        static public ParserPostProcessingTaskReader ExecuteReader(string where, Columns columns, DatabaseName databaseName)
        {
        ...
        static public ParserPostProcessingTaskReader ExecuteReader(string where, DatabaseName databaseName)
        {
            return ExecuteReader(where, ConnectionFactory.GetNewConnection(databaseName));
        }

        public static ParserPostProcessingTask LoadParserPostProcessingTask(string where, DatabaseName databaseName)
        {
            ParserPostProcessingTaskReader reader = ParserPostProcessingTask.ExecuteReader(where, databaseName);
        ...
        public static ParserPostProcessingTask LoadParserPostProcessingTaskByPk(long parser_post_processing_task_id, DateTime creation_time, DatabaseName databaseName)
        {
            return LoadParserPostProcessingTask("parser_post_processing_task_id=" + parser_post_processing_task_id + " and creation_time=Convert(datetime,'" + creation_time.ToString("yyyy-MM-dd HH:mm:ss.fff") + "',121)", databaseName);
        }

        public static ParserPostProcessingTask LoadParserPostProcessingTaskByPk(long parser_post_processing_task_id, DateTime creation_time, IDbConnection conn)
        {
            return LoadParserPostProcessingTask(" parser_post_processing_task_id=" + parser_post_processing_task_id + " and creation_time=Convert(datetime,'" + creation_time.ToString("yyyy-MM-dd HH:mm:ss.fff") + "',121)", conn);
        }
        ...
        private void ExcuteSave(IDbCommand cmd, DatabaseName databaseName)
        {
            if (parser_post_processing_task_idChanged || event_typeChanged || event_infoChanged || entity_idChanged || event_occured_atChanged || task_idChanged || atm_idChanged || creation_timeChanged || processed_timeChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    ...
                    {
                        this.parser_post_processing_task_id = ConnectionFactory.GetNextId(databaseName);
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE Parser_post_processing_task whereparser_post_processing_task_id= " + parser_post_processing_task_id + " and creation_time= " + creation_time;
        ...
        public static void DeleteParserPostProcessingTasks(string where, DatabaseName databaseName)
        {
            ConnectionFactory.ExecuteQuery("delete Parser_post_processing_task where " + where, databaseName);
        }
        ...
        {
            parser_post_processing_task_id = 0,
            event_type = 1,
            event_info = 2,
            entity_id = 3,
            event_occured_at = 4,
        ...
        #endregion
        public DataTable BulkSave(List<ParserPostProcessingTask> dataArray, SqlTransaction dbTrx, DatabaseName databaseName)
        {
            DataTable dt = new DataTable();
            CreateDataTable(dt);
            AddToDataTable(dataArray, ref dt, databaseName);
            SqlBulkCopy bulk = new SqlBulkCopy(ConnectionFactory.connectionStringCore);
        ...
        public void AddToDataTable(List<ParserPostProcessingTask> transList, ref DataTable dt, DatabaseName databaseName)
        {
            foreach (ParserPostProcessingTask tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
                Row["parser_post_processing_task_id"] = ConnectionFactory.GetNextId(databaseName);
