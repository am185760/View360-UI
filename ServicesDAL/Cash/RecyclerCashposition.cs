...
{
    [Serializable()]
    public class RecyclerCashposition
    {
        ...
        public RecyclerCashposition() { }
        public RecyclerCashposition(long cash_position_id, long atm_id, long task_id, DateTime last_trxn_at)
        {
            this.atm_id = atm_id;
        ...
        public RecyclerCashposition(long atm_id, int? cassette1_notes, int? cassette2_notes, int? cassette3_notes, int? cassette4_notes, int? cassette5_notes, int? cassette6_notes, int? cassette7_notes, long task_id, DateTime last_trxn_at, int? purge_cassette1_notes, int? purge_cassette2_notes, int? purge_cassette3_notes, int? purge_cassette4_notes, int? purge_cassette5_notes, int? purge_cassette6_notes, int? purge_cassette7_notes, decimal? total_cash_balance, decimal? total_purged_cash_balance)
        {
            this.atm_id = atm_id;
            this.atm_idChanged = true;
            ...
            this.total_cash_balanceChanged = true;
            this.total_purged_cash_balance = total_purged_cash_balance;
            this.total_purged_cash_balanceChanged = true;
        }
        private RecyclerCashposition(long cash_position_id, long atm_id, int? cassette1_notes, int? cassette2_notes, int? cassette3_notes, int? cassette4_notes, int? cassette5_notes, int? cassette6_notes, int? cassette7_notes, long task_id, DateTime last_trxn_at, int? purge_cassette1_notes, int? purge_cassette2_notes, int? purge_cassette3_notes, int? purge_cassette4_notes, int? purge_cassette5_notes, int? purge_cassette6_notes, int? purge_cassette7_notes, decimal? total_cash_balance, decimal? total_purged_cash_balance)
        {
        ...
        private long cash_position_id;
        public long CashPositionId
        {
        ...
        #endregion
        #region TotalCashBalance
        private bool total_cash_balanceChanged = false;
        private decimal? total_cash_balance;
        public decimal? TotalCashBalance
        {
            get { return total_cash_balance; }
            set
            {
                total_cash_balance = value;
                total_cash_balanceChanged = true;
            }
        }
        private string total_cash_balanceDbString
        {
            get
            {
                if (this.total_cash_balance.HasValue)
                    return total_cash_balance.ToString();
                else
                    return "null";
            }
        }
        #endregion
        #region TotalPurgedCashBalance
        private bool total_purged_cash_balanceChanged = false;
        private decimal? total_purged_cash_balance;
        public decimal? TotalPurgedCashBalance
        {
            get { return total_purged_cash_balance; }
            set
            {
                total_purged_cash_balance = value;
                total_purged_cash_balanceChanged = true;
            }
        }
        private string total_purged_cash_balanceDbString
        {
            get
            {
                if (this.total_purged_cash_balance.HasValue)
                    return total_purged_cash_balance.ToString();
                else
                    return "null";
            }
        }
        ...
        #region RecyclerCashpositionReader
        public class RecyclerCashpositionReader : IEntityReader, IEnumerator, IEnumerable
        {
            IDataReader reader;
            IDbConnection conn;
            RecyclerCashposition currentRecyclerCashposition;
            Columns columns;
            bool partialRead = false;
            private RecyclerCashpositionReader() { }
            /// 
            ///
            ///
            ...
            public RecyclerCashpositionReader(IDataReader reader, IDbConnection conn)
            {
                this.reader = reader;
                this.conn = conn;
            }
            public RecyclerCashpositionReader(IDataReader reader, IDbConnection conn, Columns columns)
            {
                this.reader = reader;
                this.conn = conn;
            ...
            {
                get { return currentRecyclerCashposition; }
            ...
            public bool Read()
            {
                if (reader.Read())
                {
                    currentRecyclerCashposition = new RecyclerCashposition();
                    if (partialRead)
                    {
                        if ((columns & Columns.cash_position_id) == Columns.cash_position_id && reader["cash_position_id"] != DBNull.Value)
                            currentRecyclerCashposition.cash_position_id = (long)reader["cash_position_id"];
                        if ((columns & Columns.atm_id) == Columns.atm_id && reader["atm_id"] != DBNull.Value)
                            currentRecyclerCashposition.atm_id = (long)reader["atm_id"];
                        if ((columns & Columns.cassette1_notes) == Columns.cassette1_notes && reader["cassette1_notes"] != DBNull.Value)
                            currentRecyclerCashposition.cassette1_notes = (int?)reader["cassette1_notes"];
                        if ((columns & Columns.cassette2_notes) == Columns.cassette2_notes && reader["cassette2_notes"] != DBNull.Value)
                            currentRecyclerCashposition.cassette2_notes = (int?)reader["cassette2_notes"];
                        if ((columns & Columns.cassette3_notes) == Columns.cassette3_notes && reader["cassette3_notes"] != DBNull.Value)
                            currentRecyclerCashposition.cassette3_notes = (int?)reader["cassette3_notes"];
                        if ((columns & Columns.cassette4_notes) == Columns.cassette4_notes && reader["cassette4_notes"] != DBNull.Value)
                            currentRecyclerCashposition.cassette4_notes = (int?)reader["cassette4_notes"];
                        if ((columns & Columns.cassette5_notes) == Columns.cassette5_notes && reader["cassette5_notes"] != DBNull.Value)
                            currentRecyclerCashposition.cassette5_notes = (int?)reader["cassette5_notes"];
                        if ((columns & Columns.cassette6_notes) == Columns.cassette6_notes && reader["cassette6_notes"] != DBNull.Value)
                            currentRecyclerCashposition.cassette6_notes = (int?)reader["cassette6_notes"];
                        if ((columns & Columns.cassette7_notes) == Columns.cassette7_notes && reader["cassette7_notes"] != DBNull.Value)
                            currentRecyclerCashposition.cassette7_notes = (int?)reader["cassette7_notes"];
                        if ((columns & Columns.task_id) == Columns.task_id && reader["task_id"] != DBNull.Value)
                            currentRecyclerCashposition.task_id = (long)reader["task_id"];
                        if ((columns & Columns.last_trxn_at) == Columns.last_trxn_at && reader["last_trxn_at"] != DBNull.Value)
                            currentRecyclerCashposition.last_trxn_at = (DateTime)reader["last_trxn_at"];
                        if ((columns & Columns.purge_cassette1_notes) == Columns.purge_cassette1_notes && reader["purge_cassette1_notes"] != DBNull.Value)
                            currentRecyclerCashposition.purge_cassette1_notes = (int?)reader["purge_cassette1_notes"];
                        if ((columns & Columns.purge_cassette2_notes) == Columns.purge_cassette2_notes && reader["purge_cassette2_notes"] != DBNull.Value)
                            currentRecyclerCashposition.purge_cassette2_notes = (int?)reader["purge_cassette2_notes"];
                        if ((columns & Columns.purge_cassette3_notes) == Columns.purge_cassette3_notes && reader["purge_cassette3_notes"] != DBNull.Value)
                            currentRecyclerCashposition.purge_cassette3_notes = (int?)reader["purge_cassette3_notes"];
                        if ((columns & Columns.purge_cassette4_notes) == Columns.purge_cassette4_notes && reader["purge_cassette4_notes"] != DBNull.Value)
                            currentRecyclerCashposition.purge_cassette4_notes = (int?)reader["purge_cassette4_notes"];
                        if ((columns & Columns.purge_cassette5_notes) == Columns.purge_cassette5_notes && reader["purge_cassette5_notes"] != DBNull.Value)
                            currentRecyclerCashposition.purge_cassette5_notes = (int?)reader["purge_cassette5_notes"];
                        if ((columns & Columns.purge_cassette6_notes) == Columns.purge_cassette6_notes && reader["purge_cassette6_notes"] != DBNull.Value)
                            currentRecyclerCashposition.purge_cassette6_notes = (int?)reader["purge_cassette6_notes"];
                        if ((columns & Columns.purge_cassette7_notes) == Columns.purge_cassette7_notes && reader["purge_cassette7_notes"] != DBNull.Value)
                            currentRecyclerCashposition.purge_cassette7_notes = (int?)reader["purge_cassette7_notes"];
                        if ((columns & Columns.total_cash_balance) == Columns.total_cash_balance && reader["total_cash_balance"] != DBNull.Value)
                            currentRecyclerCashposition.total_cash_balance = (decimal?)reader["total_cash_balance"];
                        if ((columns & Columns.total_purged_cash_balance) == Columns.total_purged_cash_balance && reader["total_purged_cash_balance"] != DBNull.Value)
                            currentRecyclerCashposition.total_purged_cash_balance = (decimal?)reader["total_purged_cash_balance"];
                    ...
                    else
                    {
                        ...
                        if (reader["purge_cassette7_notes"] != DBNull.Value)
                            currentRecyclerCashposition.purge_cassette7_notes = (int?)reader["purge_cassette7_notes"];
                        if (reader["total_cash_balance"] != DBNull.Value)
                            currentRecyclerCashposition.total_cash_balance = (decimal?)reader["total_cash_balance"];
                        if (reader["total_purged_cash_balance"] != DBNull.Value)
                            currentRecyclerCashposition.total_purged_cash_balance = (decimal?)reader["total_purged_cash_balance"];
                    }

                    currentRecyclerCashposition.isNewEntity = false;
                    return true;
                }
                else
                    return false;
            ...
            public RecyclerCashposition CurrentRecyclerCashposition
            {
                get { return currentRecyclerCashposition; }
            }
        ...
        #region RecyclerCashposition functions

        public static RecyclerCashpositionReader ExecuteReader(string where, IDbConnection conn, Columns columns)
        {
            ...
            if (Columns.total_cash_balance == (Columns.total_cash_balance & columns))
                qry.Append("total_cash_balance,");
            if (Columns.total_purged_cash_balance == (Columns.total_purged_cash_balance & columns))
                qry.Append("total_purged_cash_balance,");
            qry.Replace(',', ' ', qry.Length - 1, 1);
            qry.Append("from Recycler_cashposition ");
            ...
            return new RecyclerCashpositionReader(cmd.ExecuteReader(), conn, columns);
        }

        static public RecyclerCashpositionReader ExecuteReader(string where, Columns columns)
        {
        ...
        /// 
        /// 
        public static RecyclerCashpositionReader ExecuteReader(string where, IDbConnection conn)
        {
            if (conn.State != ConnectionState.Open)
                conn.Open();
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
            cmd.ExecuteNonQuery();
            cmd.CommandText = "Select cash_position_id,atm_id,cassette1_notes,cassette2_notes,cassette3_notes,cassette4_notes,cassette5_notes,cassette6_notes,cassette7_notes,task_id,last_trxn_at,purge_cassette1_notes,purge_cassette2_notes,purge_cassette3_notes,purge_cassette4_notes,purge_cassette5_notes,purge_cassette6_notes,purge_cassette7_notes,total_cash_balance,total_purged_cash_balance from Recycler_cashposition ";
            if (where != null && where.Trim().Length > 0)
                cmd.CommandText = string.Format("{0} where {1}", cmd.CommandText, where);

            return new RecyclerCashpositionReader(cmd.ExecuteReader(), conn);
        }

        static public RecyclerCashpositionReader ExecuteReader(string where)
        {
            return ExecuteReader(where, ConnectionFactory.GetNewConnection(DatabaseName.Cash));
        }

        public static RecyclerCashposition LoadRecyclerCashposition(string where)
        {
            RecyclerCashpositionReader reader = RecyclerCashposition.ExecuteReader(where);
            RecyclerCashposition _recyclercashposition = null;
            if (reader.Read())
                _recyclercashposition = reader.CurrentRecyclerCashposition;
            reader.Close();
            return _recyclercashposition;
        }

        public static RecyclerCashposition LoadRecyclerCashposition(string where, IDbConnection conn)
        {
            RecyclerCashpositionReader reader = RecyclerCashposition.ExecuteReader(where, conn);
            RecyclerCashposition _recyclercashposition = null;
            if (reader.Read())
                _recyclercashposition = reader.CurrentRecyclerCashposition;
            reader.Close(false);
        ...
        public static RecyclerCashposition LoadRecyclerCashpositionByPk(long cash_position_id, DateTime last_trxn_at)
        {
            return LoadRecyclerCashposition("cash_position_id=" + cash_position_id + " and last_trxn_at=Convert(datetime,'" + last_trxn_at.ToString("yyyy-MM-dd HH:mm:ss.fff") + "',121)");
        }

        public static RecyclerCashposition LoadRecyclerCashpositionByPk(long cash_position_id, DateTime last_trxn_at, IDbConnection conn)
        {
            return LoadRecyclerCashposition(" cash_position_id=" + cash_position_id + " and last_trxn_at=Convert(datetime,'" + last_trxn_at.ToString("yyyy-MM-dd HH:mm:ss.fff") + "',121)", conn);
        }

        public void Save()
        {
            if (cash_position_idChanged || atm_idChanged || cassette1_notesChanged || cassette2_notesChanged || cassette3_notesChanged || cassette4_notesChanged || cassette5_notesChanged || cassette6_notesChanged || cassette7_notesChanged || task_idChanged || last_trxn_atChanged || purge_cassette1_notesChanged || purge_cassette2_notesChanged || purge_cassette3_notesChanged || purge_cassette4_notesChanged || purge_cassette5_notesChanged || purge_cassette6_notesChanged || purge_cassette7_notesChanged || total_cash_balanceChanged || total_purged_cash_balanceChanged)
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (cash_position_idChanged || atm_idChanged || cassette1_notesChanged || cassette2_notesChanged || cassette3_notesChanged || cassette4_notesChanged || cassette5_notesChanged || cassette6_notesChanged || cassette7_notesChanged || task_idChanged || last_trxn_atChanged || purge_cassette1_notesChanged || purge_cassette2_notesChanged || purge_cassette3_notesChanged || purge_cassette4_notesChanged || purge_cassette5_notesChanged || purge_cassette6_notesChanged || purge_cassette7_notesChanged || total_cash_balanceChanged || total_purged_cash_balanceChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    qry.Append(@"insert into Recycler_cashposition(cash_position_id,atm_id,cassette1_notes,cassette2_notes,cassette3_notes,cassette4_notes,cassette5_notes,cassette6_notes,cassette7_notes,task_id,last_trxn_at,purge_cassette1_notes,purge_cassette2_notes,purge_cassette3_notes,purge_cassette4_notes,purge_cassette5_notes,purge_cassette6_notes,purge_cassette7_notes,total_cash_balance,total_purged_cash_balance) values(");
                    lock (ConnectionFactory.connectionStringCash)
                    {
                        this.cash_position_id = ConnectionFactory.GetNextId(DatabaseName.Cash);
                    ...
                    qry.Append(total_cash_balanceDbString + ",");
                    qry.Append(total_purged_cash_balanceDbString);
                    qry.Append(");");
                ...
                else
                {
                    if (!(cash_position_idChanged || atm_idChanged || cassette1_notesChanged || cassette2_notesChanged || cassette3_notesChanged || cassette4_notesChanged || cassette5_notesChanged || cassette6_notesChanged || cassette7_notesChanged || task_idChanged || last_trxn_atChanged || purge_cassette1_notesChanged || purge_cassette2_notesChanged || purge_cassette3_notesChanged || purge_cassette4_notesChanged || purge_cassette5_notesChanged || purge_cassette6_notesChanged || purge_cassette7_notesChanged || total_cash_balanceChanged || total_purged_cash_balanceChanged))
                        return;
                    qry.Append("UPDATE Recycler_cashposition set "); if (atm_idChanged)
                    {
                    ...
                    if (total_cash_balanceChanged)
                    {
                        qry.Append("total_cash_balance =" + total_cash_balanceDbString);
                        qry.Append(",");
                    }

                    if (total_purged_cash_balanceChanged)
                    {
                        qry.Append("total_purged_cash_balance =" + total_purged_cash_balanceDbString);
                        qry.Append(",");
                    }
                    ...
                    qry.Append(" and last_trxn_at = " + last_trxn_atDbString);
                }
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE Recycler_cashposition wherecash_position_id= " + cash_position_id + " and last_trxn_at= " + last_trxn_at;
        ...
        public static void DeleteRecyclerCashpositions(string where)
        {
            ConnectionFactory.ExecuteQuery("delete Recycler_cashposition where " + where, DatabaseName.Cash);
        }
        ...
        public enum Columns : uint
        {
            ...
            task_id = 9,
            last_trxn_at = 10,
            purge_cassette1_notes = 11,
            purge_cassette2_notes = 12,
            purge_cassette3_notes = 13,
            purge_cassette4_notes = 14,
            purge_cassette5_notes = 15,
            purge_cassette6_notes = 16,
            purge_cassette7_notes = 17,
            total_cash_balance = 18,
            total_purged_cash_balance = 19
        }
        #endregion
        public DataTable BulkSave(List<RecyclerCashposition> dataArray, SqlTransaction dbTrx)
        {
            DataTable dt = new DataTable();
            CreateDataTable(dt);
            AddToDataTable(dataArray, ref dt);
            SqlBulkCopy bulk = new SqlBulkCopy(dbTrx.Connection, SqlBulkCopyOptions.Default, dbTrx);
            bulk.DestinationTableName = "Recycler_cashposition";
            bulk.WriteToServer(dt); return dt;
        }
        public void CreateDataTable(DataTable dt)
        {
            string[] colNames = Enum.GetNames(typeof(RecyclerCashposition.Columns));
            for (int i = 0; i < colNames.Length; i++)
            {
                dt.Columns.Add(colNames[i]);
            }
        }
        public void AddToDataTable(List<RecyclerCashposition> transList, ref DataTable dt)
        {
            foreach (RecyclerCashposition tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
                Row["cash_position_id"] = ConnectionFactory.GetNextId(DatabaseName.Cash);
                ...
                Row["total_cash_balance"] = tran.TotalCashBalance;
                Row["total_purged_cash_balance"] = tran.TotalPurgedCashBalance;
                dt.Rows.Add(Row);
            }
        }
    }
