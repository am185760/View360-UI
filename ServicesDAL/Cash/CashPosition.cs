...
namespace ServicesDAL
{
    ...
    public class CashPosition
    {
        ...
        public CashPosition() { }
        public CashPosition(long cash_position_id, long atm_id, long task_id, DateTime last_trxn_at)
        {
            this.atm_id = atm_id;
        ...
        private CashPosition(long cash_position_id, long atm_id, int? cassette1_notes, int? cassette2_notes, int? cassette3_notes, int? cassette4_notes, int? cassette5_notes, int? cassette6_notes, int? cassette7_notes, long task_id, DateTime last_trxn_at, int? purge_cassette1_notes, int? purge_cassette2_notes, int? purge_cassette3_notes, int? purge_cassette4_notes, int? purge_cassette5_notes, int? purge_cassette6_notes, int? purge_cassette7_notes, decimal? total_cash_balance, decimal? total_purged_cash_balance)
        {
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.cash_position_id) == Columns.cash_position_id && reader["cash_position_id"] != DBNull.Value)
                            currentCashPosition.cash_position_id = (long)reader["cash_position_id"];
                        if ((columns & Columns.atm_id) == Columns.atm_id && reader["atm_id"] != DBNull.Value)
                            currentCashPosition.atm_id = (long)reader["atm_id"];
                        ...
                        if ((columns & Columns.task_id) == Columns.task_id && reader["task_id"] != DBNull.Value)
                            currentCashPosition.task_id = (long)reader["task_id"];
                        ...
                        if ((columns & Columns.total_cash_balance) == Columns.total_cash_balance && reader["total_cash_balance"] != DBNull.Value)
                            currentCashPosition.total_cash_balance = (decimal?)reader["total_cash_balance"];
                        if ((columns & Columns.total_purged_cash_balance) == Columns.total_purged_cash_balance && reader["total_purged_cash_balance"] != DBNull.Value)
                            currentCashPosition.total_purged_cash_balance = (decimal?)reader["total_purged_cash_balance"];
        ...
        public static CashPosition LoadCashPositionByPk(long cash_position_id, DateTime last_trxn_at)
        {
            return LoadCashPosition("cash_position_id=" + cash_position_id + " and last_trxn_at=Convert(datetime,'" + last_trxn_at.ToString("yyyy-MM-dd HH:mm:ss.fff") + "',121)");
        }

        public static CashPosition LoadCashPositionByPk(long cash_position_id, DateTime last_trxn_at, IDbConnection conn)
        {
            return LoadCashPosition(" cash_position_id=" + cash_position_id + " and last_trxn_at=Convert(datetime,'" + last_trxn_at.ToString("yyyy-MM-dd HH:mm:ss.fff") + "',121)", conn);
        }
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE Cash_position wherecash_position_id= " + cash_position_id + " and last_trxn_at= " + last_trxn_at;
        ...
        {
            ConnectionFactory.ExecuteQuery("delete Cash_position where " + where, DatabaseName.Cash);
        }
        ...
        #endregion
        public DataTable BulkSave(List<CashPosition> dataArray, SqlTransaction dbTrx)
        {
