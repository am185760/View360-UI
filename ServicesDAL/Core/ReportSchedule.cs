...
namespace ServicesDAL
{
    ...
    public class ReportSchedule
    {
        ...
        public ReportSchedule(long report_schedule_id, string report_name, string report_receipients, string report_temp_path, int retry_count, DateTime report_next_generated_at, string report_friendly_name, bool schedule_type, bool is_ej_enabled, bool is_graphical_report, bool is_weekly, bool is_monthly)
        {
        ...
        public ReportSchedule(string report_name, string report_physical_path, string report_receipients, string report_temp_path, int retry_count, DateTime report_next_generated_at, string report_friendly_name, int? minutes_to_schedule_again, short? report_export_type, int? report_data_age, bool schedule_type, long? organization_id, bool is_ej_enabled, string report_virtual_dir_path, bool is_graphical_report, long? criteria_id, bool is_weekly, bool is_monthly, string applicable_note_set_type, long? cit_id)
        {
            this.report_name = report_name;
            this.report_nameChanged = true;
        ...
        private ReportSchedule(long report_schedule_id, string report_name, string report_physical_path, string report_receipients, string report_temp_path, int retry_count, DateTime report_next_generated_at, string report_friendly_name, int? minutes_to_schedule_again, short? report_export_type, int? report_data_age, bool schedule_type, long? organization_id, bool is_ej_enabled, string report_virtual_dir_path, bool is_graphical_report, long? criteria_id, bool is_weekly, bool is_monthly, string applicable_note_set_type, long? cit_id)
        {
        ...
        private string report_physical_pathDbString
        {
            get
            {
                if (this.report_physical_path != null)
                    return string.Format("'{0}'", report_physical_path);
                else
                    return "null";
            }
        }
        ...
        private string report_receipientsDbString
        {
            get
            {
                if (this.report_receipients != null)
                    return string.Format("'{0}'", report_receipients);
                else
                    return "null";
            }
        }
        ...
        private string report_temp_pathDbString
        {
            get
            {
                if (this.report_temp_path != null)
                    return string.Format("'{0}'", report_temp_path);
                else
                    return "null";
            }
        }
        ...
        private string report_friendly_nameDbString
        {
            get
            {
                if (this.report_friendly_name != null)
                    return string.Format("'{0}'", report_friendly_name);
                else
                    return "null";
            }
        }
        ...
        private string report_virtual_dir_pathDbString
        {
            get
            {
                if (this.report_virtual_dir_path != null)
                    return string.Format("'{0}'", report_virtual_dir_path);
                else
                    return "null";
            }
        }
        ...
        private string applicable_note_set_typeDbString
        {
            get
            {
                if (this.applicable_note_set_type != null)
                    return string.Format("'{0}'", applicable_note_set_type);
                else
                    return "null";
            }
        }
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.report_schedule_id) == Columns.report_schedule_id && reader["report_schedule_id"] != DBNull.Value)
                            currentReportSchedule.report_schedule_id = (long)reader["report_schedule_id"];
                        ...
                        if ((columns & Columns.organization_id) == Columns.organization_id && reader["organization_id"] != DBNull.Value)
                            currentReportSchedule.organization_id = (long?)reader["organization_id"];
                        ...
                        if ((columns & Columns.criteria_id) == Columns.criteria_id && reader["criteria_id"] != DBNull.Value)
                            currentReportSchedule.criteria_id = (long?)reader["criteria_id"];
                        ...
                        if ((columns & Columns.cit_id) == Columns.cit_id && reader["cit_id"] != DBNull.Value)
                            currentReportSchedule.cit_id = (long?)reader["cit_id"];
        ...
        public static ReportSchedule LoadReportScheduleByPk(long report_schedule_id)
        {
            return LoadReportSchedule("report_schedule_id=" + report_schedule_id);
        }

        public static ReportSchedule LoadReportScheduleByPk(long report_schedule_id, IDbConnection conn)
        {
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (report_schedule_idChanged || report_nameChanged || report_physical_pathChanged || report_receipientsChanged || report_temp_pathChanged || retry_countChanged || report_next_generated_atChanged || report_friendly_nameChanged || minutes_to_schedule_againChanged || report_export_typeChanged || report_data_ageChanged || schedule_typeChanged || organization_idChanged || is_ej_enabledChanged || report_virtual_dir_pathChanged || is_graphical_reportChanged || criteria_idChanged || is_weeklyChanged || is_monthlyChanged || applicable_note_set_typeChanged || cit_idChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    ...
                    {
                        this.report_schedule_id = ConnectionFactory.GetNextId(DatabaseName.Core);
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE Report_schedule wherereport_schedule_id= " + report_schedule_id;
        ...
        {
            ConnectionFactory.ExecuteQuery("delete Report_schedule where " + where, DatabaseName.Core);
        }
        ...
        public enum Columns : uint
        {
            ...
            report_name = 1,
            report_physical_path = 2,
            report_receipients = 3,
            report_temp_path = 4,
            retry_count = 5,
            report_next_generated_at = 6,
            report_friendly_name = 7,
            minutes_to_schedule_again = 8,
            report_export_type = 9,
            report_data_age = 10,
            schedule_type = 11,
            organization_id = 12,
            is_ej_enabled = 13,
            report_virtual_dir_path = 14,
            is_graphical_report = 15,
            criteria_id = 16,
            is_weekly = 17,
            is_monthly = 18,
            applicable_note_set_type = 19,
            cit_id = 20
        }
        #endregion
        public DataTable BulkSave(List<ReportSchedule> dataArray, SqlTransaction dbTrx)
        {
        ...
        public void AddToDataTable(List<ReportSchedule> transList, ref DataTable dt)
        {
            foreach (ReportSchedule tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
                Row["report_schedule_id"] = ConnectionFactory.GetNextId(DatabaseName.Core);
