...
namespace ServicesDAL
{
    ...
    public class FileType
    {
        ...
        private FileType(long file_type_id, string path_at_ATM, string file_type_title, string copy_type, bool is_EJLog)
        {
        ...
        private string path_at_ATMDbString
        {
            get
            {
                if (this.path_at_ATM != null)
                    return string.Format("'{0}'", path_at_ATM);
                else
                    return "null";
            }
        }
        ...
        private string file_type_titleDbString
        {
            get
            {
                if (this.file_type_title != null)
                    return string.Format("'{0}'", file_type_title);
                else
                    return "null";
            }
        }
        ...
        private string copy_typeDbString
        {
            get
            {
                if (this.copy_type != null)
                    return string.Format("'{0}'", copy_type);
                else
                    return "null";
            }
        }
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        if ((columns & Columns.file_type_id) == Columns.file_type_id && reader["file_type_id"] != DBNull.Value)
                            currentFileType.file_type_id = long.Parse(reader["file_type_id"].ToString());
        ...
        public static FileType LoadFileTypeByPk(long file_type_id)
        {
            return LoadFileType("file_type_id=" + file_type_id);
        }

        public static FileType LoadFileTypeByPk(long file_type_id, IDbConnection conn)
        {
        ...
        private void ExcuteSave(IDbCommand cmd)
        {
            if (file_type_idChanged || path_at_ATMChanged || file_type_titleChanged || copy_typeChanged || is_EJLogChanged)
            {
                ...
                if (this.isNewEntity)
                {
                    ...
                    {
                        this.file_type_id = ConnectionFactory.GetNextId(DatabaseName.Core);
        ...
        {
            ConnectionFactory.ExecuteQuery("delete File_type where  " + where , DatabaseName.Core);
        }
        ...
        #endregion
        public DataTable BulkSave(List<FileType> dataArray, SqlTransaction dbTrx)
        {
        ...
        public void AddToDataTable(List<FileType> transList, ref DataTable dt)
        {
            foreach (FileType tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
                Row["file_type_id"] = ConnectionFactory.GetNextId(DatabaseName.Core);
