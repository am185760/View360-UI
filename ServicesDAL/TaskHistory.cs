...
namespace TxDAL
{
    ...
    public class TaskHistory
    {
        ...
        public TaskHistory() { }
        public TaskHistory(long task_id, int bytes_transferred, long aTM_id, DateTime creation_time, string status, long created_by, byte retry_Remaining, long task_type_id)
        {
        ...
        private TaskHistory(long task_id, bool? parsed, int bytes_transferred, string file_path_at_ATM, long aTM_id, long? file_type_id, DateTime creation_time, DateTime? download_time, DateTime? upload_time, DateTime? end_time, string status, int? zipped_file_size, long created_by, int? unZipped_file_size, DateTime? last_invoked, byte retry_Remaining, string failure_reason, string server_filepath, long task_type_id, long? cash_order_id, long? downloading_schedule_id, int? failed_to_parse_count, string archive_file_path_at_atm, string task_info)
        {
        ...
        #region TaskHistoryReader
        public class TaskHistoryReader : IEntityReader, IEnumerator, IEnumerable
        {
            IDataReader reader;
            IDbConnection conn;
            ...
            public TaskHistoryReader(IDataReader reader, IDbConnection conn)
            {
                this.reader = reader;
                this.conn = conn;
            }
            public TaskHistoryReader(IDataReader reader, IDbConnection conn, Columns columns)
            {
                this.reader = reader;
                this.conn = conn;
                ...
                if (reader.Read())
                {
                    ...
                    if (partialRead)
                    {
                        ...
                        if ((columns & Columns.archive_file_path_at_atm) == Columns.archive_file_path_at_atm && reader["archive_file_path_at_atm"] != DBNull.Value)
                            currentTaskHistory.archive_file_path_at_atm = (string)reader["archive_file_path_at_atm"];
                        if ((columns & Columns.task_info) == Columns.task_info && reader["task_info"] != DBNull.Value)
                            currentTaskHistory.task_info = (string)reader["task_info"];
                    ...
                    currentTaskHistory.isNewEntity = false;
                    return true;
                }
                else
                    return false;
            ...
            public TaskHistory CurrentTaskHistory
            {
                get { return currentTaskHistory; }
            }
        ...
        #region TaskHistory functions

        public static TaskHistoryReader ExecuteReader(string where, IDbConnection conn, Columns columns)
        {
            ...
            return new TaskHistoryReader(cmd.ExecuteReader(), conn, columns);
        }

        static public TaskHistoryReader ExecuteReader(string where, Columns columns, DatabaseName databaseName)
        {
        ...
        /// 
        /// 
        public static TaskHistoryReader ExecuteReader(string where, IDbConnection conn)
        {
            if (conn.State != ConnectionState.Open)
                conn.Open();
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED";
            cmd.ExecuteNonQuery();
            cmd.CommandText = "Selecttask_id,parsed,bytes_transferred,file_path_at_ATM,ATM_id,file_type_id,creation_time,download_time,upload_time,end_time,status,zipped_file_size,created_by,unZipped_file_size,last_invoked,retry_Remaining,failure_reason,server_filepath,task_type_id,cash_order_id,downloading_schedule_id,failed_to_parse_count,archive_file_path_at_atm,task_infofrom Task_history ";
            if (where != null && where.Trim().Length > 0)
                cmd.CommandText = string.Format("{0} where {1}", cmd.CommandText, where);

            return new TaskHistoryReader(cmd.ExecuteReader(), conn);
        }

        static public TaskHistoryReader ExecuteReader(string where, DatabaseName databaseName)
        {
            return ExecuteReader(where, ConnectionFactory.GetNewConnection(databaseName));
        }

        public static TaskHistory LoadTaskHistory(string where, DatabaseName databaseName)
        {
            TaskHistoryReader reader = TaskHistory.ExecuteReader(where, databaseName);
            TaskHistory _taskhistory = null;
            if (reader.Read())
                _taskhistory = reader.CurrentTaskHistory;
            reader.Close();
            return _taskhistory;
        }

        public static TaskHistory LoadTaskHistory(string where, IDbConnection conn)
        {
            TaskHistoryReader reader = TaskHistory.ExecuteReader(where, conn);
            TaskHistory _taskhistory = null;
            if (reader.Read())
                _taskhistory = reader.CurrentTaskHistory;
            reader.Close(false);
        ...
        public static TaskHistory LoadTaskHistoryByPk(long task_id, DatabaseName databaseName)
        {
            return LoadTaskHistory("task_id=" + task_id, databaseName);
        }

        public static TaskHistory LoadTaskHistoryByPk(long task_id, IDbConnection conn)
        {
            return LoadTaskHistory(" task_id=" + task_id, conn);
        }
        ...
        private void ExcuteSave(IDbCommand cmd, DatabaseName databaseName)
        {
            if (task_idChanged || parsedChanged || bytes_transferredChanged || file_path_at_ATMChanged || aTM_idChanged || file_type_idChanged || creation_timeChanged || download_timeChanged || upload_timeChanged || end_timeChanged || statusChanged || zipped_file_sizeChanged || created_byChanged || unZipped_file_sizeChanged || last_invokedChanged || retry_RemainingChanged || failure_reasonChanged || server_filepathChanged || task_type_idChanged || cash_order_idChanged || downloading_schedule_idChanged || failed_to_parse_countChanged || archive_file_path_at_atmChanged || task_infoChanged)
            {
                ...
                else
                {
                    if (!(task_idChanged || parsedChanged || bytes_transferredChanged || file_path_at_ATMChanged || aTM_idChanged || file_type_idChanged || creation_timeChanged || download_timeChanged || upload_timeChanged || end_timeChanged || statusChanged || zipped_file_sizeChanged || created_byChanged || unZipped_file_sizeChanged || last_invokedChanged || retry_RemainingChanged || failure_reasonChanged || server_filepathChanged || task_type_idChanged || cash_order_idChanged || downloading_schedule_idChanged || failed_to_parse_countChanged || archive_file_path_at_atmChanged || task_infoChanged))
                        return;
                    qry.Append("UPDATE Task_history set "); if (parsedChanged)
                    {
        ...
        {
            IDbCommand cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE Task_history wheretask_id= " + task_id;
        ...
        public static void DeleteTaskHistorys(string where)
        {
            ConnectionFactory.ExecuteQuery("delete Task_history where " + where, DatabaseName.Cash);
        }
        ...
        #endregion
        public DataTable BulkSave(List<TaskHistory> dataArray, SqlTransaction dbTrx, DatabaseName databaseName)
        {
            DataTable dt = new DataTable();
            CreateDataTable(dt);
            AddToDataTable(dataArray, ref dt, databaseName);
            SqlBulkCopy bulk = new SqlBulkCopy(dbTrx.Connection, SqlBulkCopyOptions.Default, dbTrx);
            bulk.DestinationTableName = "Task_history";
            bulk.WriteToServer(dt); return dt;
        }
        public void CreateDataTable(DataTable dt)
        {
            string[] colNames = Enum.GetNames(typeof(TaskHistory.Columns));
            for (int i = 0; i < colNames.Length; i++)
            {
                dt.Columns.Add(colNames[i]);
            }
        }
        public void AddToDataTable(List<TaskHistory> transList, ref DataTable dt, DatabaseName databaseName)
        {
            foreach (TaskHistory tran in transList)
            {
                DataRow Row;
                Row = dt.NewRow();
